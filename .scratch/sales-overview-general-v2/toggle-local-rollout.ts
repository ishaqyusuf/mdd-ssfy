...
try {
	switch (mode) {
		case "read":
			console.log(
				JSON.stringify({ hasRolloutPolicy, policy: currentPolicy }, null, 2),
			);
			break;
		case "set-office-v2":
			await persist({
			...
			console.log("Local office rollout set to V2 for QA.");
			break;
		case "restore-missing": {
			const { salesOverviewView: _rollout, ...restoredMeta } = currentMeta;
			await persist(restoredMeta);
			console.log("Removed the temporary local rollout policy.");
			break;
		}
		case "restore": {
			...
			await persist({
				...currentMeta,
				salesOverviewView: normalizeSalesOverviewViewSettings(JSON.parse(value)),
			});
			console.log("Restored the previous local rollout policy.");
			break;
		}
		default:
