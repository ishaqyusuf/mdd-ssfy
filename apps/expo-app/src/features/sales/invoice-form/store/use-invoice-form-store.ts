...
const steps: InvoiceFormStep[] = ["customer", "details", "items", "costs", "review"];
...
type InvoiceFormState = {
  ...
  actions: {
    ...
    updateExtraCost: (type: NewSalesFormExtraCost["type"], amount: number) => void;
...
export const useInvoiceFormStore = create<InvoiceFormState>((set, get) => {
  ...
  return {
    ...
    actions: {
      ...
      applyResolvedCustomer(customer, options) {
        set((state) => {
          const nextMeta = {
            ...
            billingAddressId:
              customer.billing?.id ?? customer.billingId ?? state.meta.billingAddressId ?? null,
            shippingAddressId:
      ...
      addBlankLineItem() {
        set((state) => ({
          ...fromSharedSalesFormState(addSalesFormLineItem(sharedSnapshot(state))),
          selectorOpen: false,
          workflowSelectorLineUid: null,
          validationError: null,
        }));
      ...
      removeLineItem(uid) {
        set((state) => ({
          ...fromSharedSalesFormState(removeSalesFormLineItem(sharedSnapshot(state), uid)),
          validationError: null,
        }));
      },
      ...
      updateExtraCost(type, amount) {
        set((state) => {
          const index = state.extraCosts.findIndex((cost) => cost.type === type);
      ...
      removeExtraCostAtIndex(index) {
        set((state) =>
          fromSharedSalesFormState(removeSalesFormExtraCost(sharedSnapshot(state), index)),
        );
      },
      ...
      setTaxRate(taxRate) {
        set((state) =>
          fromSharedSalesFormState(setSalesFormTaxRate(sharedSnapshot(state), taxRate)),
        );
      },
      validateBeforeSave() {
        const state = get();
...
function mapResolvedCustomer(
  ...
  return {
    ...
    billingAddressId:
      Number(value.billing?.id ?? value.billingId ?? fallback?.billingAddressId ?? 0) ||
      0,
    shippingAddressId:
      Number(
        value.shipping?.id ?? value.shippingId ?? fallback?.shippingAddressId ?? 0,
      ) || 0,
    billingAddress: billingAddress || fallback?.billingAddress || "",
    shippingAddress: shippingAddress || fallback?.shippingAddress || "",
    paymentTerm: normalizeSalesFormPaymentTerm(value.netTerm, fallback?.paymentTerm),
...
function buildMergedLineItemPatch(
  ...
  const shelfItems = existingShelf.length ? [...existingShelf] : [...incomingShelf];
...
function buildLineUnitPricePatch(line: NewSalesFormLineItem, unitPrice: number) {
...
function buildLineDescriptionPatch(line: NewSalesFormLineItem, description: string) {
...
function syncShelfRowsToLineQty(
...
function syncShelfRowsToLinePrice(
  ...
  return shelfItems.map((row, index) => {
    if (index > 0) return row;
    return patchShelfRowQty(patchShelfRowPrice(row as ShelfRowDraft, unitPrice), qty);
  });
}
