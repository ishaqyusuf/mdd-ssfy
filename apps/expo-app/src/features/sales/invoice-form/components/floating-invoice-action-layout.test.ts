...
describe("floating invoice action layout", () => {
  it("keeps centered invoice actions on predictable stacked lanes", () => {
    ...
    expect(INVOICE_FLOATING_SECONDARY_OFFSET - INVOICE_FLOATING_BASE_OFFSET).toBe(
      64,
    );
    expect(INVOICE_FLOATING_TERTIARY_OFFSET - INVOICE_FLOATING_SECONDARY_OFFSET).toBe(
      64,
    );
  });
