...
function OfficeVisibilityControl({ item }: { item: Item }) {
	const trpc = useTRPC();
	...
	const updateVisibility = useMutation(
		trpc.dealerPortal.updateCustomerOfficeVisibility.mutationOptions({
			onSuccess: async () => {
				...
				toast({
					title:
						item.officeVisibility === "SHARED"
							? "Customer is now private."
							: "Customer shared with the office.",
					variant: "success",
				});
			},
			onError: (error) => {
	...
	const shared = item.officeVisibility === "SHARED";

	return (
		<label className="flex items-center gap-2 whitespace-nowrap text-sm">
			<Switch
				aria-label={`${shared ? "Remove" : "Allow"} office access for ${customerName(item)}`}
				checked={shared}
...
export const columns: Column[] = [
	...
	{
		header: "Office access",
		accessorKey: "officeVisibility",
		cell: ({ row: { original: item } }) => (
			<OfficeVisibilityControl item={item} />
		),
	},
	{
		header: "Created",
