...
export function DealerCustomerSelectorDialog(
  ...
  const results = useMemo(() => {
    if (!normalizedQuery) return props.customers.slice(0, 5);
    return props.customers
      ...
      .slice(0, 10);
  }, [normalizedQuery, props.customers]);
    ...
    >
      ...
      >
        <DialogHeader className="border-b bg-gradient-to-r from-slate-50 to-white px-6 py-5">
          <DialogTitle>
            {showCreateForm
              ? "Create Quote: Add Customer"
              : "Create Quote: Select Customer"}
          </DialogTitle>
          <DialogDescription>
            {showCreateForm
              ? "Create a dealer customer and use it on this quote."
              : "Search by customer name, phone, or email. When empty, recent dealer customers appear here."}
          </DialogDescription>
        </DialogHeader>
        ...
        {showCreateForm ? (
          <div className="p-4">
            <CustomerFormClient
              formId="dealer-quote-customer-form"
              mode="modal"
