...
import {
	DealerDevQuickLogin,
	type DevQuickLoginDealer,
} from "./dev-quick-login";

type DealerLoginFormProps = {
	callbackURL?: string;
	devQuickLoginDealers?: DevQuickLoginDealer[];
};

export function DealerLoginForm({
	callbackURL = "/",
	devQuickLoginDealers = [],
}: DealerLoginFormProps) {
	...
	async function sendMagicLink() {
		...
		try {
			...
			setNotice(
				"If this dealer account is active, a login link is on its way.",
			);
		} catch {
	...
	return (
		<form className="space-y-5" onSubmit={onSubmit}>
			...
			{notice ? (
				<output className="flex items-start gap-3 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-sm font-medium text-emerald-700">
			...
			{devQuickLoginDealers.length > 0 ? (
				<DealerDevQuickLogin
					callbackURL={callbackURL}
					dealers={devQuickLoginDealers}
				/>
			) : null}
		</form>
	);
