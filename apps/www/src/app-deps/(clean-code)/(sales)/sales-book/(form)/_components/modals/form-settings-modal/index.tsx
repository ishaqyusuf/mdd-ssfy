			...
			<Context.Provider value={value}>
				<Modal.Content>
					<Modal.Header
						...
						subtitle={"Configure form step sequence for each item section"}
					/>
						...
						<Modal.Footer submitText="Save" onSubmit={value.save}>
							<DropdownMenu>
								...
								<DropdownMenuContent className="">
									{salesSetting.rootStep?.stepProducts?.map((stepProd) => (
										<DropdownMenuItem
											onClick={() => createSection(stepProd.uid)}
											disabled={arr.fields.some((s) => s.uid === stepProd.uid)}
											className="uppercase"
											key={stepProd.uid}
										>
						...
						{arr.fields.map((field, index) => (
							<SortableItem key={field.id} value={field.id} asChild>
								<div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2">
									...
									>
										<Icons.GripIcon className="size-4" aria-hidden="true" />
									</SortableDragHandle>
									<ConfirmBtn
