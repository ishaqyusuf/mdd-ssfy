...
function resolveRoutePrintConfig(input: {
	templateId?: string | null;
	pageBreakMode?: SalesPageBreakMode | null;
	printConfig?: Partial<SalesPrintSettings> | null;
}) {
...
export function buildSalesDocumentRouteFromQuery(input: {
	...
	if (printConfig.pageBreakMode !== DEFAULT_SALES_PAGE_BREAK_MODE) {
		url.searchParams.set("pageBreakMode", printConfig.pageBreakMode);
	}
	if (printConfig.templateId !== DEFAULT_SALES_PRINT_TEMPLATE_ID) {
		url.searchParams.set("templateId", printConfig.templateId);
	}
	if (!printConfig.showImages) {
		url.searchParams.set("showImages", "false");
	}
	if (!printConfig.headlineFirstPage) {
		url.searchParams.set("headlineFirstPage", "false");
	}
...
export async function resolveSalesPrintAccess(
	...
	const printConfig = getSalesPrintConfigOverrides(request);
...
export async function prepareSalesPdfPreview(
	request: SalesPrintRequest,
	dependencies: SalesPrintDependencies = defaultDependencies,
) {
	const access = await resolveSalesHtmlPreviewAccess(request, dependencies);
	return buildSalesPrintViewerUrl(access, {
		preview: true,
		printConfig: access.printConfig,
		mode: resolveSalesPrintMode(request.mode),
		pricingMode: request.pricingMode ?? null,
