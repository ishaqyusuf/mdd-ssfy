...
export function nav(
	...
	if (isAdmin) {
		routes.Dashboard.push(_route("Dashboard", Icons.dashboard, "/dashboard"));
	...
	if (role == "Production") {
		routes.Services.push(
			...
			_route("Unit Production", Icons.production, "/tasks/unit-productions"),
		);
	}
	if (!isAdmin) {
		if (viewInstallation) {
			...
			routes.Services.push(_route("Payments", Icons.payment, "/payments"));
		}
		if (__can.viewTech) {
			routes.Services.push(
	...
	if (viewCustomerService) {
		routes.Services.push(
			_route("Customer Service", Icons.customerService, "/customer-services"),
		);
	}
	...
	Object.entries(routes).map(([title, r]) => {
		...
		const __routes = r
			.filter(
				(route, index) => r.findIndex((fr) => fr.title == route.title) == index,
			)
