...
export function InboundReceivingPage({ initialSettings }: Props) {
	const trpc = useTRPC();
		...
		setReceiveInputs((current) => {
			...
			for (const item of selectedShipment.items) {
				...
				next[item.id] = next[item.id] || {
					...
					qtyGood: String(
						receivedGoodQty > 0 ? item.qtyGood : (item.qty ?? ""),
					),
													...
													return (
														...
														>
															...
															<div className="mt-1 flex flex-wrap gap-x-3 gap-y-1">
																...
																<span>
																	Good {Number(input?.qtyGood || item.qty || 0)}
																</span>
																<span>
																	Issue{" "}
																	{Number(
