...
export function InboundReceivingPage() {
    ...
    const uploadDocumentsMutation = useMutation(
        trpc.inventories.uploadInboundDocuments.mutationOptions({
            ...
            onError(error) {
                toast.error(
                    error.message || "Unable to upload inbound documents",
                );
            },
        }),
    );
                ...
                <div className="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="text-xs text-slate-500">
                            Snap receipt, extract invoice lines, then receive
                            into stock.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                                ...
                                <p className="mt-1 text-xs text-slate-500">
                                    {shipmentSummary.active} still active in the
                                    receiving queue.
                                </p>
                            </div>
                        ...
                        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                            <div className="border-b border-slate-100 bg-slate-50/80 px-4 py-3">
                                ...
                                <p className="text-xs text-slate-500">
                                    Pick an inbound to review documents and
                                    receive stock.
                                </p>
                            </div>
                            {shipments.length ? (
                                ...
                                <div className="p-6">
                                    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
                                        No inbound shipments yet. Create one
                                        from the receiving tray to start.
                                    </div>
                                </div>
                            )}
                        </div>
                                ...
                                <div className="space-y-4 rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 via-white to-slate-100/70 p-5">
                                    ...
                                    {selectedDemandRows.length ? (
                                        ...
                                        <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-3 text-sm text-slate-500">
                                            Select rows in the receiving tray to
                                            assign linked order demand to this
                                            inbound.
                                        </div>
                                    )}
                                </div>
            ...
            <div className="space-y-6 xl:sticky xl:top-6 xl:self-start">
                <Card className="p-4 space-y-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-500">
                            Open shortage demand that still needs inbound
                            coverage.
                        </p>
                    </div>
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    ...
                                    if (!selectedDemandIds.length) {
                                        toast.error(
                                            "Select demand rows to create inbound from",
                                        );
                                        return;
                                    }
                    ...
                    <div className="space-y-2">
                        {demandQueue.map((row) => {
                            ...
                            const orderNo =
                                row.lineItemComponent.parent.sale?.orderId;
                            return (
                                <label
                                    key={row.id}
                ...
                <Card className="p-4 space-y-4">
                    <div>
                        ...
                        <p className="text-xs text-slate-500">
                            Receipt uploads, extraction, assignment, and
                            receiving timeline.
                        </p>
                    </div>
