...
import {
	type TaskRunDiagnosticsRow,
	type TaskRunDiagnosticsTableMeta,
	columns,
	getTaskRunDiagnosticsRowId,
} from "./columns";
import { EmptyState, NoResults } from "./empty-states";
import { useTaskRunDiagnosticsTableStore } from "./store";
import { DataTableHeader } from "./table-header";

const NON_CLICKABLE_COLUMNS = new Set(["actions"]);
const TABLE_ID = "task-run-diagnostics";
...
type Props = {
	...
	filters: TaskRunDiagnosticsInput;
	hasFilters?: boolean;
...
export function DataTable({
	...
	const { setColumns, bindShowColumnDividers } =
		useTaskRunDiagnosticsTableStore();
	...
	const { data, isFetching } = useSuspenseQuery(
		trpc.taskRunDiagnostics.list.queryOptions(filters),
	);
	...
	const table = useReactTable({
		data: tableData,
		getRowId: getTaskRunDiagnosticsRowId,
		columns,
		...
		meta: {
			reviewingDiagnosticId,
			onMarkReviewed,
		} satisfies TaskRunDiagnosticsTableMeta,
	});
	...
	}, [data.page, data.total, isFetching, onPageInfoChange, pageCount]);
	...
	return (
		<div className="relative">
			<div className="w-full">
				...
				>
					<DndContext
						id="task-run-diagnostics-table-dnd"
						sensors={sensors}
