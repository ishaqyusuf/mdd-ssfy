...
const TABLE_ID = "bug-report-access-employees" satisfies TableId;
				...
				>
					...
					>
						{headerGroup.headers.map((header, headerIndex, headers) => {
							...
							const isLastVisible = !nextVisibleHeader;
...
function renderHeaderContent<TData>(
	...
	const content = (
		<span className={cn(HEADER_TEXT_CLASS, "truncate")} title={label}>
			{label}
		</span>
	);
