...
export function InboundManagementColumnVisibility() {
	const { columns, showColumnDividers, setShowColumnDividers } =
		useInboundManagementTableStore();
			...
			<PopoverContent className="w-[220px] p-0" align="end" sideOffset={8}>
				<div className="flex max-h-[450px] flex-col space-y-2 overflow-auto p-4">
					<div className="flex items-center space-x-2">
						<Checkbox
							id="inbound-management-column-dividers"
						...
						/>
						<label
							htmlFor="inbound-management-column-dividers"
						...
						.map((column) => {
							...
							return (
								<div key={column.id} className="flex items-center space-x-2">
									<Checkbox
										id={`inbound-management-${column.id}`}
									...
									/>
									<label
										htmlFor={`inbound-management-${column.id}`}
										className="text-sm peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
									>
										{label}
									</label>
