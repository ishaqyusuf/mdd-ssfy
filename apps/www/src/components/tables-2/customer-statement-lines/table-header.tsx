...
import type {
	CustomerStatementLineRow,
	CustomerStatementLinesTableMeta,
} from "./columns";
...
const TABLE_ID = "customer-statement-lines" satisfies TableId;
const tableConfig = TABLE_CONFIGS[TABLE_ID];

function getMeta(
	table: Table<unknown> | undefined,
): CustomerStatementLinesTableMeta | undefined {
	return (
		table as unknown as
			| {
						...
						{headerGroup.headers.map((header, headerIndex, headers) => {
							...
							if (!canReorder) {
								...
								return (
									...
									>
										{renderHeaderContent(
											header as Header<CustomerStatementLineRow, unknown>,
											columnId,
											table as unknown as Table<CustomerStatementLineRow>,
											tableScroll,
										)}
...
function renderHeaderContent(
	header: Header<CustomerStatementLineRow, unknown>,
	columnId: string,
	table?: Table<CustomerStatementLineRow>,
	tableScroll?: TableScrollState,
) {
	const label = getHeaderLabel(header.column.columnDef);
	const meta = getMeta(table as unknown as Table<unknown>);
	...
	if (columnId === "select") {
		return (
			<Checkbox
				checked={
					meta?.allLinesSelected
						? true
						: meta?.hasPartialSelection
							? "indeterminate"
							: false
				}
				onCheckedChange={(checked) => meta?.toggleAllLines(checked === true)}
