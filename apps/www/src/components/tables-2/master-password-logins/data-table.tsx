...
import {
	type MasterPasswordLoginRow,
	type MasterPasswordLoginsTableMeta,
	columns,
	getMasterPasswordLoginRowId,
} from "./columns";
import { EmptyState, NoResults } from "./empty-states";
import { useMasterPasswordLoginsTableStore } from "./store";
import { DataTableHeader } from "./table-header";

const NON_CLICKABLE_COLUMNS = new Set(["actions"]);
const TABLE_ID = "master-password-logins";
...
export function DataTable({
	...
	const { setColumns, bindShowColumnDividers } =
		useMasterPasswordLoginsTableStore();
	...
	const { data, isFetching } = useSuspenseQuery(
		trpc.masterPasswordLoginAudits.list.queryOptions(filters),
	);
	const tableData = data.rows ?? [];

	const table = useReactTable({
		data: tableData,
		getRowId: getMasterPasswordLoginRowId,
		columns,
		...
		meta: {
			clearingRowId,
			onClearRecord,
		} satisfies MasterPasswordLoginsTableMeta,
	});
	...
	}, [data.page, data.pageCount, data.total, isFetching, onPageInfoChange]);
	...
	return (
		<div className="relative">
			<div className="w-full">
				...
				>
					<DndContext
						id="master-password-logins-table-dnd"
						sensors={sensors}
