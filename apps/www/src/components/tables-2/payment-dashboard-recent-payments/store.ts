...
import type { PaymentDashboardRecentPaymentRow } from "./columns";

interface PaymentDashboardRecentPaymentsTableState {
	columns: Column<PaymentDashboardRecentPaymentRow, unknown>[];
	setColumns: (
		columns: Column<PaymentDashboardRecentPaymentRow, unknown>[],
	) => void;
...
export const usePaymentDashboardRecentPaymentsTableStore =
	create<PaymentDashboardRecentPaymentsTableState>((set) => ({
		columns: [],
