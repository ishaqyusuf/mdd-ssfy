...
function withCustomSelectionPrice(
	...
	const resolvedBasePrice = firstFiniteNumber(
		price,
		component.basePrice,
		component.salesPrice,
	);
	const resolvedSalesPrice = profileAdjustedSalesPrice(
		component.salesPrice,
		resolvedBasePrice,
		profileCoefficient,
	);
...
export function ItemWorkflowPanel() {
	...
	const shelfProductSearchQuery = useNewSalesFormShelfProductSearchQuery(
		{
	...
	const shelfProducts = useMemo(
		() => (shelfProductSearchQuery.data || []) as ShelfProductOption[],
	...
	function saveSelectedComponent({
		...
		component: WorkflowComponent;
		visibleComponentsOverride?: WorkflowComponent[];
	...
	function openCustomComponentDialog(
		...
		if (
			...
			customComponentDialog.lineUid === line.uid &&
			customComponentDialog.stepIndex === stepIndex
		) {
		...
		const selectedComponent =
			selectedComponents.find(
				(component) => String(component?.uid || "") === selectedUid,
			) || null;
		...
		const selectedIsCustom = Boolean(
			selectedOption ||
				...
				stepSnapshot?.custom === true ||
				selectedComponent?.custom === true ||
				selectedComponent?._metaData?.custom === true,
		);
		...
		const selectedPrice = firstFiniteNumber(
			...
			stepSnapshot?.salesPrice,
			step?.price,
		);
		const hydratedSelectedOption =
			selectedIsCustom && selectedOption
				? {
						...selectedOption,
						price: selectedPrice,
					}
				: selectedIsCustom && selectedUid
					? ({
							id: [
								"selected",
								selectedUid,
								selectedComponent?.id ?? stepSnapshot?.componentId ?? "none",
							].join(":"),
							label: selectedTitle || "CUSTOM",
							componentId:
								typeof (selectedComponent?.id ?? stepSnapshot?.componentId) ===
								"number"
									? Number(selectedComponent?.id ?? stepSnapshot?.componentId)
									: undefined,
							uid: selectedUid,
							title: selectedTitle || "CUSTOM",
							price: selectedPrice,
	...
	function mergeVisibleComponentForSelection(component: WorkflowComponent) {
		const uid = String(component?.uid || "");
		if (!uid) return [component, ...visibleComponents];
		const merged = visibleComponents.map((visibleComponent) =>
			String(visibleComponent?.uid || "") === uid
				? {
						...visibleComponent,
						...component,
					}
	...
	async function submitCustomComponentDialog() {
		...
		const componentForSelection = withCustomSelectionPrice(
			result.component as WorkflowComponent,
			customComponentDialog.price,
			activeProfileCoefficient,
		);
		saveSelectedComponent({
	...
	function renderCustomComponentInlinePanel(
		line: (typeof record.lineItems)[number],
		stepIndex: number,
	) {
		if (
										...
										process.env.NODE_ENV !== "production" ? (
											<div className="ml-auto flex items-center gap-1 rounded-md border bg-muted/30 p-1">
												<Button
													...
													variant={
														shelfUiVersion === "v1" ? "default" : "ghost"
													}
												...
												<Button
													...
													className="h-7 px-2 text-xs"
													variant={
														shelfUiVersion === "v2" ? "default" : "ghost"
													}
															...
															>
																<DialogContent className="max-w-sm">
																	<DialogHeader>
																		...
																		<DialogDescription>
																			Clearing categories will remove all
																			selected products in this section.
																		</DialogDescription>
																	</DialogHeader>
																	<DialogFooter>
																		<Button
																	...
																	(row, rowIndex: number) => (
																		...
																		>
																					...
																					<div className="min-w-[260px] space-y-3 p-2">
																						<div className="space-y-1">
																							...
																							<p className="text-xs text-muted-foreground">
																								Base price recalculates sales
																								price. Custom price overrides
																								the final line price.
																							</p>
																						</div>
																						<div className="space-y-2">
																			...
																			<Input
																				...
																				value={
																					money(getShelfRowDisplayTotal(row)) ||
																					"$0.00"
																				}
																				readOnly
																			/>
						...
						doorSizeVariantModal.stepIndex >= 0
							? activeLineSteps[doorSizeVariantModal.stepIndex]?.meta
								...
								routeData?.stepsByUid?.[
									activeLineSteps[doorSizeVariantModal.stepIndex]?.step?.uid ||
										""
								...
								meta: {
									...
									selectedComponents: selectedComponents.map((component) => {
										...
										const nextPricing = {
											...(component?.pricing || {}),
											[dependenciesUid]: {
												...(component?.pricing?.[dependenciesUid] || {}),
												id: id || component?.pricing?.[dependenciesUid]?.id,
												price,
											},
										};
										return {
											...component,
											basePrice: price == null ? component?.basePrice : price,
											salesPrice:
												price == null
													? component?.salesPrice
													: profileAdjustedDoorSalesPrice(
