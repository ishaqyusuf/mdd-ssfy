...
export function ComponentItemCard({
	...
	const { cls } = useMemo(() => {
		const cls = new ComponentHelperClass(stepUid, component?.uid, component);
		return {
			cls,
		};
	}, [component, stepUid]);
	...
	}, [selectState, cls, component, ctx, onSelect, swapDoor]);
	const multiSelect = cls.isMultiSelect();
	const isCustomComponent =
		component?._metaData?.custom === true ||
		(component as { custom?: boolean }).custom === true;
	const isSelected =
		(multiSelect && cls.multiSelected()) ||
		String(stepForm?.componentUid ?? "") === String(component.uid ?? "");
	...
	return (
		<div className="group relative flex min-h-[25vh] flex-col p-2 xl:min-h-[35vh]">
						...
						<div className="absolute hidden bottom-0 right-0">
							<div className="flex  gap-2 items-center text-xs font-semibold px-2">
								...
								{component.salesPrice && (
									<Badge className="h-5 px-1" variant="default">
										${component.salesPrice}
									</Badge>
								)}
							</div>
			...
			<div className="absolute  -left-3s flex flex-col gap-2 z-20">
				...
				>
					<div
						className={cn(!component?.variations?.length && "hidden", "px-1")}
					>
			...
			>
				...
				<div
					className={cn(!component?.sectionOverride?.overrideMode && "hidden")}
				>
					<Icons.LucideVariable className="size-4 text-muted-foreground/70" />
				</div>
				<div className={cn(!component.redirectUid && "hidden")}>
			...
			>
				<div>
					<Menu open={open} onOpenChanged={setOpen}>
						<Menu.Item
							...
							SubMenu={
								<>
									<Menu.Item
										onClick={() => {
											openComponentModal(ctx.cls, component);
										}}
										Icon={Icons.Info}
									>
										Details
									</Menu.Item>
									<Menu.Item onClick={editVisibility} Icon={Icons.Variable}>
										Visibility
									</Menu.Item>
									<Menu.Item
										disabled={cls.isDoor()}
										onClick={cls.isDoor() ? editDoorPrice : editPrice}
										Icon={Icons.dollar}
									>
										Price
									</Menu.Item>
						...
						<DeleteComponentMenuItem action={deleteStepItem} />
					</Menu>
				</div>
			</div>
			{outOfConfig ? (
	...
	return (
		<Menu.Item
			...
			SubMenu={
				<>
					{cls.redirectUid && (
						<Menu.Item onClick={() => cls.saveComponentRedirect(null)}>
							Cancel Redirect
						</Menu.Item>
					)}
