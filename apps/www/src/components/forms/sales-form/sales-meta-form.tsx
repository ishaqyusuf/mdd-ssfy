	...
	return (
		<div>
			{tab === "summary" ? (
				...
				<SalesHistory salesNo={md?.salesId} />
			)}
		</div>
	);
}
		...
		<div className="space-y-4 pb-4">
			...
			<section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
				...
				<div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-2">
					...
					<CompactField
						label="P.O No"
						icon={<Icons.ReceiptText className="size-4" />}
					>
						<LabelInput
						...
						<>
							...
							>
								<DatePicker
									...
									setValue={(e) => {
										zus.dotUpdate("metaData.paymentDueDate", e);
									}}
								/>
							</CompactField>
							<CompactField
							...
							>
								<DatePicker
									...
									setValue={(e) => {
										zus.dotUpdate("metaData.prodDueDate", e);
									}}
								/>
							</CompactField>
						</>
					...
					{md.extraCosts.map((k, i) =>
						k.type == "Labor" ? (
							...
							>
								<ConfirmBtn
									onClick={async (e) => {
										if (k.id) await deleteSalesExtraCost(k.id);

										zus.removeKey(`metaData.extraCosts.${i}`);
										calculateTotal();
									}}
									size="sm"
									trash
							...
							<div className="flex items-center gap-2">
								...
								<span className="text-xs text-slate-500">
									{md.paymentMethod != "Credit Card" || (
										<span>({md.pricing?.cccPercentage || 3.5}%)</span>
									)}
								</span>
							</div>
						}
	...
	return (
		<div
			className={cn(
				...
				label && "grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_auto]",
				className,
			)}
		>
	...
	return (
		...
		>
			...
			>
				...
				>
					<span>{itemText(options?.find((o) => itemValue(o) == value))}</span>
				</SelectValue>
			</SelectTrigger>
					...
					{options?.map((option, index) =>
						SelItem ? (
							...
							<SelectItem key={index} value={itemValue(option)}>
								{Item ? <Item option={option} /> : <>{itemText(option)}</>}
							</SelectItem>
						),
					)}
				</ScrollArea>
