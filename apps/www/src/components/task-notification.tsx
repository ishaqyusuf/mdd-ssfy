...
import {
	removeTaskMonitorTask,
	type TaskMonitorStatus,
	type TaskMonitorTask,
	updateTaskMonitorTask,
						...
						<div className="w-[min(360px,calc(100vw-2rem))] overflow-hidden rounded-lg border bg-background shadow-2xl">
							<div className="flex items-center justify-between gap-3 border-b px-3 py-2">
								<div className="flex items-center gap-2">
									<TaskStatusIcon status={hasFailures ? "FAILED" : "SYNCING"} />
									<div>
...
function TaskNotificationWatcher({ task }: { task: TaskMonitorTask }) {
	...
	useEffect(() => {
		if (error) {
			updateTaskMonitorTask(task.runId, {
				status: "FAILED",
		...
		if (run.status === "COMPLETED") {
			...
			window.setTimeout(() => removeTaskMonitorTask(task.runId), 2500);
			stop?.();
			return;
		}
	...
	}, [error, run?.status, stop, task.runId]);
		...
		<div className="flex items-start gap-3 rounded-md px-2 py-2 hover:bg-muted/60">
			...
			{task.status === "FAILED" ? (
				<Button
					...
					variant="ghost"
					size="icon-xs"
					onClick={() => removeTaskMonitorTask(task.runId)}
...
function TaskStatusIcon({ status }: { status: TaskMonitorStatus }) {
	...
	return <Icons.Loader2 className={cn(className, "animate-spin text-primary")} />;
}
...
function shortRunId(runId: string) {
	return runId.length > 10 ? `${runId.slice(0, 6)}...${runId.slice(-4)}` : runId;
}
