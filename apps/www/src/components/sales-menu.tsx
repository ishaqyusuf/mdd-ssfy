    ...
    const actions = useMemo<SalesMenuActions>(
        () => ({
            ...
            async copyAs(as) {
                ...
                const result = await copySalesUseCase(state.slug, as as any, state.type);
    ...
    return (
        <SalesMenuContext.Provider value={value}>
            <DropdownMenu.Root open={isOpen} onOpenChange={setOpen}>
                ...
                <DropdownMenu.Content align={align} className={contentClassName || "w-[185px]"}>
                    {children}
...
function useSalesPrintAction() {
    ...
    return async function runPrint(params?: SalesPrintProps, options?: PrintActionProps) {
        if (!state.salesIds.length) return;

        const sp = newSalesHelper();
        await sp.generateTokenSalesIds(state.salesIds, params?.mode || state.type || "order");

        if (options?.share) {
            await sp.share(`Hello! download your sales ${sp.shareUrl}`, "+234 8186877306");
            return;
        }
        ...
        <DropdownMenu.Sub>
            ...
            <DropdownMenu.SubContent>
                <DropdownMenu.Item
                    onSelect={(e) => {
                        e.preventDefault();
                        void runPrint({ mode: "order-packing", dispatchId: "all" }, { pdf: false });
                    }}
                >
                    <Printer className="mr-2 size-4 text-muted-foreground/70" />
                    Order & Packing
        ...
        <DropdownMenu.Sub>
            ...
            <DropdownMenu.SubContent>
                <DropdownMenu.Item
                    onSelect={(e) => {
                        e.preventDefault();
                        void runPrint({ mode: "order-packing", dispatchId: "all" }, { pdf: true });
                    }}
                >
                    <FileText className="mr-2 size-4 text-muted-foreground/70" />
                    Order & Packing
...
function SalesMenuSubTrigger(props: ComponentProps<typeof DropdownMenu.SubTrigger>) {
    return <DropdownMenu.SubTrigger {...props} />;
}

function SalesMenuSubContent(props: ComponentProps<typeof DropdownMenu.SubContent>) {
