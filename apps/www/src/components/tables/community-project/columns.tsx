...
import { updateProjectArchivedAction, updateProjectSupervisorAction } from "@/actions/community/project-actions";
...
function getProductionMeta(item: ItemType) {
  ...
  const completed = item.homeTasks?.filter((task) => task.producedAt).length || 0;
  const started =
    item.homeTasks?.filter((task) => !task.producedAt && task.prodStartedAt).length || 0;
  const queued =
    item.homeTasks?.filter(
      (task) => !task.producedAt && !task.prodStartedAt && task.sentToProductionAt,
    ).length || 0;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="h-8 justify-start px-3">
          {item.meta?.supervisor?.name ? (
            <span className="truncate text-left">
              {item.meta.supervisor.name}
              {item.meta.supervisor.email ? ` • ${item.meta.supervisor.email}` : ""}
            </span>
          ) : (
            <>
      ...
      <PopoverContent align="start" className="w-72 space-y-3 p-4">
        <div className="space-y-1">
          <h4 className="text-sm font-semibold text-slate-900">Update supervisor</h4>
          <p className="text-xs text-slate-500">
            Save a direct owner for this project so operations can route follow-ups quickly.
          </p>
        </div>
    ...
    <div className="relative z-10 flex justify-end">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" onClick={(event) => event.stopPropagation()}>
          ...
          <DropdownMenuItem onClick={() => modal.openModal(<ProjectModal data={item as any} />)}>
...
const project: Column = {
  ...
  cell: ({ row: { original: item } }) => (
    <>
      ...
      <Item.Description>{item.builder?.name || "No builder"}</Item.Description>
    </>
  ),
};
      ...
      return (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500">{item.refNo || "No ref"}</span>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <Button asChild variant="outline">
              <Link href={`/community/project-units?projectSlug=${item.slug}`}>Open units</Link>
            </Button>
            <Button asChild>
              <Link href={`/community/projects/${item.slug}`}>Open overview</Link>
            </Button>
          </div>
        </div>
      );
