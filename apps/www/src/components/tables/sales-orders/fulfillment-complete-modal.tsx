...
interface FulfillmentCompleteModalProps {
	...
	defaultDeliveryMode: DeliveryOption;
	isLoading?: boolean;
...
export function FulfillmentCompleteModal({
	...
	const [deliveryMode, setDeliveryMode] =
		React.useState<DeliveryOption>(defaultDeliveryMode);
	...
	React.useEffect(() => {
		...
		setDeliveryMode(
			(defaultDispatch?.deliveryMode as DeliveryOption | null) ||
				defaultDeliveryMode,
		);
		setRecipient(defaultRecipient || "");
		setCompletedDateValue(toDateInputValue(new Date()));
		setDriverId(defaultDispatch?.driverId || null);
	}, [open, selectableDispatches, defaultRecipient, defaultDeliveryMode]);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						Choose an existing dispatch or create a new one, then complete
						fulfillment.
					</DialogDescription>
				</DialogHeader>
								...
								return (
									...
									>
										<button
											...
											disabled={!selectable}
											onClick={() =>
												selectable && setSelectedDispatchKey(dispatch.id)
											}
										>
											<div className="font-medium">Dispatch #{dispatch.id}</div>
										...
										{hasMultipleDispatches && !isCompleted(dispatch.status) && (
											<Button
												...
												size="sm"
												disabled={
													isSubmitting || deletingDispatchId === dispatch.id
												}
					...
					<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
						<div className="space-y-1">
							...
							<select
								...
								value={deliveryMode}
								onChange={(event) =>
									setDeliveryMode(event.target.value as DeliveryOption)
								}
								disabled={isSubmitting}
							>
				...
				<DialogFooter className="flex justify-end gap-2">
					...
					<Button
						...
						onClick={() =>
							onConfirm({
								selectedDispatchId:
									selectedDispatchKey === "new" ? null : selectedDispatchKey,
