		...
		trpc.dealer.createAccount.mutationOptions({
			onSuccess: async () => {
				...
				toast({
					...
					description: "The dealer account was created and queued for onboarding.",
					variant: "success",
				});
			},
			onError: (error) => {
		...
		if (mode === "existing") {
			if (!selectedCustomer?.id || !selectedCustomer.email) {
				toast({
					...
					description: "Select a customer with an email before onboarding them.",
					variant: "destructive",
				});
				return;
			}
							...
							dealers.map((dealer) => {
								...
								return (
									<TableRow key={dealer.id}>
										<TableCell className="font-medium">{dealerName}</TableCell>
