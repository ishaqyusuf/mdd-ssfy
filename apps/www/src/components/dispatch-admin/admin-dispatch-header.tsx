						...
						>
							...
							<div className="space-y-2">
								{group.dispatches.map((dispatch) => {
									const isKeep = selection?.keepDispatchId === dispatch.id;
									const shouldDelete =
										selection?.deleteDispatchIds?.includes(dispatch.id) ||
										false;
									return (
										<div
											key={dispatch.id}
										...
										>
											<div className="space-y-1">
												<p className="text-sm font-medium">
													Dispatch #{dispatch.id} • {dispatch.status || "queue"}
												</p>
												<p className="text-xs text-muted-foreground">
													Driver: {dispatch.driverName || "Unassigned"} • Items:{" "}
													{dispatch.itemCount} • Packed:{" "}
											...
											<div className="flex items-center gap-3">
												<Btn
													...
													onClick={() =>
														updateSelection(group.salesId, {
															keepDispatchId: dispatch.id,
															deleteDispatchIds: (
																selection?.deleteDispatchIds || []
															).filter((id) => id !== dispatch.id),
														})
													}
												>
													Keep
												...
												<div className="flex items-center gap-2 text-sm">
													<Checkbox
														...
														onCheckedChange={(value) => {
															...
															if (value) ids.add(dispatch.id);
															else ids.delete(dispatch.id);
															updateSelection(group.salesId, {
																deleteDispatchIds: [...ids],
															});
														}}
													/>
													<span>Delete</span>
...
export function AdminDispatchHeader() {
	...
	return (
		<div className="flex flex-col gap-3">
			...
			{isSuperAdmin && (
				<>
					<SweeperDialog open={sweeperOpen} onOpenChange={setSweeperOpen} />
