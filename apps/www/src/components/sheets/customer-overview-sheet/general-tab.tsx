...
type PendingPaymentOrder =
	OverviewData["general"]["pendingPaymentOrders"][number];
type StatementLine = {
	salesId: number;
	orderNo: string;
	date: string;
...
export function GeneralTab({ setCustomerName }: Props) {
	...
	const openStatementReport = () => {
		...
		const params = new URLSearchParams({
			...
			statementCustomerId: String(data.customer.id),
			statementStatus: "overview",
		});
		router.push(`/sales-book/customers?${params.toString()}`);
	};
	...
	return (
		<div className="space-y-4">
			...
			<Button
				...
				disabled={!canGenerateStatement || !data?.customer.id}
				onClick={openStatementReport}
				variant="outline"
			>
				...
				<CardContent className="space-y-4">
					<div className="grid gap-3 md:grid-cols-4">
						<AnalyticsCard
							...
							description={formatCurrency.format(
								data?.general.pendingPayment || 0,
							)}
						/>
						<AnalyticsCard
						...
						<AnalyticsCard
							...
							description={formatCurrency.format(
								data?.general.totalSalesValue || 0,
							)}
						/>
						<AnalyticsCard
							title="Quotes"
							...
							description={formatCurrency.format(
								data?.general.totalQuotesValue || 0,
							)}
						/>
					</div>
			...
			<Card>
				...
				<CardContent className="grid gap-2 md:grid-cols-4">
					<SendSalesReminder
						salesIds={(data?.general.pendingPaymentOrders || []).map(
							(sale) => sale.id,
						)}
					>
						<Button
			...
			{items.slice(0, 6).map((item) => (
				...
				>
					...
					<span className="text-right">
						{formatCurrency.format(item.invoice.total || 0)}
					</span>
