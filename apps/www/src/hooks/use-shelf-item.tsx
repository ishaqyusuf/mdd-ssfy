...
export function useShelfItemContext({ shelfUid }) {
	...
	const categoryIds = useMemo(
		() => (Array.isArray(shelf.categoryIds) ? shelf.categoryIds : []),
		[shelf.categoryIds],
	);
	const options = useMemo(() => {
		if (categoryIds.length == 0)
			return categories?.filter((a) => a.type == "parent");
		const _catId = Number([...categoryIds].pop());
	...
	const products = useAsyncMemo(async () => {
		...
		return products;
	}, [categoryIds, categories, prodRefreshToken]);
	const baseLinePath =
	...
	return {
		...
		...shelf,
		setCategoryIds(ids) {
			cls.dotUpdateItemForm(`shelfItems.lines.${shelfUid}.categoryIds`, ids);
		},
