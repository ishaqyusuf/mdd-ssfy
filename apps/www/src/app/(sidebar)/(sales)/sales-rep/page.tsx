...
import { DataTable as RecentSalesDataTable } from "@/components/tables-2/sales-orders/data-table";
...
type RecentSalesInput = RouterInputs["sales"]["getOrders"];
...
export default async function SalesRepProfile(props: {
	...
	const recentSalesFilters = {
		size: 5,
		showing: null,
	} satisfies RecentSalesInput;
	...
	batchPrefetch([
		...(defaultTab === "recent-sales"
			? [
					trpc.sales.getOrders.infiniteQueryOptions(recentSalesFilters, {
							...
							<Tabs value={defaultTab} className="space-y-4">
								...
								<TabsContent value="recent-sales" className="space-y-4">
									<Suspense
										fallback={
											<SalesOrdersSkeleton
												initialSettings={initialSalesSettings}
												rowCount={5}
											/>
										}
									>
										<RecentSalesDataTable
											defaultFilters={recentSalesFilters}
											embedded
											initialSettings={initialSalesSettings}
											singlePage
										/>
