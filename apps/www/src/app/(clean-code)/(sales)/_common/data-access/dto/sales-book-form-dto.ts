...
function shelfCategoryIdsFromLegacyLine(line: any) {
	const meta = (line?.meta || {}) as any;
	if (Array.isArray(meta.categoryIds)) {
		return meta.categoryIds.map((id) => Number(id)).filter((id) => id > 0);
	}
	...
	return [meta.shelfParentCategoryId, line?.categoryId]
		.map((id) => Number(id || 0))
	...
	return data.order.items.map((item) => {
		...
		return {
			...
			formSteps: item.formSteps
				.map((item) => ({
					...
					component: item.component
						? {
								...
								meta: (item.component.meta || {}) as any as StepComponentMeta,
							}
						: null,
					step: {
						...item.step,
				...
				.filter(
					(f, fi) =>
						item.formSteps.findIndex((p) => p.stepId == f.stepId) == fi,
				),
...
export function transformSalesBookFormItem(
	...
	const itemArray = (items || [null])
		...
		.map((item, itemIndex) => {
			...
			const _ = {
				...
				formStepArray: formSteps.map(({ step, component, ...rest }) => ({
					component,
	...
	return itemArray?.every((item) => item?.item?.meta?.lineIndex > -1)
		? itemArray.sort(
				(item, item2) => item.item.meta.lineIndex - item2.item.meta.lineIndex,
			)
		: itemArray;
}
...
export function transformShelfItem(item: SalesFormItems[number]) {
	...
	const sorted = shelfItems.sort(
		(a, b) => Number(a?.meta?.itemIndex || 0) - Number(b?.meta?.itemIndex || 0),
	);

	sorted.map((line) => {
		const lineMeta = (line?.meta || {}) as any;
		const lineUid =
			lineMeta.lineUid ||
			lineMeta.sectionUid ||
			`shelf-line-${line.id || generateRandomString()}`;
		const siblings = sorted.filter((a) => {
			const siblingMeta = (a?.meta || {}) as any;
			return (
				(siblingMeta.lineUid ||
					siblingMeta.sectionUid ||
					`shelf-line-${a.id}`) == lineUid
			);
		});
		if (line?.id == siblings?.[0]?.id) {
			shelfItemsData.lineUids.push(lineUid);
			shelfItemsData.lines[lineUid] = {
				categoryIds: shelfCategoryIdsFromLegacyLine(line),
				productUids: [],
				products: {},
			};
			const lineData = shelfItemsData.lines[lineUid];
...
export function transformMultiDykeItem(
	...
	let safeTitles: any = [];
	multiComponent.primary = ((multiComponent.uid && multiComponent.multiDyke) ||
	...
	_comps.map((item) => {
		const component = item.housePackageTool?.doors?.length
			...
			: item.housePackageTool?.door || item.housePackageTool?.stepProduct?.uid
				? {
		...
		if (component) {
			...
			const cData = {
				...
				mouldingPriceData: item?.housePackageTool?.priceData || ({} as any),
