...
function harness() {
	...
	const command = async <T>(parts: (string | number)[]) => {
		...
		if (parts[0] === "SET") { values.set(key, String(parts[2])); expect(parts.slice(3)).toEqual(["EX", 600, "NX"]); return "OK" as T; }
...
test("a retry executes only its original read once, including concurrent redemption", async () => {
	...
	const results = await Promise.allSettled([0, 1].map(() => executeAssistantReadRetry(actor, id!, { signal: new AbortController().signal, authorize: async ticket => { expect(ticket).toMatchObject({ conversationId: "chat", runId: "run", toolCallId: "call" }); return true; } }, h.deps)));
	expect(results.filter(result => result.status === "fulfilled")).toHaveLength(1);
	expect(h.calls).toEqual([{ toolId: "sales_get_order_status", version: 1, input: { orderNo: "QA-42" } }]);
});

test("scope changes, permission loss, stale history, expiry and cancellation stop execution", async () => {
	for (const scenario of ["scope", "permission", "history", "expired", "cancelled"] as const) {
		const h = harness();
		const id = await createAssistantReadRetry(actor, request, h.deps);
		...
		await expect(executeAssistantReadRetry(current, id!, { signal: controller.signal, authorize: async () => scenario !== "history" }, h.deps)).rejects.toBeInstanceOf(Error);
