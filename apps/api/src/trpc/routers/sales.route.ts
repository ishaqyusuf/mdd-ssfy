...
import { applyGuardedPackingPolicyTransition } from "@api/utils/guarded-packing-policy-transition";
...
export const salesRouter = createTRPCRouter({
	...
	getGuardedPackingSettings: protectedProcedure.query(async (props) => {
		await requireSuperAdmin(props.ctx);
		return {
			settings: await getGuardedPackingSettings(props.ctx.db),
		};
	}),
	updateGuardedPackingSettings: protectedProcedure
		.input(guardedPackingPolicyInputSchema)
		.mutation(async (props) => {
			await requireSuperAdmin(props.ctx);
			const result = await updateGuardedPackingSettings(
				props.ctx.db,
				props.input,
				{
					afterPersist: (tx, change) =>
						applyGuardedPackingPolicyTransition(
							{
								db: tx as typeof props.ctx.db,
								actorUserId: props.ctx.userId,
							},
							{
								previous: change.previousPolicy,
								next: change.nextPolicy,
							},
							{
								reconcile: (_db, dispatchIds) =>
									reconcilePendingGuardedPackingDispatchesInTransaction(
										tx,
										dispatchIds,
									),
							},
