...
const appRouterSource = readFileSync(`${import.meta.dir}/_app.ts`, "utf8");
...
const adminGuardSource = readFileSync(
	...
	.split("export async function requireSuperAdmin(ctx: TRPCContext)")[1]
	?.split("export async function getEmployeePermissionOptions")[0];
...
describe("mobile access permission boundaries", () => {
	...
	it("uses optimistic status updates and append-only events", () => {
		expect(querySource).toContain("status: current.status,");
	...
	it("requires a live requester when advancing access but permits closing old requests", () => {
		...
		expect(querySource).toContain("if (changed.count !== 1)");
	});

	it("checks live account and role authority for admin review", () => {
		expect(adminGuardSource).toContain(
			"getActiveCompanyMemberWhere({ id: ctx.userId })",
		);
		expect(adminGuardSource).toContain(
		...
		const adminList = querySource
			.split("export async function getMobileAccessRequestsForAdmin")[1]
			?.split("export async function updateMobileAccessRequest")[0];
		const adminUpdate = querySource.split(
