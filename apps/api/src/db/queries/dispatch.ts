...
function formatPackingAddress(address?: {
...
async function emitPackingListActivity(
	ctx: TRPCContext,
	payload: {
		salesId: number;
		orderNo: string;
		dispatchId: number;
		status: "queue" | "completed" | "cancelled";
	},
) {
...
async function getPackingListDispatchIds(db: TRPCContext["db"]) {
	...
	for (const note of notes) {
		const dispatchId = Number(
			note.tags.find((tag) => tag.tagName === "dispatchId")?.tagValue || 0,
		);
		if (
			!Number.isFinite(dispatchId) ||
			dispatchId <= 0 ||
			seen.has(dispatchId)
		) {
			continue;
...
function formatPackingListCustomerName(order?: {
	customer?: {
		name?: string | null;
...
function mapPackingListRow(row: {
	id: number;
	salesOrderId: number;
	status: string | null;
	createdAt: Date | null;
...
export async function sendSaleForPickup(
	ctx: TRPCContext,
	data: SendSaleForPickupSchema,
) {
	if (!ctx.userId) {
		throw new TRPCError({
			code: "UNAUTHORIZED",
	...
	const reusableDelivery = sale.deliveries.find(
		(delivery) => delivery.status !== "completed",
	);

	if (reusableDelivery) {
		const dispatch = await ctx.db.orderDelivery.update({
			where: {
				id: reusableDelivery.id,
			},
			data: {
				deliveryMode: "pickup",
		...
		await emitPackingListActivity(ctx, {
			salesId: sale.id,
	...
	const created = await ctx.db.orderDelivery.create({
		data: {
			deliveryMode: "pickup",
			status: "queue" as SalesDispatchStatus,
			meta: {},
...
export async function getPackingQueue(ctx: TRPCContext) {
...
export async function getPackingList(
	...
	if (input.tab === "cancelled" && !(await userHasPackingAdminAccess(ctx))) {
		throw new TRPCError({
			code: "FORBIDDEN",
...
export async function signPackingSlip(
	...
	const [dispatch, author] = await Promise.all([
	...
	const packedBy = author?.name || "System";
	const receivedBy =
		input.receivedBy?.trim() ||
		dispatch.order?.shippingAddress?.name ||
	...
	return {
		...
		dispatchId: dispatch.id,
		salesId: dispatch.salesOrderId,
		orderNo: dispatch.order?.orderId ?? null,
		packedBy,
		receivedBy,
	};
}
...
export async function getDispatchSummary(ctx: TRPCContext) {
	...
	return {
		...
		byStatus: {
			...
			packingQueue: byStatus["packing queue"] ?? 0,
