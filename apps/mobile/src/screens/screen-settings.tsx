...
export default function SettingsExampleScreen({
	...
	const { data: profile } = useQuery(useTRPC().user.getProfile.queryOptions());
				...
				<View className="p-4 gap-6">
					...
					{!auth.isAdmin || (
						<Section title="Job Configuration">
							...
							<SettingsItem
								...
								subLabel="Allow the Custom Project option in job submission"
								isLast
								rightElement={
									<Toggle
					...
					<Section title="App">
						...
						<SettingsItem
							icon="FileText"
							label="Documents"
							subLabel="Review insurance and workers comp documents"
							isLast={false}
							onPress={() => router.push("/documents")}
							rightElement={
								<Icon
