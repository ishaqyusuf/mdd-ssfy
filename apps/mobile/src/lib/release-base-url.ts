...
export function resolveConfiguredReleaseBaseUrl(input: {
	appVariant?: unknown;
	envVariant?: string;
	baseUrl?: string;
	isDev?: boolean;
	...
	if (!RELEASE_VARIANTS.has(variant) || (input.isDev && variant === "production")) return null;
	const baseUrl = input.baseUrl?.trim();
	if (!baseUrl) {
		throw new Error("Preview and production mobile builds require EXPO_PUBLIC_BASE_URL.");
	}
	if (variant === "production" && !isPublicHttpsOrigin(baseUrl)) {
		throw new Error("Production mobile builds require a public HTTPS EXPO_PUBLIC_BASE_URL origin.");
	}
