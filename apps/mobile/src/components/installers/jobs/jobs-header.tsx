...
export function JobsHeader() {
  ...
  return (
    <View style={{ paddingTop: insets.top }} className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
      <View className="flex-row items-center justify-between px-4 py-3">
        <TouchableOpacity onPress={() => navigation.goBack()} className="p-2 rounded-full active:bg-gray-200 dark:active:bg-gray-700">
          <Ionicons name="arrow-back" size={24} color={colorScheme === 'dark' ? '#F9FAFB' : '#1F2937'} />
        </TouchableOpacity>
        <Text className="text-xl font-bold text-gray-800 dark:text-gray-100 flex-1 text-center">All Jobs</Text>
        <TouchableOpacity className="p-2 rounded-full active:bg-gray-200 dark:active:bg-gray-700">
          <MaterialIcons name="filter-list" size={24} color={colorScheme === 'dark' ? '#F9FAFB' : '#1F2937'} />
        </TouchableOpacity>
      </View>
