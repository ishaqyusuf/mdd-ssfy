...
describe("buildPackingPayload", () => {
	...
	it("does not allocate handled quantity across the wrong hand", () => {
		const result = buildPackingPayload({
			salesItemId: 13,
			enteredQty: { lh: 2, rh: 3 },
			deliverables: [{ submissionId: 4001, qty: { lh: 1, rh: 5 } }],
		});
		...
		expect(result.remainder).toEqual({ qty: 0, lh: 1, rh: 0 });
	});
});
