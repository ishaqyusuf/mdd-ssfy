...
export function SalesFinanceReceivablesColumnVisibility() {
		...
		<Popover>
			<PopoverTrigger asChild>
				<Button
					variant="outline"
					size="icon"
					aria-label="Configure receivables columns"
				>
					<Icons.Tune size={18} />
				</Button>
			</PopoverTrigger>
			<PopoverContent className="w-[240px] p-0" align="end" sideOffset={8}>
				<div className="flex max-h-[450px] flex-col gap-2 overflow-auto p-4">
					<div className="flex items-center gap-2 text-sm">
						<Checkbox
							id="sales-finance-receivables-column-dividers"
							checked={showColumnDividers}
							onCheckedChange={(checked) =>
								setShowColumnDividers(checked === true)
							}
						/>
						<label htmlFor="sales-finance-receivables-column-dividers">
							Column dividers
						</label>
					</div>
					...
					{columns
						...
						.map((column) => {
							...
							return (
								...
								>
									<Checkbox
										id={`sales-finance-receivables-${column.id}`}
										checked={column.getIsVisible()}
										onCheckedChange={(checked) =>
											column.toggleVisibility(checked === true)
										}
									/>
									<label htmlFor={`sales-finance-receivables-${column.id}`}>
										{meta?.headerLabel ||
