...
import type { TaskEventRow } from "./columns";

interface TaskEventsTableState {
	columns: Column<TaskEventRow, unknown>[];
	setColumns: (columns: Column<TaskEventRow, unknown>[]) => void;
...
export const useTaskEventsTableStore = create<TaskEventsTableState>((set) => ({
	columns: [],
