...
export function SalesAccountingColumnVisibility() {
	const { columns, showColumnDividers, setShowColumnDividers } =
		useSalesAccountingTableStore();
			...
			<PopoverContent className="w-[220px] p-0" align="end" sideOffset={8}>
				<div className="flex max-h-[450px] flex-col space-y-2 overflow-auto p-4">
					<div className="flex items-center space-x-2">
						<Checkbox
							id="sales-accounting-column-dividers"
						...
						/>
						<label
							htmlFor="sales-accounting-column-dividers"
						...
						.map((column) => {
							...
							return (
								<div key={column.id} className="flex items-center space-x-2">
									...
									/>
									<label
										htmlFor={`sales-accounting-${column.id}`}
										className="text-sm peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
									>
										{label}
									</label>
