...
export function InventoryBackordersColumnVisibility() {
		...
		<Popover>
			<PopoverTrigger asChild>
				<Button
					variant="outline"
					size="icon"
					aria-label="Inventory backorder columns"
				>
					<Icons.Tune size={18} />
				</Button>
			</PopoverTrigger>
			...
			<PopoverContent className="w-[240px] p-0" align="end" sideOffset={8}>
				<div className="flex max-h-[450px] flex-col space-y-2 overflow-auto p-4">
					<div className="flex items-center space-x-2">
						<Checkbox
							id="inventory-backorders-column-dividers"
						...
						/>
						<label
							htmlFor="inventory-backorders-column-dividers"
						...
						.map((column) => {
							...
							return (
								<div key={column.id} className="flex items-center space-x-2">
									<Checkbox
										id={`inventory-backorders-${column.id}`}
									...
									/>
									<label
										htmlFor={`inventory-backorders-${column.id}`}
										className="text-sm peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
									>
										{label}
									</label>
