...
export function CommunityBuildersColumnVisibility() {
	const { columns, showColumnDividers, setShowColumnDividers } =
		useCommunityBuildersTableStore();
			...
			<PopoverContent className="w-[220px] p-0" align="end" sideOffset={8}>
				<div className="flex max-h-[450px] flex-col space-y-2 overflow-auto p-4">
					<div className="flex items-center space-x-2">
						<Checkbox
							id="community-builders-column-dividers"
						...
						/>
						<label
							htmlFor="community-builders-column-dividers"
						...
						.map((column) => {
							...
							return (
								<div key={column.id} className="flex items-center space-x-2">
									<Checkbox
										id={`community-builders-${column.id}`}
									...
									/>
									<label
										htmlFor={`community-builders-${column.id}`}
										className="text-sm peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
									>
										{label}
									</label>
