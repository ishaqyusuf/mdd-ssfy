...
export function ContractorAccountingColumnVisibility() {
			...
			<PopoverContent className="w-[240px] p-0" align="end" sideOffset={8}>
				<div className="flex max-h-[450px] flex-col gap-2 overflow-auto p-4">
					<div className="flex items-center gap-2 text-sm">
						<Checkbox
							id="contractor-accounting-column-dividers"
							checked={showColumnDividers}
							onCheckedChange={(checked) =>
								setShowColumnDividers(checked === true)
							}
						/>
						<label htmlFor="contractor-accounting-column-dividers">
							Column dividers
						</label>
					</div>
					...
					{columns
						...
						.map((column) => {
							...
							return (
								...
								>
									<Checkbox
										id={`contractor-accounting-${column.id}`}
										checked={column.getIsVisible()}
										onCheckedChange={(checked) =>
											column.toggleVisibility(checked === true)
										}
									/>
									<label htmlFor={`contractor-accounting-${column.id}`}>
										{meta?.headerLabel ||
