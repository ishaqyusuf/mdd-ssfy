	...
	return new Promise<DriverProofAttachment>((resolve, reject) => {
		...
		reader.onload = () => {
			...
			if (!base64 || base64.length > 8_000_000) {
				reject(
					new Error(`${file.name} is too large. Choose an image under 5 MB.`),
				);
				return;
			}
			resolve({
						...
						<input
							...
							onChange={async (event) => {
								...
								try {
									...
									setValue("attachments", [...attachments, ...next], {
										shouldDirty: true,
									});
								} catch (error) {
									toast.error(
	...
	const submit = form.handleSubmit(async (values) => {
		...
		try {
			...
			await actions.completeWithProof.mutateAsync({
				...
				receivedDate: new Date(`${values.receivedDate}T12:00:00`),
		...
		<form onSubmit={submit} className="space-y-5 p-4 sm:p-5">
			...
			{!online || draft.storageError ? (
				<Alert variant="destructive">
					<CloudOff />
					<AlertTitle>
						{!online ? "You are offline" : "Draft storage needs attention"}
					</AlertTitle>
					<AlertDescription>
						{draft.storageError ||
			...
			<div className="space-y-2">
				...
				<Input
					...
					{...form.register("receivedBy", {
						validate: (value) =>
							Boolean(value.trim()) || "Recipient name is required.",
					})}
					aria-invalid={Boolean(form.formState.errors.receivedBy)}
				/>
			...
			<div className="space-y-2">
				<Label htmlFor="driver-received-date">Actual delivery date</Label>
				<Input
					id="driver-received-date"
					type="date"
					max={localDateValue()}
					{...form.register("receivedDate", {
						required: "Actual delivery date is required.",
					})}
					aria-invalid={Boolean(form.formState.errors.receivedDate)}
				/>
