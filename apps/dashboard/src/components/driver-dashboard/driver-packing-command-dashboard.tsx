...
import { formatDriverCustomerName, getDriverPrimaryAction } from "./model";
			...
			<header className="flex items-start justify-between gap-4 border-b px-4 py-4 sm:px-5">
				<div>
					...
					<p className="mt-1 text-xs text-muted-foreground">
						Pack exact quantities. Pending review never counts as packed.
					</p>
				</div>
				{canEditPacking && showPackAction ? (
					<Button variant="outline" onClick={onPack} className="hidden sm:flex">
						Pack items
					</Button>
				) : null}
			</header>
...
export function DriverPackingCommandDashboard({
	...
	const pickup = dispatch?.deliveryMode === "pickup";
	const customerName = formatDriverCustomerName(
		order.customer?.businessName || order.customer?.name,
	);
	const address = formatAddress(detail.address);
	const routeAddress =
		(pickup ? detail.routing.origin : "") ||
		detail.destination.route.formattedAddress ||
		detail.destination.route.address1 ||
		address;
	const phone = String(
	...
	const destinationPreflightCandidate =
		stage === "packed" &&
		lifecycle.blockers.startTrip.length > 0 &&
		lifecycle.blockers.startTrip.every(
			(blocker) => blocker === "DESTINATION_REQUIRED",
		);
	const guardedPhysicalVerification =
		lifecycle.pendingPackingReportCount > 0 &&
		!lifecycle.blockers.startTrip.includes("PACKING_REVIEW_PENDING") &&
	...
	const primaryAction = getDriverPrimaryAction({
		...
		canStartTrip: capabilities.canStartTrip || destinationPreflightCandidate,
	...
	const startTrip = () => {
		if (detail.destination.requiresNormalization) {
			setDestinationOpen(true);
			return;
		}
		void startTripNow();
	};
	const destinationReviewStops: DestinationReviewStop[] = dispatch
		? [
				{
					dispatchId: dispatch.id,
					orderNo: order.orderId || String(dispatch.id),
		...
		<div className="relative min-h-0 flex-1 overflow-y-auto bg-emerald-950/[0.025]">
			<div className="mx-auto w-full max-w-[1500px] px-4 pb-28 pt-5 sm:px-6 sm:pb-8 lg:px-8">
				<section className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
					<div className="min-w-0">
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Order {order.orderId} · {formatDeliveryTime(dispatch?.dueDate)}
						</p>
					</div>
					<div className="hidden shrink-0 items-center gap-2 sm:flex">
						...
						{primaryAction.kind === "proof" ? (
							<Button
								onClick={() =>
									void setParams({ mode: "proof" }, DRIVER_STOP_URL_OPTIONS)
								}
							>
								<CheckCircle2 className="mr-2 size-4" /> Complete with proof
							</Button>
							...
							>
								...
								{actions.startTrip.isPending ? "Starting trip…" : "Start trip"}
							</Button>
							...
							<Button
								...
								onClick={capabilities.canReportException ? openHelp : undefined}
							>
				...
				{complete ? (
					<section className="mt-5 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950 shadow-sm dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-100">
						...
						<div>
							<h2 className="font-semibold">Delivery completed</h2>
							<p className="mt-1 text-sm text-emerald-800 dark:text-emerald-200">
								Recipient acknowledgement and proof of delivery were saved.
							</p>
						</div>
					</section>
				) : null}
				...
				>
					...
					<CommandMetric
						...
						note={complete ? "Delivery closed" : `${pending} waiting`}
					/>
					<CommandMetric
						label="Inventory"
						value={
						...
						tone={
							inventoryBlocked && !guardedPhysicalVerification
								? "warning"
								: "success"
						}
					/>
					<CommandMetric
						...
						tone={complete || inProgress || canDispatch ? "success" : "warning"}
					/>
					<CommandMetric
						label="Load status"
						value={`${packed} / ${total}`}
				...
				<div className="mt-4 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_22.5rem]">
					...
					<aside className="grid content-start gap-4">
						<GoogleRouteMap
							title={
								pickup ? "Directions to pickup" : "Directions to this stop"
							}
							description={
								pickup
									? "Navigate from your current location to the warehouse."
									: "Start from the warehouse or switch to your current location."
							}
							origin={pickup ? undefined : detail.routing.origin}
							destinations={
								dispatch
									? [
											{
						...
						<section className="rounded-xl border bg-card p-4 shadow-sm">
							<p className="text-[11px] font-bold uppercase tracking-[0.14em] text-emerald-700 dark:text-emerald-300">
								{pickup ? "Pickup at" : "Deliver to"}
							</p>
							<h2 className="mt-1 font-semibold">{customerName}</h2>
							<p className="mt-1 text-sm text-muted-foreground">
								{pickup ? routeAddress : address || "Address review required"}
							</p>
							{detail.destination.displaySecondary ? (
								<div className="mt-3 rounded-lg border border-emerald-600/20 bg-emerald-50 p-3 dark:bg-emerald-950/20">
									<p className="text-[11px] font-bold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">
										Verified route destination
									</p>
									<p className="mt-1 text-sm">{routeAddress}</p>
								</div>
							) : null}
							<div className="mt-4 grid grid-cols-2 gap-2">
								...
								<Button
									...
									onClick={() =>
										window.open(
											`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(routeAddress)}`,
											"_blank",
											"noopener,noreferrer",
										)
									}
							...
							<div className="divide-y px-4 text-sm">
								...
								<div className="flex items-center justify-between gap-3 py-3">
									...
									<strong>
										{pickup
											? "Pickup"
											: detail.destination.verified && routeAddress
												? "Verified"
												: "Review"}
									</strong>
								</div>
			...
			<div className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-[auto_1fr] gap-2 border-t bg-background/95 px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 backdrop-blur sm:hidden">
				...
				{primaryAction.kind === "proof" ? (
					...
					<Button variant="secondary" disabled>
						<CheckCircle2 className="mr-2 size-4" /> Delivery completed
					</Button>
				) : primaryAction.kind === "cancelled" ? (
					<Button variant="secondary" disabled>
						{primaryAction.label}
