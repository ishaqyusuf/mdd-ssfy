...
type AutoSaveFlushOptions = {
	...
	payloadOverride?: NewSalesFormSaveDraftInput;
};
...
export function useNewSalesFormAutoSave(
	options: UseNewSalesFormAutoSaveOptions,
) {
	const {
		enabled = true,
		delayMs = 1000,
	...
	const payloadKey = useMemo(() => {
		...
		return JSON.stringify({
			...
			meta: payload.meta,
			specialOrderDeclaration: payload.specialOrderDeclaration,
			specialOrderChangeReason: payload.specialOrderChangeReason,
		});
	}, [payload]);
		...
		queuePromiseRef.current = (async () => {
			...
			try {
				while (mountedRef.current && queuedPayloadRef.current) {
					...
					onSaving?.(
						next,
						next.autosave === false ? "manual-flush" : "debounced-change",
					);
	...
	const flush = useCallback(
		async (
			...
			const next = {
				...(options.payloadOverride ?? payload),
				autosave: reason !== "manual-flush",
				commitIntent:
					options.commitIntent ??
					(reason === "manual-flush" ? "draft" : "autosave"),
			};
