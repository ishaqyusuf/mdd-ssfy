...
export function createRecoverySnapshot(
	...
	return {
		...
		savedAt,
		...(options.manualSaveRequired ? { manualSaveRequired: true } : {}),
...
export function getRecoveryStorageKey(source: {
	...
	const scope =
		String(source.slug || source.salesId || "draft").trim() || "draft";
...
export function writeRecoverySnapshot(
	...
	if (!isBrowser()) return;
	const snapshot = createRecoverySnapshot(payload, undefined, options);
	try {
...
export function readRecoverySnapshot(
	key: string,
): NewSalesFormRecoverySnapshot | null {
...
export function createPayloadFingerprint(
