...
const MATERIAL_REVIEW_REFRESH_INTERVAL_MS = 5_000;
...
export function ProductionMaterialReviewPanel({
	...
	const queueQuery = useInfiniteQuery(
		trpc.sales.productionSubmissionMaterialReviews.infiniteQueryOptions(
			...
			},
			{
				...
				refetchInterval: orderContext
					? MATERIAL_REVIEW_REFRESH_INTERVAL_MS
					: false,
			},
		),
	);
	...
	const productionItems: ReviewProductionItem[] = Array.isArray(
		detail?.productionItems,
	)
		? detail.productionItems.flatMap((item: unknown) => {
				if (!item || typeof item !== "object") return [];
				const row = item as Record<string, unknown>;
				if (typeof row.salesItemId !== "number") return [];
				return [
					{
						salesItemId: row.salesItemId,
						title: String(row.title || "Production item"),
						description:
		...
		: [];
	const productionItemBySalesItemId = new Map(
		productionItems.map((item) => [item.salesItemId, item] as const),
	);
	const materialReviewRows: ReviewMaterialRow[] = materialSnapshot.map(
		(material, index) => {
			...
			const hasValidComponent =
				...
				componentId > 0;
			return {
				key: `${String(componentId)}-${index}`,
				componentId,
				name: String(row.name || "Required material"),
				description:
					productionItemBySalesItemId.get(Number(row.salesItemId))
						?.description || null,
				readiness,
	...
	function handleQueueScroll(event: UIEvent<HTMLDivElement>) {
		if (!queueQuery.hasNextPage || queueQuery.isFetchingNextPage) return;
		...
		<div className="flex flex-col gap-3">
			...
			<Card className="overflow-hidden rounded-xl bg-card shadow-sm">
				...
				>
					{showReviewQueue ? (
						<div
							aria-label="Pending material reviews"
							role={standalone ? "region" : undefined}
							tabIndex={standalone ? 0 : undefined}
							className={cn(
								"flex flex-col",
								standalone &&
									"overflow-y-auto overscroll-contain lg:border-r lg:pr-4",
							)}
							onScroll={standalone ? handleQueueScroll : undefined}
							style={
								standalone
									? {
											height:
												"clamp(18rem, calc(100dvh - 30rem + var(--header-offset, 0px)), 34rem)",
										}
									: undefined
							}
						>
							...
							) : !standalone && queueQuery.hasNextPage ? (
								<Button
									type="button"
									variant="outline"
							...
							<div className="flex flex-col gap-4">
									...
									<Alert>
										...
										<AlertTitle>
											{detail.submissions.length
												? "Part of this submission was retracted"
									...
									<div className="divide-y">
										{materialReviewRows.length ? (
											materialReviewRows.map((material) => {
												const checkboxId = `material-review-${selectedReviewId}-${material.key}`;
												const isSelected =
													material.componentId !== null &&
													manualComponentIds.includes(material.componentId);
												return (
													<label
														key={material.key}
														htmlFor={checkboxId}
														className={cn(
															...
															!material.canMarkAvailable && "cursor-default",
														)}
													>
														<Checkbox
															id={checkboxId}
														...
														<span className="min-w-0">
															...
															{material.description ? (
																<span className="mt-1 block text-xs font-medium uppercase text-muted-foreground">
																	{material.description}
																</span>
															) : null}
												...
												{detail.linkedInboundReceipts.map((receipt) => {
													...
													return (
														<div
															...
															className="grid gap-3 py-3 sm:grid-cols-[minmax(0,1fr)_100px_100px]"
														>
								...
								{needsConfigurationException ? (
									<Alert variant="warning">
										...
										<AlertDescription>
											Approve only after confirming the completed item is
											physically available. This records an auditable exception
											and does not create or move inventory.
										</AlertDescription>
									</Alert>
								) : null}
										...
										onClick={() =>
											batchAssign.execute({
												...
												dueDate: batchDueDate
													? createProductionDueDate({
															year: Number(batchDueDate.slice(0, 4)),
															month: Number(batchDueDate.slice(5, 7)),
															day: Number(batchDueDate.slice(8, 10)),
														})
													: null,
											})
										}
	...
	return (
		<>
			<div className="space-y-3 border-b pb-3 last:border-b-0 last:pb-0">
				...
				{progress.isCompleted ||
				(scope === "worker" && !progress.canSubmitMore) ? (
