...
type LegacyGeneralTabData = NonNullable<
	ReturnType<typeof useSaleOverview>["data"]
> & {
	dispatchList?: Array<{ id: number; title: string }> | null;
	due?: number | null;
	...
	status?: {
		assignment?: { color?: string | null; status?: string | null } | null;
		delivery?: { color?: string | null; status?: string | null } | null;
		production?: { color?: string | null; status?: string | null } | null;
	} | null;
};
...
export function GeneralTab({
	...
	const legacyData = data as LegacyGeneralTabData | undefined;
	...
	} as unknown as Partial<LegacyGeneralTabData>;
	const saleData = legacyData || ph;
	...
	const paymentPercentage =
		invoiceTotal > 0 ? (invoicePaid / invoiceTotal) * 100 : 0;
	...
	return (
		<DataSkeletonProvider value={{ loading: !saleData?.id } as any}>
			<div className="relative mt-0 space-y-6 py-6">
				...
				<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
						...
						<div>
							<div className="mb-3 flex items-center justify-between gap-3">
								...
								<SalesCustomerEditButton
									...
									billingAddressId={saleData?.addressData?.billing?.id}
									salesId={saleData?.id}
									shippingAddressId={saleData?.addressData?.shipping?.id}
									...
									<div>
										...
										>
											<Button
												...
												onClick={(e) => {
													customerQuery.open(saleData.accountNo);
												}}
							...
							<div className="space-y-3">
									...
									<div>
										...
										<DataSkeleton className="font-medium" placeholder="Order">
											<p className="font-medium capitalize">
												{saleData?.isQuote ? "Quote" : saleData?.type}
											</p>
										</DataSkeleton>
									</div>
									{!isQuote ? (
										<div>
											...
											>
												{hasInventoryInbound ? (
													<InventoryInboundStatusBadge
														ownership={saleData?.inventoryInboundOwnership}
													/>
												) : (
													<SalesInboundStatusBadge
								...
								<div className="grid grid-cols-2 gap-2 text-sm">
									<div>
										...
										>
											<p className="font-medium">{saleData.salesDate}</p>
										</DataSkeleton>
									</div>
									{isQuote || (
										<div>
					...
					<div className="space-y-6">
								...
								<Card className="border-border/40 bg-muted/20">
									<CardContent className="p-4">
										<div className="flex flex-wrap items-end justify-between gap-3">
											...
											<div className="text-right">
												...
												<p className="text-xs text-muted-foreground">
													<DataSkeleton className="inline" placeholder="$0.00">
										...
										<p className="mt-3 text-xs text-muted-foreground">
											{cccPending > 0 ? (
												<>
													<Money value={invoicePending} /> order balance +{" "}
								...
								<div className="mt-2 grid grid-cols-2 gap-4 border-t border-border/40  pt-3">
									<div>
										...
										<DataSkeleton
											...
											placeholder="NET10"
										>
											<p className="text-sm font-medium">{saleData.netTerm}</p>
										</DataSkeleton>
									</div>
									<div>
						...
						{saleData.due && !isQuote ? (
							onCreatePayment ? (
								<Button className="w-full" onClick={onCreatePayment}>
										...
										<SalesOverviewPaymentMethodSelect
											...
											value={
												Number(invoicePending || 0) <= 0
													? saleData?.paymentSummary?.methodLabel ||
														saleData?.paymentMethod
													: saleData?.paymentMethod
											}
											disabled={isQuote || Number(invoicePending || 0) <= 0}
											className="py-2"
										/>
										{costLines.map((c, ci) => {
											const key = String(c.id ?? `${c.label}-${ci}`);
											return (
												<React.Fragment key={key}>
													<div className="flex justify-between text-sm">
														...
														{c.format === "count" ? (
															...
															<DataSkeleton className="" placeholder="$0.00">
																<span>
				...
				<div>
					...
					<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
						{[
							...
							<Card key={addressType} className="border-border/40">
								<CardContent className="p-4">
										...
										<SalesAddressEditButton
											...
											onEdit={() =>
												onEditAddress?.({
													addressId: address?.id,
													addressType:
														addressType === "bad" ? "billing" : "shipping",
													label:
														address?.title ||
														(addressType === "bad"
															? "Billing address"
											...
											readOnly={Boolean(
												saleData?.isDealerSale || addressEditingLocked,
											)}
										/>
									</div>
									<DataSkeleton
									...
									>
										<address className="text-sm not-italic text-muted-foreground">
											{address?.lines?.filter(Boolean).map((line, li) => (
				...
				<div className={cn(!isQuote || "hidden")}>
					...
					{saleData?.stats?.prodAssigned?.total === 0 && saleData?.id ? (
						<Card className="border-border/40">
							<CardContent className="p-4">
								...
								<CardContent className="p-4">
									...
									>
										<p className="text-sm text-muted-foreground">
											...
											{saleData?.stats?.prodAssigned?.total} items assigned
										</p>
									</DataSkeleton>
								</CardContent>
							</Card>
								...
								<CardContent className="p-4">
									...
									>
										<p className="text-sm text-muted-foreground">
											...
											{saleData?.stats?.prodCompleted?.total} items completed
										</p>
									</DataSkeleton>
								</CardContent>
							</Card>
