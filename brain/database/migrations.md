...
- Local development now uses explicit DB/Redis profiles. `bun run dev` defaults to hosted dev DB plus remote dev Redis through `scripts/with-dev-infra.ts`; local Docker MySQL and local Docker Redis are opt-in with `bun run dev:local`, `bun run dev:local-db:remote-redis`, `bun run dev:remote-db:local-redis`, or the service-only `dev:services:local*` commands.
