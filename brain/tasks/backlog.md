...
- [ ] Fix production Square terminal payment device loading/authorization bug before re-enabling terminal payments in production. Current failure: connected terminals sometimes do not load, and selected devices can fail with Square `BAD_REQUEST`, `Merchant not authorized for device_id=device:238CS149B2002443` (API + UI + Validation)
