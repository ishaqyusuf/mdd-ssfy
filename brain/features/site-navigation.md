...
- The sidebar footer account dropdown opens upward inside the expanded sidebar at the expanded sidebar's inner footer width. Clicking the footer user control expands the sidebar and opens the account menu. Hovering out of the sidebar still collapses the sidebar, but if the account menu was open its requested-open state is preserved while hidden; hovering back over the sidebar expands it and restores the open menu.
...
- The footer account dropdown is a portal/floating hover surface: entering it cancels pending sidebar collapse, leaving it only schedules collapse when the pointer is not moving to another nav hover surface, and it does not schedule sidebar expansion by itself.
