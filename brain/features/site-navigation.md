...
- Active `apps/www` sidebar links should resolve to current App Router routes. The 2026-06-17 cleanup removed the no-route sales commission item, retargeted unit production to `/community/unit-productions`, retargeted mobile app support to `/settings/app-download`, and kept edit-order as a meta matcher instead of a clickable `/sales-book/edit-order` URL.
