...
- The Inventory tab segment order is `Stock | Inbounds | Non Stock`, keeping stock work first, linked inbound shipments second, and untracked/component rows last.
...
- Shared web `SalesMenu.MarkAs` production-complete and fulfilled actions now call `inventories.salesInventoryMarkAsPreflight` before starting the existing `update-sales-control` tasks. Orders without inventory-backed rows continue through the legacy Mark As path; configured orders with required components still awaiting inbound or allocation are blocked behind a shadcn alert dialog that summarizes blocked orders, pending quantity, open inbound quantity, and sample component blockers. The dialog can run `inventories.resolveSalesInventoryMarkAsAvailabilityForContinue` only when every blocker is safe to resolve by cancelling unlinked mutable inbound demand; shipment-linked, partially received, or allocation-required blockers stay paused for Inventory tab review. Continue-mode resolution writes order-scoped `SalesHistory` audit rows and reports `auditHistoryCount`; order status is only set to `AVAILABLE` for orders whose demand rows are confirmed cancelled by that apply. Stale preview demand that cannot be cancelled during apply does not recompute components, update order inventory status, write Mark As audit history, or allow continue.
