...
- Current Phase: Dealer portal data surfaces implemented; shared sales form contract next
- Next Step: Move the shared sales form system behind dealer-owned contracts so dealers can create quotes/orders for their own customers using dealer sales profiles.
...
### Resume Prompt
Continue the Dealership Program from `brain/plans/ongoing.md`. Current phase: dealer portal data surfaces implemented; shared sales form contract next. Next step: move the shared sales form system behind dealer-owned contracts so dealers can create quotes/orders for their own customers using dealer sales profiles. Important product decisions: dealers are managed from a dedicated Dealers page, adding a dealer supports existing customer lookup or new dealer name/email invite, onboarding sends a setup guide through the channel notification system, dealer-managed customers are separate dealer-owned records, dealer auth uses Better Auth from `packages/auth`, the new sales form system in `packages/sales` is shared by `apps/www` and `apps/dealership`, and v1 billing is ledger-only. Validation status: `bun install`, Prisma generation, and `@gnd/db` typecheck pass; full API/www/notifications/email/dealership typechecks inherit unrelated existing errors from `apps/api`, `packages/ui`, `packages/email`, and `packages/sales`, while filtered checks show no dealer-specific errors. Runtime smoke is blocked by portless sandbox permissions. Update `brain/plans/ongoing.md` as progress continues.
