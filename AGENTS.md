...
- Sync production data only when asked. Start with `bun run db:sync --to-local -- --dry-run`; `db:sync` defaults to `--from-prod --to-local`, `--to-preview` explicitly changes the destination, and production is never a valid destination.
