...
export function Search() {
  ...
  const combinedData = useMemo(() => {
    const getSearchResultAction = (item: SearchItem) => {
      const name = item.name || item.type;
      const meta = (item.meta || {}) as Record<
        string,
        string | number | boolean | null | undefined
      >;

      if (item.href) {
        return () => nav.navigateToPath(item.href as string);
      }
      ...
      switch (name) {
        case "sales":
          return () =>
            nav.navigateToPath(
              item.href || `/sales-book/orders?salesId=${meta?.salesId ?? item.id}`,
            );
        case "quotes":
          return () =>
            nav.navigateToPath(
              item.href || `/sales-book/quotes?salesId=${meta?.salesId ?? item.id}`,
            );
        case "dispatch":
          return () =>
            nav.navigateToPath(
              item.href ||
                `/sales-book/dispatch?dispatchId=${meta?.dispatchId ?? item.id}`,
            );
        case "employees":
          return () =>
            nav.navigateToPath(
              item.href || `/hrm/employees?employeeId=${meta?.employeeId ?? item.id}`,
            );
        case "customers":
          return () =>
            nav.navigateToPath(
              item.href ||
                `/sales-book/customers?customerId=${meta?.customerId ?? item.id}`,
            );
        case "projects":
          return () =>
            nav.navigateToPath(
              item.href || `/community?projectId=${meta?.projectId ?? item.id}`,
            );
        case "units":
          return () =>
            nav.navigateToPath(
              item.href || `/community/project-units?unitId=${meta?.unitId ?? item.id}`,
            );
        case "templates":
          return () =>
            nav.navigateToPath(
              item.href ||
                `/community/templates?templateId=${meta?.templateId ?? item.id}`,
            );
        case "builders":
          return () =>
            nav.navigateToPath(
              item.href || `/community/builders?builderId=${meta?.builderId ?? item.id}`,
            );
        default:
          return item.action || (() => {});
      }
    ...
    const mappedSearchResults = searchResults.map((res) => ({
      ...res,
      action: getSearchResultAction(res),
    }));
