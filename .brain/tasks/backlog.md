...
## Next Up
- [ ] Implement the customer-approved sales quantity-reduction and wallet-credit plan: immutable before/after line evidence, secure customer approval, stale-revision guards, canonical repricing, due-first settlement, exact idempotent wallet credit, inventory/document reconciliation, and Sales Overview/invoice/Finance/customer visibility (`.brain/plans/2026-08-04-sales-quantity-reduction-wallet-credit.md`) (Schema + Sales Domain + Payments + API + Jobs + UI + Print + Inventory + Validation)
...
- [ ] Legacy sales-form mobile + architecture refactor follow-through: after the domain root and shell extraction land, continue step-family migration for HPT, moulding, service, shelf-items, takeoff, modal re-homing, and import cleanup (`brain/legacy-sales-form-mobile-architecture-plan.md`) (Architecture + UI + Validation)
...
- [ ] Validate the full production Sales Overview terminal payment flow after deploy/restart, including terminal selection, checkout creation, customer payment completion, polling, and payment application. The production gates are re-enabled and the authorization root cause was fixed by normalizing `device:`-prefixed ids before Terminal Checkout. (Ops + Validation)
