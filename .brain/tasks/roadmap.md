...
- Status: Done
- Plan Status: Implemented
- Spec File: brain/plans/2026-07-13-spec-task-monitor-client-simplification-and-error-ledger.md
- GitHub Issue: https://github.com/ishaqyusuf/mdd-ssfy/issues/42
- Scratch Map: .scratch/task-monitor-client-simplification/map.md
- Created Date: 2026-07-13
- Completed Date: 2026-07-13
- Local Only: yes
- Latest Implementation Note: Production task feedback now shows only a spinner-style loading circle and terminal success/destructive toasts, while development/local keeps the detailed monitor. Generic task starts, start failures, and terminal runs are persisted in `TaskRunDiagnostic`, finalized from server-side Trigger status, and exposed through `/task-events/diagnostics` for Super Admin review. Hosted dev schema push succeeded; `migrate dev` remains blocked by shadow database permissions on the hosted target.

### App Auto Update Parity
- Priority: High
- Description: Track local-only spec in `brain/plans/2026-07-12-spec-app-auto-update-parity.md`; align the GND Expo app's installed-build OTA behavior with the local al-ghurobaa auto-update flow, including launch checks, foreground cooldown checks, blocking update progress modal, automatic reload, manual Settings diagnostics, and explicit GND workflow interruption guardrails.
- Related Feature: Expo mobile app, EAS Update, mobile build variants
- Status: Roadmap
- Plan Status: Proposed
- Plan File: brain/plans/2026-07-12-spec-app-auto-update-parity.md
...
- Priority: Medium
- Description: Track the measurement-aware, unordered-token shelf product
  search plan in
