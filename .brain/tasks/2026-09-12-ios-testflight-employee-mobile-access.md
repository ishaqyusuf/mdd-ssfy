...
- September 15 isolated backend candidate: local `master` is 187 commits ahead
  ...
  a candidate-owned frozen dependency install succeeds; offline Prisma schema
  validation and client generation succeed; the same forty tests pass against
  that isolated install; `git diff --check` passes; and the worktree is clean.
  Auth typecheck reports only the known `packages/errors` NodeNext extension
  baseline, while broad Biome on the old remote baseline exposes pre-existing
  formatting/lint debt. The branch is local only: no push, migration, Vercel
  link, deployment or alias change.
