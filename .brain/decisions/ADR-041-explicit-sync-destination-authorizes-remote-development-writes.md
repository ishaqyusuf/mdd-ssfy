# ADR-041: Explicit Sync Destination Authorizes Remote-Development Writes
...
`db:sync` defaults to production-to-local, while `--to-remote` selects the
hosted remote-development database. GND previously required both
`--to-remote` and `GND_ALLOW_REMOTE_DEV_DB_SYNC=1` before writing remotely.
...
production destinations, rejects a remote target that resolves to production,
...
An explicit `bun run db:sync --to-remote` is the authorization signal for a
remote-development write. GND no longer reads or requires
...
- Remote synchronization requires one explicit, visible destination choice.
...
- `bun run db:sync --to-remote -- --help`
