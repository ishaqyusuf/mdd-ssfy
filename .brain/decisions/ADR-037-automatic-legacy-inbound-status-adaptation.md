...
- `ORDERED` synchronizes requirements, advances linked pending shipments to
  `in_progress`, and creates `in_progress` shipments for unlinked demand.
...
When no deterministic supplier exists, the shipment is created with
`supplierId=null`; the supplier can be assigned later. Status is the only
required inbound header value. Expected date and PO/reference remain optional,
and the system never creates a placeholder supplier.
...
- Supplier configuration no longer blocks inbound creation.
- Existing clients remain compatible during the alias window.
- `InboundShipment.supplierId` is nullable; the additive migration preserves all
...
- Authenticated local browser validation on order `09068PC` confirmed guarded
  `ORDERED` adaptation created inbound `#109` as `in_progress` with two linked
  demand rows, no supplier, no expected date, and the historical status shown
  canonically as `In progress`.
