...
## Purpose
Grouped route reference for the `apps/dashboard` Next.js App Router surface.

## Notes
- Source scanned from `apps/dashboard/src/app/**/page.tsx`.
...
- Active optimization and prioritization should use `apps/dashboard/src/components/sidebar/links.ts` (`linkModules`) as the source of truth for current user-facing navigation.
...
- Old dealer guest signup/create-password files under `apps/dashboard/src/app-deps` were removed during the 2026-06-17 unused-code cleanup because they were not live `src/app` routes.
