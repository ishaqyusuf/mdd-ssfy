...
Date: 2026-04-15
Scope: `apps/dashboard` Next.js App Router performance rules for active user-facing routes.
...
When working on `apps/dashboard`, use Midday as the reference shape:
...
- Treat `apps/dashboard/src/components/sidebar/links.ts` as the authoritative active-route scope.
...
- HRM
- Production / Jobs
- Any future linked workspace in `apps/dashboard`

## Validation Follow-up
- After a linked-route optimization pass, validate the highest-traffic shortlist before declaring the work stable.
- Use `brain/engineering/dashboard-performance-validation-checklist.md` as the required follow-up checklist.
