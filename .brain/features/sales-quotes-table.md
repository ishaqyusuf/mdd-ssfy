...
- `/sales-book/quotes`
  ...
  - renders `apps/dashboard/src/components/tables-2/sales-quotes/*`
...
- `/sales-book/quotes/bin`
  ...
  - renders the same `apps/dashboard/src/components/tables-2/sales-quotes/*` table with `bin` enabled
...
- 2026-07-17 density/width tuning:
  ...
  - filtered `@gnd/dashboard` typecheck scan produced no diagnostics for touched Quotes/embed/table-config files.
...
- 2026-06-16:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics for the migrated quotes files.
...
- 2026-06-16 quote bin:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics for quote-bin and shared quote table files.
...
- 2026-06-16 quote table follow-up:
  - removed the P.O column from `apps/dashboard/src/components/tables-2/sales-quotes/columns.tsx` without adding or removing quote filter params.
  - removed the leftover P.O column and mobile PO badge from `apps/dashboard/src/components/tables/sales-quotes/columns.tsx` so the still-live legacy quote embed does not reintroduce the field before its migration.
  - added a domain-level quote sort guard in `apps/dashboard/src/components/tables-2/sales-quotes/sort.ts` and removed the derived `displayName` customer sort from the quote table surface.
  - focused Biome and filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the touched quote table/route/config files.
...
- 2026-07-16 restarted Sales Orders parity pass:
  ...
  - added `apps/dashboard/src/components/tables-2/sales-quotes/migration-parity.test.ts` and enrolled Sales Quotes in `apps/dashboard/src/components/page-sticky-header.test.ts` so the route cannot regress to the failed shared wrapper, legacy quote table, `@gnd/ui/data-table`, or route-level `fetchInfiniteQuery`.
  - validation: focused Sales Quotes/audit tests passed with 10 tests / 46 assertions; the restarted parity suite passed with 42 tests / 286 assertions; targeted Biome passed for the route/test/audit files; static scans found no live `PageStickyHeader`, legacy quote table, `@gnd/ui/data-table`, or route-level `fetchInfiniteQuery` usage in the route/new table; filtered `@gnd/dashboard` typecheck scan reported no touched-file diagnostics; `git diff --check` passed.
...
- 2026-07-16 sales-rep embed follow-up:
  ...
  - Added `apps/dashboard/src/components/tables-2/sales-rep-embeds/migration-parity.test.ts` and enrolled the sales-rep quote embed in `apps/dashboard/src/components/page-sticky-header.test.ts`.
