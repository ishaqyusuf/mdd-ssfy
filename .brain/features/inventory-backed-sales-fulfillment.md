...
- Active orders with a recognized manual `SalesOrders.inventoryStatus` but no inventory-backed rows return `setupMode=legacy_status_locked`, and opening the Inventory tab now starts one guarded automatic adaptation for the exact saved status. `ORDERED` synchronizes requirements and creates supplier-resolved `in_progress` inbounds (or advances linked pending inbounds); `PENDING ORDER` creates supplier-resolved `pending` inbounds; `AVAILABLE` runs the guarded manual-fulfillment core without recording physical stock movement. Missing or ambiguous suppliers return partial success and open the stock/create-inbound recovery path without fabricating a supplier. Automatic failures do not loop; the UI exposes operator Retry and a confirmed `Clear legacy status and configure from scratch` fallback. API aliases `override -> continue` and `reset -> clear` remain accepted for older clients, while new history writes use canonical action names.
