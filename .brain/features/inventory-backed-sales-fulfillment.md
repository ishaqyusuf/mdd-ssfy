...
- Needs items now use a flat row with an explicit `border-border` bottom divider
  ...
  facts and separates available fulfillment coverage from linked ordered
  inbound. `AVAILABLE: x OF y` compares allocated plus received inbound quantity
  with total required quantity, clamps duplicate evidence to the requirement,
  ...
  remaining requirement after available quantity. When ordered inbound exists
  but available quantity is zero, the redundant zero-available badge is hidden
  and Ordered uses the full required quantity as its denominator. When neither
...
- `sales.getOrders` and `sales.inboundIndex` also expose `inventoryInboundOwnership`, including compact linked inbound shipment status summaries. Only active, non-deleted, non-cancelled inbound shipments count as inventory-owned inbound status. The canonical Sales Orders table now routes rows without inventory-owned inbound into Sales Overview Inventory/Needs with Create inbound expanded; inventory-owned rows show the linked inbound shipment status such as `In progress`, `Received`, or a multi-inbound count and open the Inventory/Inbounds workspace. A single linked shipment opens directly, while several linked shipments preserve the list for operator selection. The separate inbound-management action retains its legacy manual-status recovery behavior for orders that have not entered inventory-owned inbound.
- Sales inventory projection and Sales Orders applicability counts include only requirements that resolve through the canonical tracking policy as tracked inventory. Untracked and component-kind rows remain visible under `Not Needed` but do not make the Inbound column show `Set status` or `Inventory required`; a ready projection with zero current tracked requirements resolves to `N/A`, even when an older projection count included those non-actionable rows.
- The Sales Orders inbound-column applicability check uses `SalesInventoryProjectionState` when present and falls back to active required positive-quantity `LineItemComponents` when an older synchronized order predates that marker. Existing inventory needs therefore remain actionable instead of being mislabeled `Not synced`; the fallback does not claim synchronization for rows without positive required-component evidence.
- Recognized legacy manual inbound statuses (`AVAILABLE`, `ORDERED`, and `PENDING ORDER`) now take precedence over `not_synced` or failed projection attention in the Sales Orders Inbound column. Those rows render the preserved status as an amber locked compatibility badge and open the Inventory review state instead of the ordinary create-inbound flow. Active linked inventory inbound ownership and an in-progress synchronization remain higher-precedence canonical states; completed zero-need projections continue to render `N/A`.
- The Sales Orders Inbound `N/A` state is an accessible informational button.
  Clicking it leaves the row closed and shows the applicability description in
  a toast, distinguishing a zero-need sale from a legacy sale that completed
  production before inventory synchronization was available.
...
- Active orders with a recognized manual `SalesOrders.inventoryStatus` and no canonical representation return `inventoryLegacyCompatibility.state=legacy_locked`, including partially synchronized orders that have inventory needs but no linked inbound. Opening Inventory starts one guarded automatic adaptation for the exact saved status. `ORDERED` synchronizes requirements and creates `in_progress` inbounds (or advances linked pending inbounds); `PENDING ORDER` creates `pending` inbounds; `AVAILABLE` runs the guarded manual-fulfillment core without recording physical stock movement. Inbound status is sufficient for creation: supplier, expected date, and PO/reference are optional, and unresolved supplier demand is grouped into a supplier-less inbound that can be completed later. Automatic failures do not loop; the UI exposes operator Retry and a confirmed `Clear legacy status and configure from scratch` fallback. API aliases `override -> continue` and `reset -> clear` remain accepted for older clients, while new history writes use canonical action names.
- Shared web `SalesMenu.MarkAs` production-complete and fulfilled actions call
  `inventories.salesInventoryMarkAsPreflight` before starting the existing
  `update-sales-control` tasks. Orders without inventory-backed rows continue
  through the legacy path; configured orders with unresolved required
  components show the `Inventory needs attention` summary. Both actions now
  expose an explicit `Mark available and continue` override. The resolver still
  preserves canonical inventory/component/inbound state, writes an order-scoped
  `SalesHistory` workflow-availability override record, and permits the chosen
  status task to continue even when the detailed component preflight remains
  blocked. The separate existing availability resolver retains its safe-only
  behavior for guarded unlinked, unreceived mutable demand and requires a clean
...
- Sales-demand inbound creation accepts an initial `pending` or `in_progress` shipment status plus an optional operator note. Existing linked shipment status updates accept the same optional note. Created and status-updated lifecycle activity carries the linked sales order numbers, so it appears in the relevant Sales Overview activity stream.
...
- Sales overview Inventory/Needs exposes `Mark as available` for active tracked shortages. It reuses the inbound form with an explicit `mark_available` operation, preselects every eligible item, and offers a Mark / unmark all checkbox plus per-item quantity controls for selective fulfillment. It receives selected demand through the canonical inbound stock/movement/log path. The order prompt becomes `AVAILABLE` only when no open demand remains; partial selections preserve the outstanding need. No new database column is required. The Inbounds badge reads from a lightweight linked-shipment count before the detail list opens, and linked shipments render as collapsible rows with status, receiving, item, demand, and read-only lock details.
- Creating an inbound explicitly refetches the active infinite Sales Orders query. The Inbound column gives actual active linked-inbound ownership precedence over projection/setup labels, so a newly created shipment immediately renders its inventory status instead of remaining `Not synced` until a page refresh.
- Material Sales Handoff deep links keep `inventoryCreateInbound=true` in the URL for the lifetime of the Sales Overview Create inbound secondary pane. The mounted pane prevents duplicate opens, copied-link reload reopens the same continuation, and pane or outer-overview close clears the flag while retaining the existing trigger-focus restoration path.
- Validation: `bun test packages/sales/src/sync-sales-inventory-line-items.test.ts packages/sales/src/sales-inventory-overview.test.ts apps/dashboard/src/hooks/sales-overview-open-params.test.ts` passed with 22 tests and 40 assertions. In-app browser validation passed on `/sales-book/orders?sales-overview-id=08578AD&sales-type=order&mode=sales&salesTab=inventory`, confirming the old sheet shows Inventory `New`, renders Dyke production detail values as inventory components, and shows a single merged inventory component table after cold reload.
