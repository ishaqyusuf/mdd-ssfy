...
## Current Behavior
- The route uses the restarted Sales Orders table standard through `apps/dashboard/src/components/tables-2/inventory-backorders/*`.
...
- 2026-07-17 density update:
  ...
  - Full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2547 assertions.
  - Focused Biome passed for the Backorders columns, parity test, and `table-configs`.
  - Filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics for Backorders/table-configs.
...
- `bun test apps/dashboard/src/components/tables-2/inventory-backorders/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 6 tests / 19 assertions.
- `bun test apps/dashboard/src/components/tables-2/*/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 81 tests / 714 assertions.
...
- Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the touched backorders table, route, workspace, registry, and audit files while broad typecheck remains subject to existing unrelated baseline errors.
