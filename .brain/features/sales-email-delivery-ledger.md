...
- Local jobs/dev commands now resolve the development database through `scripts/with-dev-infra.ts`, so `bun run dev --filter jobs` uses the selected dev DB profile from the root dev router.
