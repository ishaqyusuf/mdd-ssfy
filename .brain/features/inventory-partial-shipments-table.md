...
## Current Behavior
- The route uses the restarted Sales Orders table standard through `apps/dashboard/src/components/tables-2/inventory-partial-shipments/*`.
...
- 2026-07-17 density/scroll validation:
  - `bun test apps/dashboard/src/components/tables-2/inventory-partial-shipments/migration-parity.test.ts` passed with 4 tests / 25 assertions.
  - `bun test apps/dashboard/src/components/tables-2` passed with 306 tests / 2553 assertions.
  - Focused Biome passed for Partial Shipments columns, parity test, and `table-configs`.
  - Filtered `@gnd/dashboard` typecheck grep produced no touched-file diagnostics for Partial Shipments or `table-configs`.
...
- `bun test apps/dashboard/src/components/tables-2/inventory-partial-shipments/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 6 tests / 22 assertions.
- `bun test apps/dashboard/src/components/tables-2/*/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 87 tests / 752 assertions.
...
- Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the touched partial-shipment table, route, workspace, registry, and audit files while broad typecheck remains subject to existing unrelated baseline errors.
