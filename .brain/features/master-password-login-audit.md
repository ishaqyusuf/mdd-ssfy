...
- When a master password is used, `MasterPasswordLoginAudit` records the target user snapshot, app surface, platform, IP address, best-effort ISO country code from trusted Vercel/Cloudflare edge headers, browser/user agent, safe session id, and login timestamp.
...
- The page lists active audit records with search, platform filtering, pagination, column visibility, and refresh.
