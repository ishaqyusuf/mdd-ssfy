...
- Manual order inbound-status saves map `notes.saveInboundNote` to `inventory.inbound.changed` and scope the event from the returned order. The event refreshes Sales Orders lists/summaries, inventory inbound reads, saved page-tab count data, and `notes.activityTree`, including inactive tab queries where configured. Inventory shipment creation and lifecycle status mutations therefore refresh the Sales Overview activity stream without a page reload.
