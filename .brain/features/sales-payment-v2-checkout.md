...
- The staff payment method control is adaptive and remains immediately after the compact payment-date control. Check renders one grouped method trigger plus check-number input. Terminal opens a nested, availability-aware device menu and returns to the normal trigger labeled with the selected device. Non-terminal selection clears terminal identity/session state, and submission sends check data only for Check and device/session data only for Terminal.
