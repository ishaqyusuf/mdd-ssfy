...
- 2026-07-28: Production Connected-mode pairing is now working. Staff terminal status polling explicitly bypasses the dashboard QueryClient's 60-second `staleTime`, so each 2-second poll reaches Square instead of repeatedly returning a cached `PENDING` response. `CANCEL_REQUESTED` remains an intermediate state until Square reports `CANCELED`, at which point the UI clears the loading overlay and shows an explicit no-payment-applied cancellation notice. On completion, the browser rebuilds the selected sale ids and order numbers before resubmitting the verified terminal session; the server refuses to finalize a checkout when no order was credited. Completion is reverified server-side, and the matching local `SquarePayments` row is atomically claimed `PENDING -> PROCESSING -> COMPLETED` with the verified tip while the order payment is recorded. A guarded orphan-recovery claim permits a previously false-completed row only when it has no linked sale payment. Cancellation uses Square's cancel-checkout endpoint rather than dismissal and persists `CANCELED` only after Square confirms it. Live localhost QA completed two $0.01 charges on Terminal 2443; both Square checkouts were recovered and linked exactly once to order `08870LM`, whose paid amount became $0.02 and due balance became $192.05.
- 2026-07-28: Production readiness polling now covers the complete 10-second Square `PING` action deadline plus a 2-second response grace period. A regression test proves that an acknowledgement arriving after the former 5-second cutoff succeeds instead of being rejected. Fresh 18-second diagnostics and the corrected 12-second application check still left both Terminals 1451 and 2443 `PENDING`, while Square reported the production location, paired `TERMINAL_API` sessions, and recent `AVAILABLE` device updates correctly. This isolates the remaining blocker to the physical terminals not acknowledging their GND Connected-mode sessions; no checkout or local pending-payment row is created until one acknowledges.
- 2026-07-22: Staff terminal checkout now treats the Square device selector as a controlled form field, requires an explicit choice when multiple devices are online, and shows actionable load/validation/availability feedback instead of silently logging an invalid form or silently routing to the last-used terminal.
- 2026-07-22: The terminal-payment mutation re-reads Square devices and same-app paired device codes at submission time, canonicalizes prefixed/unprefixed device ids, accepts only a server-observed `AVAILABLE` paired device, and persists the server-observed terminal id/name only after Square creates the checkout. Offline/stale/unpaired selections and Square checkout failures do not create a local pending payment.
- 2026-07-22: Before creating a checkout, the API sends a non-payment Square Terminal `PING` and waits for device acknowledgement. A terminal that is online in the merchant device inventory but not actively signed into GND Connected/Terminal API mode now fails with an actionable sign-in instruction before checkout creation or local pending-payment persistence.
- 2026-07-22: Local terminal diagnosis can explicitly use production Square through `.env.local` with `SQUARE_FORCE_PRODUCTION=true`; Turborepo now forwards that variable to `dev`, while local development remains sandbox by default when the override is absent.
- 2026-07-22: Sandbox terminal testing uses Square's successful simulated Terminal device id and bypasses physical pairing/readiness checks; production mode alone intersects paired device codes and requires live hardware acknowledgement.
- 2026-07-22: Authenticated browser QA on `gndprodesk.localhost` loaded production Terminals 1451 and 2443 and created a $0.01 checkout on 2443. Square returned HTTP 200 for device lookup, checkout creation/status, and dismissal, but the physical terminal did not present the request. Follow-up same-app `PING` actions to both 1451 and 2443 remained `PENDING` with no device metadata, proving the hardware was not acknowledging GND Connected mode. The guarded UI now reports this exact condition and no longer creates a checkout while the terminal is in that state. Physical sign-in to GND Terminal API mode and a repeated payment test remain required.
...
- Direct staff payments capture the submitted sale ids and selected print mode before payment state can reset. After confirmed payment success, the Make Payment surface enters `Preparing to print` and awaits the shared hidden same-page viewer; no placeholder or document tab is opened. Invoice plus packing slip remains one combined `order-packing` document. Print readiness starts the success close timer, while preparation failure preserves the successful payment and exposes print-only `Retry print` and `Close` actions.
- The staff payment method control is adaptive and remains immediately after the compact payment-date control. Check renders one grouped method trigger plus check-number input; invalid Check state uses the red group border without a visible error label while retaining assistive text. Terminal opens a nested, availability-aware device menu and returns to the normal trigger labeled with the selected device. The date trigger, date clear action, method control, and Apply action share the standard 36px control height. Non-terminal selection clears terminal identity/session state, and submission sends check data only for Check and device/session data only for Terminal.
- The compact payment-date control is available only to an active `Super Admin`. Other staff see the payment-method control use the full reclaimed row width and record the implicit current New York business date. The protected payment mutation independently rejects a non-null manual date from any other role, so hiding the control is not the authorization boundary.
- Make Payment options use a compact single-column flat list with divider-separated rows. Each checkbox is followed by an inline title, em dash, and short description; individual option cards and borders are intentionally omitted, including the payment-link-only `Payment already received` row. The Wallet control follows the same flat inline treatment as `Use wallet balance — Apply up to <amount>.`, while its section header retains the available balance.
- The standalone Make Payment dialog opts out of the shared `max-w-lg` cap so its intended 560px responsive width is available to the date and adaptive payment-method controls. Its sticky footer spans both 24px content gutters without a right-edge gap, stacks at narrow widths, and uses the compact accessible `Apply` plus right-chevron action; the side-sheet footer contract remains unchanged.
- Authenticated development QA verified post-payment printing from both staff entry points. A `$0.01` Credit Card payment on `09376AD` through the standalone modal and a `$0.01` Credit Card payment on `09360PC` through Sales Overview's secondary payment sheet each used Invoice plus Packing Slip, entered `Preparing to print`, resolved print access, mounted the hidden viewer, loaded the PDF iframe, and logged `print-dialog-called` without opening another browser tab. The order balances moved from `$82.04` to `$82.03` and `$112.66` to `$112.65`, respectively.
