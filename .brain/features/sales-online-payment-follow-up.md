...
- First runtime slice implemented for the `apps/dashboard` notification bell: unread count badge, lazy inbox/archive dropdown, notification type filter, mark-unread-as-read on open, single notification archive, archive all, load-more controls, recipient-scoped status mutations, per-user channel delivery preferences, and domain-specific notification row icons/actions.
...
- `apps/dashboard/src/components/notification-center/notification-center.tsx` now opens as a counted actionable inbox, marks unseen notifications as read on open, lazy-loads archive only when the archive tab is active, and exposes type filtering, archive-all, and load-more controls.
- `apps/dashboard/src/components/notification-center/notification-settings-sheet.tsx` opens from the bell dropdown settings button and lets the logged-in user toggle Email, In-App, and WhatsApp preferences for channels they can access.
- `apps/dashboard/src/hooks/use-notifications.ts` now exposes exact unread count, type summaries, mark-seen, archive-one, archive-all, and update-pending state on top of protected current-user notes queries.
