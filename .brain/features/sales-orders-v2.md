...
- Saved page-tab counts render through the shared `AnimatedNumber` component in
  the inline tab bar, overflow menu, and Edit Tabs dialog, so query-count
  refreshes transition instead of snapping between values.
...
- Saved filter tabs are enabled on `/sales-book/orders` through the shared `pageTabs` surface. Saving preserves active filters and `sort`, so the explicit `Payment Review = Needs review` filter can be saved as a reusable tab such as `Payment Review`; count badges use the same distinct-order `SalesPayments.reviewStatus = "needs_review"` grouping as the table when `paymentReview=needs_review`. The generated `All` tab counts toward adaptive placement: one or two resolved tabs render after search, while three or more render in their own full-width row above search, query badges, and right-aligned header actions. The save `+` action stays inside the tab group, the entire strip stays hidden when there are no saved tabs and no active saveable filter/search/sort query, and Edit remains the single saved-tab management control. The visible tab row does not show a General/Public badge; visibility is managed inside the edit dialog. Sales list, quote list, and payment-review mutation refreshes now call the typed page-tabs invalidation registry so `/sales-book/orders` saved-tab counts refresh even when the triggering activity happens from another page.
...
- The adaptive Orders saved-tab row shows at most three tabs on mobile, five from `lg`, and eight from `2xl`. Remaining tabs are available from a `+N` dropdown immediately before Edit with title, default marker, and record count. Selecting an overflow tab promotes it into the final visible slot and moves the displaced tab into the dropdown without changing persisted order. Other saved-tab pages retain the shared three/five default.
...
- 2026-07-29 date cutoff labels:
  ...
  - The dashboard typecheck remains on its existing unrelated baseline. The full repository suite completed with 2,422 passing, one skipped, and 32 unrelated baseline failures; the focused date-filter suite passed within that run.
...
- 2026-07-22 batch payment review:
  ...
  - Authorized local browser QA selected orders `08897CST` and `08894LM` together and invoked `Reviewed` once. Both latest payments were updated by the same batch at `2026-07-22 17:19:39` with the shared batch note, proving the UI reached the dedicated multi-order mutation. The exact pre-QA fields were restored afterward, the queue returned to three eligible payments, and the temporary auth token/session were removed.
  - The post-action DOM assertion could not be captured because the shared Next process stopped returning response bytes after the mutation and logged `write EIO`; a separate temporary Next instance also compiled but stalled serving requests. The deterministic client test remains the no-reload proof at the orchestration boundary: it awaits the single invalidation before clearing selection and closing the menu.
