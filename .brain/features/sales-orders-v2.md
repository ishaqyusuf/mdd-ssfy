...
- The legacy sales form Shelf Items, HPT, take-off HPT, Moulding, and Service desktop grids now use domain-local table-core modules instead of embedded raw `@gnd/ui/table` / `table-sm` tables: `components/tables-2/sales-form-shelf-items/*` for shelf section/product rows, `components/tables-2/sales-form-hpt-lines/*` for HPT door-size rows, `components/tables-2/sales-form-takeoff-hpt-lines/*` for the take-off size/swing/quantity/labor/estimate row, `components/tables-2/sales-form-moulding-lines/*` for moulding rows, and `components/tables-2/sales-form-service-lines/*` for service rows. The clean-code sales-form service step, moulding step, and door-size select modal also use `components/tables-2/clean-code-sales-form-service-lines/*`, `components/tables-2/clean-code-sales-form-moulding-lines/*`, and `components/tables-2/clean-code-door-size-select-lines/*` instead of raw desktop tables. The mobile card paths and existing form context bindings, category/product comboboxes, supplier badge, swap-door path, swing selector, handle/no-handle quantity modes, quantity inputs, pricing/edit controls, labor popovers, animated totals, delete confirmations, Add Product/Add Door Size/Add Moulding/Add Line controls, HPT note toggle/panels, and old service/moulding recalculation behavior are preserved.
