...
The `/community/projects` route now renders through `apps/dashboard/src/components/tables-2/community-projects/*` while preserving the existing community projects query, filter params, analytics cards, project modal, supervisor editor, addon editor, archive actions, and overview/unit navigation.
...
- The old `apps/dashboard/src/components/tables/community-project/*` files were removed on 2026-07-17 after import scans confirmed there were no live source consumers outside Brain notes and negative audit assertions.

## Validation
- `bun test apps/dashboard/src/components/tables-2/community-projects/migration-parity.test.ts` passed with 4 tests / 33 assertions.
...
- Filtered `@gnd/dashboard` typecheck reported no touched-file diagnostics for the Community Projects route/header/table/settings files while the full command still exits nonzero from unrelated baseline issues.
