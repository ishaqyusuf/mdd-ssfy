...
The `/contractors/jobs/payments` payout history surface now renders through `apps/dashboard/src/components/tables-2/contractor-payouts/*` while preserving the existing `jobs.contractorPayouts` query, filter params, payout search metadata, detail links, and selected payout print-report workflow.

The `/contractors/jobs/payment-dashboard` recent payments panel now renders through `apps/dashboard/src/components/tables-2/payment-dashboard-recent-payments/*`, and the same dashboard's Ready for payout contractor queue now renders through `apps/dashboard/src/components/tables-2/payment-dashboard-contractors/*`, while preserving the existing `jobs.paymentDashboard` query and the finance dashboard cards/checklist.

The `/contractors/jobs/payment-portal` payable jobs workspace now renders its selected-contractor job list through `apps/dashboard/src/components/tables-2/payment-portal-jobs/*` while preserving the existing `jobs.paymentDashboard`, `jobs.paymentPortal`, `jobs.jobReview`, `jobs.createPaymentPortal`, job overview, print-selected, and payout summary flows.

The `/contractors/jobs/payments/[paymentId]` payout detail page now renders its Included jobs section through `apps/dashboard/src/components/tables-2/contractor-payout-overview-jobs/*` while preserving the existing `jobs.contractorPayoutOverview` query, payout cancel/reverse actions, print report action, summary cards, adjustments, and activity history.
...
- The old `apps/dashboard/src/components/tables/contractor-payouts/*` files were removed on 2026-07-17 after import scans confirmed there were no live source consumers outside Brain notes and negative audit assertions.
...
## Validation
- `bun test apps/dashboard/src/components/tables-2/contractor-payouts/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 7 tests / 38 assertions.
...
- Filtered `@gnd/dashboard` typecheck reported no touched-file diagnostics; broad typecheck still inherits unrelated baseline errors.
...
- 2026-07-17 dashboard slice:
  ...
  - Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the dashboard/table registry/audit files while broad typecheck remains blocked by unrelated baseline errors.
...
- 2026-07-17 payment portal jobs slice:
  ...
  - Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the payment-portal/table registry/audit files while broad typecheck remains blocked by unrelated baseline errors.
...
- 2026-07-17 payout detail included-jobs slice:
  ...
  - Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the payout detail/table registry/audit files while broad typecheck remains blocked by unrelated baseline errors.
