...
## Current Behavior
- The route uses the restarted Sales Orders table standard through `apps/dashboard/src/components/tables-2/inventory-allocations/*`.
...
## Validation
- `bun test apps/dashboard/src/components/tables-2/*/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 75 tests / 682 assertions.
...
- Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the touched allocation table, route, and registry files while broad typecheck remains subject to existing unrelated baseline errors.
