...
- In the Inventory Needs tab, the primary action is labeled `Mark as available`. Clicking it opens the existing inbound form both inline and as a secondary `InboundCreatePane`, locks the displayed status to `Available`, and preselects every eligible item at its current eligible quantity. A Mark / unmark all checkbox and per-item controls let the operator refine that selection before saving. The client sends `operation="mark_available"` separately from the real shipment status. The server creates a normal pending inbound, links only the selected quantity, and receives it through the canonical inbound receipt path so `InventoryStock`, stock movement/log evidence, demand receipt, component state, and backorder allocation use the same rules as ordinary receiving. A guarded inventory-package write sets the order prompt to `AVAILABLE` only when its predicate confirms no open inbound demand remains; partial or concurrently changed demand preserves the remaining need.
...
- `Mark as available` requires `editOrders`; ordinary create-inbound behavior retains its existing authenticated inbound boundary.
