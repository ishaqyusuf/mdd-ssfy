...
## Current Behavior
- The route uses the restarted Sales Orders table standard through `apps/dashboard/src/components/tables-2/inventory-variants/*`.
...
## Validation
- `bun test apps/dashboard/src/components/tables-2/*/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 78 tests / 698 assertions.
...
- Filtered `@gnd/dashboard` typecheck grep reported no diagnostics for the touched variants table, route, workspace, registry, and audit files while broad typecheck remains subject to existing unrelated baseline errors.
