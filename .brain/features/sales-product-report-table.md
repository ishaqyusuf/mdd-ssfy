...
- Table module: `apps/dashboard/src/components/tables-2/sales-statistics/*`
...
- Legacy files under `apps/dashboard/src/components/tables/sales-statistics/*` were removed after import scans found no remaining consumers.

## Validation
- 2026-07-17 density update: focused Sales Statistics parity test passed with 4 tests / 45 assertions; full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2523 assertions; focused Biome passed; touched-file typecheck scan produced no diagnostics; `git diff --check` passed for touched files; `git diff -- apps/dashboard/src/components/tables-2/core` stayed clean. Browser proof covered `/sales-book/top-selling-products` and `/product-report` on the direct local dev route, with `56px` rows, `45px` header, table-owned vertical/horizontal overflow at a 760px viewport (`scrollTop 0 -> 600`, `scrollLeft 0 -> 230`), header offset `0px -> 70px`, no document-level horizontal overflow, and screenshot evidence at `/private/tmp/gnd-sales-statistics-table.jpg`.
...
- Full `@gnd/dashboard` typecheck remains blocked by existing workspace baseline errors; filtered output had no diagnostics for this product-report slice.
