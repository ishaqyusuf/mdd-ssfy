...
- Implemented task monitor client simplification and the generic task-run diagnostics ledger.
  ...
  - Added `TaskRunDiagnostic` / `TaskRunDiagnosticStatus` Prisma schema and `Users` relations for actor/reviewer tracking.
  - Added protected `taskRunDiagnostics` tRPC routes for register, start-failure, finalize, list, detail, and mark-reviewed behavior.
  - Added server-side task start/start-failure logging from `triggerTask` and terminal finalization through `finalizeTaskRunDiagnosticAction`, which retrieves Trigger.dev run status server-side before upserting diagnostics.
  - Added Super Admin diagnostics UI at `/task-events/diagnostics`, linked from `/task-events`.
  - Validation: `bun run db:generate` passed; `bun --filter @gnd/db push:dev` passed; `bun test apps/api/src/db/queries/task-run-diagnostics.test.ts` passed; `bun test apps/api/src/trpc/routers/task-events.route.test.ts` passed; targeted `bunx biome check` over touched files passed; `bun --filter @gnd/db typecheck` passed.
  - Validation limitations: `bun run db:migrate --ui=tui` reached Prisma but failed with `P3014` because the hosted PlanetScale/Vitess target cannot create a shadow database; root `bun run db:push` does not exist; broad `bun run typecheck`, `bun --filter @gnd/api typecheck`, and `bun --filter @gnd/www typecheck` remain blocked by pre-existing unrelated repo diagnostics. Filtered API/WWW typecheck output showed no remaining touched-file diagnostics after fixes.
  - Brain files updated: `brain/features/background-task-monitor.md`, `brain/database/schema.md`, `brain/database/relationships.md`, `brain/database/migrations.md`, `brain/api/endpoints.md`, `brain/api/contracts.md`, `brain/api/permissions.md`, `brain/decisions/ADR-012-task-run-diagnostics-ledger.md`, `brain/tasks/roadmap.md`, `brain/tasks/done.md`, and `brain/progress.md`.

- Published the synthesized spec for task monitor client simplification and a task error ledger.
  - Created GitHub issue https://github.com/ishaqyusuf/mdd-ssfy/issues/42 with the `ready-for-agent` label.
  - Added `brain/plans/2026-07-13-spec-task-monitor-client-simplification-and-error-ledger.md` covering the production UX simplification, durable task-run diagnostics model, server-side finalization/reconciliation direction, Super Admin review surface, sales email ledger boundary, user stories, implementation decisions, testing seams, and out-of-scope boundaries.
  - Updated `.scratch/task-monitor-client-simplification/map.md`, `brain/features/background-task-monitor.md`, and `brain/tasks/roadmap.md` with the spec and issue pointers.
  - No runtime code, database schema, migration, API contract, permission constant, build, typecheck, or browser behavior changed in this spec publication pass.

- Created a local Wayfinder scratch map for simplifying the background task monitor.
  - Added `.scratch/task-monitor-client-simplification/map.md` to capture the destination, known code facts, user constraints, frontier tickets, fog, and out-of-scope boundaries for production task-monitor simplification plus a database-backed developer error ledger.
  - Updated `brain/features/background-task-monitor.md` with the planned direction: production hides developer-facing task details/counts, normal users get loading and closeable terminal success/destructive toasts, and developer/admin diagnostics move to durable persistence.
  - Added a roadmap entry in `brain/tasks/roadmap.md`.
  - No runtime code, database schema, migration, API contract, permission constant, build, typecheck, or browser behavior changed in this planning pass.

- Created a local-only spec for App Auto Update parity with al-ghurobaa.
  - Added `brain/plans/2026-07-12-spec-app-auto-update-parity.md` covering the requested Expo app auto-update behavior: installed-build launch checks, foreground checks with cooldown/config, full-screen Downloading/Updating/Restarting modal, automatic reload, failed-update continuation, Settings > App Updates diagnostics, and GND-specific restart interruption guardrails for sales/jobs/dispatch/warehouse workflows.
  - Added the task to `brain/tasks/roadmap.md` as a proposed local-only roadmap item with `ready-for-agent` triage vocabulary but no external issue publication.
  - No code, database schema, migration, API contract, permission, build, typecheck, or runtime behavior changed in this planning pass.
...
- Added visible user feedback for sales email background failures.
  ...
  - Updated the floating task monitor to mark runtime failures in the persisted task row, show one-time destructive failure toasts for recovered/background failures, read hard run exceptions from `run.error`, and link failed sales email rows to `/sales-book/emails`.
  ...
  - `bun --filter @gnd/www typecheck` remains blocked by pre-existing baseline diagnostics in API query code and shared UI React types; targeted checks for this feedback change passed.
...
- 2026-07-09: Followed up on invoice email failures from `/sales-book/orders`. Direct simple/composed sales document notification jobs now convert expected user-fixable document email validation errors into structured failed email output with a readable UI message instead of a raw Trigger failure. Sales document emails remain link-based by default, and PDF attachments are now guarded by `ATTACH_SALES_EMAIL_PDF=true` plus the existing simple-email `skipPdfAttachment: false` payload opt-in. Validation: focused task-feedback and sales PDF attachment tests passed; focused Biome passed; scoped `git diff --check` passed; filtered www/jobs/notifications typecheck output showed no touched-file diagnostics while broad package typecheck remains blocked by existing unrelated repo errors.
- 2026-07-09: Fixed direct invoice/quote email feedback getting stranded when a shared sales dropdown closes. `SalesMenuRoot` now owns the direct sales document email Trigger watcher and exposes it through context, while dropdown items only invoke the stable controller. `useTaskTrigger` now dismisses the active spinner toast on completion, failure, and unmount so “Sending email...” cannot linger after the watcher resolves. Validation: focused `SalesMenu`/task-trigger Biome passed, focused task-feedback tests passed, scoped `git diff --check` passed, and filtered `@gnd/www` typecheck output showed no touched-file diagnostics.
- 2026-07-09: Switched sales document email feedback to the bottom-right task monitor instead of toast notifications. Direct and composed sales email triggers now run silent but monitored, the task monitor auto-opens and expands a failed task row when an email/background task fails, and task rows can expand to show run id, status, task metadata, timestamps, and full error text. Validation: focused task monitor/email UI Biome passed, focused task-feedback tests passed, scoped `git diff --check` passed, and filtered `@gnd/www` typecheck output showed no touched-file diagnostics.
