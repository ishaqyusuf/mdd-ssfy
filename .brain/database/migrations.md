...
- Production sync destinations are explicit through one command: `db:sync` defaults to `--from-prod --to-local`, while `--to-remote` selects and authorizes a remote-development write and `--to-prod` is rejected. Put `--dry-run` and other synchronizer options after `--`. Source-equals-target is always refused, local targets remain local-only, and cursor state is stored separately by destination mode.
- PlanetScale production and remote-development branches may share the same host and database path. In `remote-dev` mode, the sync safety check treats distinct branch-scoped usernames as distinct targets; the same branch username is still refused even if its password differs. Local-mode targets retain the stricter host/database identity check.
