...
- `db:generate`, `db:migrate`, `db:pull`, `db:push`, and `db:studio` use `local-infra-kit/bin/db.ts` with the `gnd` profile. Local development is the default; each accepts only `--local`, `--preview`, or `--prod`, with tool arguments after `--`.
...
- `db:sync` uses `local-infra-kit/bin/db-sync.ts`. It defaults to `--from-prod --to-local`; `--to-preview` explicitly selects preview, and `--to-prod` is always rejected. `db:sync --help` prints the public contract without resolving database endpoints or starting services. Options such as `--dry-run`, `--table`, and `--reset-cursor` follow `--`.
...
- Dev infra uses the shared `../../local-infra-kit` GND profile and publishes `DATABASE_URL` as the application database URL. Local values live in `.env.local`, preview overrides live in ignored `.env.preview`, and production values live in `.env.production`. Redis is independently selectable with `--redis-local` or `--redis-preview`.
