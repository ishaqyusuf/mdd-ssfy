...
- The shared router loads `.env` plus exactly one root profile from `.env.local`, `.env.dev`, `.env.preview`, or `.env.production`. It pins the selected `DATABASE_URL`, permits local or hosted non-production targets, refuses the production database identity outside production mode, prints a credential-free target fingerprint, and requires production confirmation. Production migration maps to Prisma migrate deploy; generation does not start services.
- `db:sync` uses `local-infra-kit/bin/db-sync.ts`. It defaults to `--from-prod --to-local`; `--to-preview` explicitly selects preview, `--from-local --to-preview` publishes local data, and `--to-prod` is always rejected. `db:sync --help` prints the public contract without resolving database endpoints or starting services.
...
- Local development uses explicit DB/Redis profiles through the shared `../../local-infra-kit` GND profile. `bun run dev` and `bun run dev --local` default to local Docker MySQL only; `bun run dev --preview` uses the hosted preview DB only; `bun run dev --prod` runs the production-env www smoke flow on port 3015. Redis is opt-in with `--redis-local` or `--redis-preview`. Dev profiles accept `--filter`, `--f`, `-f`, or `-filter` selectors using Turbo selector syntax directly. Exact package filters are validated against workspace package names; graph/glob/path/range selectors pass through to Turbo.
...
- The Compose file retains the explicit project name `www` after its 2026-07-28
  move from `apps/www` to `apps/dashboard`. This preserves ownership of the
  existing `gnd-mysql` container and the populated `www_gnd_mysql_data` volume;
  removing or renaming that project identity would make Compose select a new,
  empty `dashboard_gnd_mysql_data` volume.
...
- Dev infra uses the shared `../../local-infra-kit` GND profile and publishes `DATABASE_URL` as the application database URL. Shared defaults live in `.env`; local, hosted development, preview, and production database values live in `.env.local`, `.env.dev`, `.env.preview`, and `.env.production` respectively. Exactly one profile is selected and Redis is independently selectable with `--redis-local` or `--redis-preview`.
- Production sync destinations are explicit through one command: `db:sync` defaults to `--from-prod --to-local`, while `--to-preview` selects and authorizes a preview write and `--to-prod` is rejected. Put `--dry-run` and other synchronizer options after `--`. Source-equals-target is always refused, local targets remain local-only, and cursor state is stored separately by destination mode.
- PlanetScale production and preview branches may share the same host and database path. In `preview` mode, the sync safety check treats distinct branch-scoped usernames as distinct targets; the same branch username is still refused even if its password differs. Local-mode targets retain the stricter host/database identity check.
...
- 2026-07-08 web bug reporting schema addition:
  ...
  - Docker recovery attempt on 2026-07-08: `docker compose -f apps/dashboard/docker-compose.yml restart mysql` hung and was cancelled, Docker Desktop was quit and reopened, `docker desktop start` reported Docker Desktop already running, but `docker info` still could not connect to `unix:///Users/M1PRO/.docker/run/docker.sock`.
