...
## Goal
Remove only the Super Admin app-download settings page from `apps/dashboard`.
...
1. Remove the settings page route and UI:
   - Delete `apps/dashboard/src/app/(sidebar)/settings/app-download/page.tsx`.
   - Delete `apps/dashboard/src/components/settings/app-download-settings-page.tsx`.

2. Remove Settings navigation only:
   - Remove the `Settings > App Download` link from `apps/dashboard/src/components/sidebar-links.ts`.
...
## Validation
- `rg -n "\"/settings/app-download\"|'\\/settings\\/app-download'|settings/app-download/page|AppDownloadSettingsPage|app-download-settings-page" apps/dashboard/src -S`
- `rg -n "support/mobile-app|href=\"/api/download-app\"|AppDownloadSupportPage" apps/dashboard/src -S`
- `bunx biome check apps/dashboard/src/components/sidebar-links.ts`
