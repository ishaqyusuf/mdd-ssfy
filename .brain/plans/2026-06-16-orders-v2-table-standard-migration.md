...
## Objective
Migrate route-level table pages in `apps/dashboard` to the current `components/tables-2/sales-orders` standard while preserving existing page routes, page queries, filter params, filter metadata, and header components. This is a UI/table-module migration, not a query-contract or `/v2` route migration.

## Assumptions
- The latest table standard is `apps/dashboard/src/components/tables-2/sales-orders/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` is frozen for this migration. Do not modify the core folder in any way.
- Scope starts with route-level list/table pages in `apps/dashboard`. Print/PDF document tables, small static tables inside forms/modals, and one-off report layouts are not forced into the table standard unless they use the legacy shared table systems or become route-level operational lists.
- Cleanup happens only after import scans, route parity, and validation prove the old code is unused.

## Current Audit Highlights
- Current standard:
  - Canonical route: `apps/dashboard/src/app/(sidebar)/(sales)/sales-book/orders/page.tsx`
  - Compatibility redirect: `apps/dashboard/src/app/(sidebar)/(sales)/sales-book/orders/v2/page.tsx`
  - Table: `apps/dashboard/src/components/tables-2/sales-orders/*`
  - Core primitives, used as-is: `apps/dashboard/src/components/tables-2/core/*`
  - Settings/config, extend only if needed by a migrated table: `apps/dashboard/src/utils/table-settings.ts`, `apps/dashboard/src/utils/table-configs.ts`, `apps/dashboard/src/hooks/use-table-settings.ts`
...
- Current migration boundaries:
  - Add only new domain table folders under `apps/dashboard/src/components/tables-2/<domain>/*`.
  ...
  - The older `apps/dashboard/src/components/tables/sales-orders-v2/*` directory was unused by imports and has been removed.
  - `apps/dashboard/src/components/tables/.DS_Store` was a tracked filesystem artifact and has been removed.
- Legacy table generations still active:
  - `@gnd/ui/data-table` wrappers used by most `apps/dashboard/src/components/tables/*` tables.
  - `apps/dashboard/src/components/(clean-code)/data-table/*`, mostly as legacy cell/search-param support.
  - `apps/dashboard/src/components/_v1/data-table/*`, `apps/dashboard/src/components/common/data-table/*`, and `apps/dashboard/src/app/_components/data-table/*` still have live or `app-deps` consumers.
...
2. Keep `apps/dashboard/src/components/tables-2/sales-orders/*` as the canonical implementation.
3. Confirm `components/tables/sales-orders-v2/*` has no runtime imports.
4. Delete only after confirmation:
   - `apps/dashboard/src/components/tables/sales-orders-v2/columns.tsx`
   - `apps/dashboard/src/components/tables/sales-orders-v2/data-table.tsx`
5. Remove accidental filesystem artifacts such as `apps/dashboard/src/components/tables/.DS_Store`.
6. Keep `/sales-book/orders/v2` as a redirect to `/sales-book/orders` that preserves query params.
7. Validation.
   - `rg "tables/sales-orders-v2|components/tables/sales-orders-v2" apps/dashboard/src packages apps/api/src`
...
- Focused Biome check passed for:
  ...
  - `apps/dashboard/src/app/(sidebar)/(sales)/sales-book/orders/v2/page.tsx`
  - `apps/dashboard/src/components/sales-orders-v2-header.tsx`
  - `apps/dashboard/src/components/sales-tabs.tsx`
- `bun test apps/dashboard/src/components/sidebar-links.test.ts` passed.
...
- Known baseline validation blockers outside this route normalization:
  - full `@gnd/dashboard` typecheck still reports broad pre-existing workspace errors; filtered output for touched route/table/header files did not mention the migrated orders route or table header.
  - `apps/dashboard/src/lib/routing/redirect-engine.test.ts` currently expects `/production/dashboard`, while the app redirects to `/production/dashboard/v2`; this production-route expectation predates the orders route cleanup.
...
2. Add `apps/dashboard/src/components/tables-2/<domain>/*`.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/sales-quotes/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched quotes route/table/header/config files; the full `@gnd/dashboard` gate remains subject to existing workspace baseline errors.
  - import scan confirmed the canonical quotes route no longer imports `components/tables/sales-quotes`.
  - `git status --short apps/dashboard/src/components/tables-2/core` and `git diff -- apps/dashboard/src/components/tables-2/core` were clean.
...
- Validation:
  - focused Biome check passed for `apps/api/src/db/queries/sales-orders-v2.ts`, `apps/dashboard/src/components/tables-2/sales-orders/data-table.tsx`, and `/sales-book/orders/bin`.
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched orders route/table/header files.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/shelf-items/*`.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/sales-email-ledger/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched sales-email-ledger/table registry/audit files while the broad typecheck remains subject to existing workspace baseline errors.
  ...
  - `git diff --check` passed and `components/tables-2/core` has no diff.
  - 2026-07-17 density validation: focused sales-email-ledger migration-parity tests passed with 3 tests / 39 assertions; full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2527 assertions; focused Biome passed for sales-email-ledger columns, migration-parity test, and `table-configs`; touched-file typecheck scan produced no diagnostics; touched-file `git diff --check` passed; `components/tables-2/core` stayed untouched; HTTP route smoke returned `200` for `/sales-book/emails`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched quote-bin/quotes table files.
...
- Removed the P.O column from `apps/dashboard/src/components/tables-2/sales-quotes/columns.tsx` for the actual quotes table pages.
- Removed the leftover P.O column and mobile PO badge from the then-live legacy `apps/dashboard/src/components/tables/sales-quotes/columns.tsx` module used by the sales-rep quote embed, without changing quote query/filter contracts. The 2026-07-16 sales-rep embed migration later moved that embed to `tables-2/sales-quotes`.
...
- Added `apps/dashboard/src/components/tables-2/sales-quotes/sort.ts` to normalize quote sort params to existing query-safe fields and removed the derived `displayName` customer sort from the quote table config.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched quote files.
...
- Validation:
  ...
  - full `apps/dashboard/src/components/tables-2` suite passed with 289 tests / 2343 assertions.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/customers/*`.
...
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/customers/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/customers/columns.tsx`
  - removed the now-unreferenced `apps/dashboard/src/components/customer-v2/customer-directory-v2-page.tsx`
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched customers route/table/header/config files; the full gate remains subject to existing workspace baseline errors.
  - import scans found no remaining references to `components/tables/customers` or `CustomerDirectoryV2Page`.
  - `git diff --check` passed and `git diff -- apps/dashboard/src/components/tables-2/core` was clean.
...
- Validation:
  ...
  - full `apps/dashboard/src/components/tables-2` suite passed with 289 tests / 2338 assertions.
...
- Validation:
  ...
  - full `apps/dashboard/src/components/tables-2` suite passed with 289 tests / 2340 assertions.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/sales-dispatch/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/sales-dispatch/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/sales-dispatch/columns.tsx`
  - removed `apps/dashboard/src/components/tables/sales-dispatch/batch-actions.tsx`
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched dispatch route/table/header/config files; the full gate remains subject to existing workspace baseline errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inbound-management/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/inbound-managment/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/inbound-managment/columns.tsx`
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched inbound route/table/header/hook/config files; the full gate remains subject to existing workspace baseline errors.
  - import scans found no remaining references to `components/tables/inbound-managment`, `tables/inbound-managment`, or `inbound-managment` in `apps/dashboard/src`.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/sales-accounting/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/sales-accounting/data-table.tsx`
  - kept `apps/dashboard/src/components/tables/sales-accounting/columns.tsx` because the customer transaction subtable still imports it.
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep finished with no diagnostics for touched accounting route/table/header/hook/config files; the full gate remains subject to existing workspace baseline errors.
  ...
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/sales-statistics/*`.
...
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/sales-statistics/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/sales-statistics/columns.tsx`
  - removed `apps/dashboard/src/components/tables/sales-statistics/column.invoice.tsx`
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for `sales-statistics`, `product-report`, `ProductReport`, `table-settings`, or `table-configs`.
...
- Validation:
  ...
  - full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2523 assertions.
  - focused Biome passed and touched-file typecheck scan produced no diagnostics.
  - `git diff --check` passed for touched files.
  - `git diff -- apps/dashboard/src/components/tables-2/core` stayed clean.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/sales-production/*`.
- Updated `apps/dashboard/src/components/production-workspace.tsx` to import the new table directly, expose `SalesProductionColumnVisibility`, pass `initialTableSettings`, and render the queue table flush in the card content for compact table density.
...
- Validation:
  ...
  - broad `@gnd/dashboard` typecheck remains blocked by existing unrelated API/UI baseline errors, but the touched-file typecheck filter reported no diagnostics for the production workspace/routes/table/config/test files.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics in the touched production v2 route/table/workspace files; broad typecheck remains blocked by unrelated baseline errors.
  - HTTP/HTTPS smokes returned `200` for `/production/dashboard/v2` and `/sales-book/productions/v2` after route warmup.
  - `git diff --check` passed and `apps/dashboard/src/components/tables-2/core` diff stayed clean.

Phase 3 packing-list evidence captured on 2026-07-17:
- Added `apps/dashboard/src/components/tables-2/packing-list/*` for `/sales/packing-list`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics in packing-list, table registry, or audit files; broad typecheck remains blocked by unrelated baseline errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-products/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/inventory-products/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/inventory-products/columns.tsx`
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched inventory route/table/header/settings/config files.
  - import scans found no remaining references to `components/tables/inventory-products` or `tables/inventory-products`.
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the inventory slice.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-categories/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/inventory-categories/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/inventory-categories/columns.tsx`
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched category route/table/header/settings/config files.
  - import scans found no remaining references to `components/tables/inventory-categories` or `tables/inventory-categories`.
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the category slice.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-import/*`.
...
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/inventory-import/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/inventory-import/columns.tsx`
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched imports route/table/control-center/settings/config files.
  - import scans found no remaining references to `components/tables/inventory-import` or `tables/inventory-import`.
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the imports slice.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-variants/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched variants files while the broad typecheck remains subject to existing unrelated baseline errors.
  - `git diff --check` passed.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-allocations/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched allocation files while the broad typecheck remains subject to existing unrelated baseline errors.
  - `git diff --check` passed.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-inbounds/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched inbounds files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-backorders/*`.
...
- Validation:
  ...
  - 2026-07-17 density validation: focused Backorders parity tests passed with 3 tests / 20 assertions; full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2547 assertions; focused Biome passed for Backorders columns, parity test, and `table-configs`; filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics; direct local HTTP route smoke returned `200`; the initial HTTPS proxy route smoke timed out with zero bytes after 25s; `git diff --check` and clean `components/tables-2/core` diff passed.
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched backorders files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-partial-shipments/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched partial-shipment files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-dispatch-mode/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched dispatch files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched production-plan files while the broad typecheck remains subject to existing unrelated baseline errors.
  ...
  - density follow-up validation passed with focused Production Plan parity tests (3 / 23), the full `apps/dashboard/src/components/tables-2` suite (306 / 2562), focused Biome, filtered typecheck grep with no Production Plan/table-config diagnostics, clean `components/tables-2/core` diff, and tracked `git diff --check`.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-kind-review/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched review files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched stocks files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched supplier files while the broad typecheck remains subject to existing unrelated baseline errors.
  - `git diff --check` passed and `apps/dashboard/src/components/tables-2/core` diff stayed clean.

Phase 4 inventory item dashboard evidence captured on 2026-07-17:
- Added the domain table module under `apps/dashboard/src/components/tables-2/inventory-item-dashboard/*`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no diagnostics for touched inventory item dashboard files while the broad typecheck remains subject to existing unrelated baseline errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/community-builders/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/builder/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/builder/columns.tsx`
- Validation:
  - focused Biome check passed for the builders route, header, table settings/config, and new community-builders `tables-2` files.
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched builders route/table/header/settings/config files.
  - import scans found no remaining runtime references to `components/tables/builder` or `tables/builder`.
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the builder slice.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/community-templates/*`.
...
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/community-template/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/community-template/columns.tsx`
  - removed `apps/dashboard/src/components/community-template-search-filter.tsx`
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched templates route/table/header/settings/config files or the retargeted legacy install-cost modal row type.
  ...
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the templates slice.
...
- Validation:
  ...
  - broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline errors, but a filtered log scan found no diagnostics for install-cost route/component/table/settings/config touched paths.
...
- Added `apps/dashboard/src/components/tables-2/community-model-cost-form-tasks/*` for the desktop task pricing grid inside the existing Community Template model-cost modal.
- Updated `apps/dashboard/src/components/forms/community-model-cost-form.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` / `table-sm` markup.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 225 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/community-install-cost-form-tasks/*` for the desktop task quantity grid inside the existing Community Template install-cost modal.
- Updated `apps/dashboard/src/components/forms/community-install-cost-form.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` / `table-sm` markup.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 229 tests / 2337 assertions.
...
- Added `apps/dashboard/src/components/tables-2/builder-form-tasks/*` for the editable builder task grid inside the existing Community Builders modal.
- Updated `apps/dashboard/src/components/builder-form.tsx` to render the domain-local table-core module instead of the embedded `@gnd/ui/namespace` table markup.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 233 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/inventory-product-form-variants/*` for the product-form variant grid inside the existing inventory product form.
- Updated `apps/dashboard/src/components/forms/inventory-products/product-variants.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` markup, manual page/window sticky header state, `IntersectionObserver`, and Show More pagination.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 237 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/inventory-product-form-sub-components/*` for the Door Builder Components grid inside the existing inventory product form.
- Updated `apps/dashboard/src/components/forms/inventory-products/category-sub-components-section.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` / `table-sm` markup and the incomplete row-sortable wrapper.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 241 tests / 2337 assertions.
...
- Added `apps/dashboard/src/components/tables-2/role-form-permissions/*` for the role create/edit permission grid inside the existing roles/profile sheet form.
- Updated `apps/dashboard/src/components/forms/role-form.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` markup and old `Table*` cells while preserving role name, submit/cancel behavior, role form reset, loading skeletons, and existing `createRoleAction` payload shape.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 245 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/context/action/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics after narrowing existing role-form-family type issues.
...
- Added `apps/dashboard/src/components/tables-2/employee-form-permissions/*` for the employee-specific permission override grid inside the existing employee create/edit modal.
- Updated `apps/dashboard/src/components/modals/employee-form-modal.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` markup and old `Table*` cells while preserving employee defaults, role/profile/office fields, save mutation, employee-list invalidation, and the selected `permissionIds` array contract.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 249 tests / 2337 assertions.
  - focused Biome passed for the touched modal/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics; broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline API/UI errors.
  - HTTPS proxy smoke returned `200` for `/hrm/employees/v2`.
...
- Added `apps/dashboard/src/components/tables-2/job-scope/*` for the non-custom task scope table inside the existing Job Overview modal.
- Updated `apps/dashboard/src/components/modals/job-overview/job-scope.tsx` to render the domain-local table-core module instead of the `@gnd/ui/namespace` table while preserving job overview context usage, the scope description, custom-job explanatory state, and visible task title/rate/qty/maxQty/total values.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 253 tests / 2337 assertions.
  - focused Biome passed for the touched modal/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/new-job-install-tasks/*` for the install-cost task quantity grid inside the existing New Job modal.
- Updated `apps/dashboard/src/components/modals/new-job/install-tasks-list.tsx` to render the domain-local table-core module instead of embedded raw `<table>` markup while preserving builder-task gating, missing-task config fallback, task title/rate/max quantity data, `job.tasks.${index}.qty` form binding, admin-only max constraint, disabled zero-max inputs, quantity suffix, validation error display, and live total calculation.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 257 tests / 2337 assertions.
  - focused Biome passed for the touched modal/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics; broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline API/UI errors.
...
- Added `apps/dashboard/src/components/tables-2/sales-form-takeoff-hpt-lines/*` for the one-row take-off HPT size/swing/quantity/labor/estimate grid inside the existing legacy sales form take-off section.
- Updated `apps/dashboard/src/components/forms/sales-form/take-off/hpt-form.tsx` to render the domain-local table-core module instead of embedded raw `<table>` markup.
...
- Validation:
  ...
  - full `apps/dashboard/src/components/tables-2` suite passed with 289 tests / 2337 assertions.
...
- Added `apps/dashboard/src/components/tables-2/clean-code-door-size-select-lines/*` for the door-size option grid inside the clean-code sales form door-size select modal.
- Updated `apps/dashboard/src/app-deps/(clean-code)/(sales)/sales-book/(form)/_components/modals/door-size-select-modal/index.tsx` to render the domain-local table-core module instead of embedded raw `@gnd/ui/table` / `ScrollArea` markup.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 285 tests / 2337 assertions.
  - focused Biome passed for the touched modal/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/clean-code-sales-form-moulding-lines/*` for the moulding-line grid inside the clean-code sales form moulding step.
- Updated `apps/dashboard/src/app-deps/(clean-code)/(sales)/sales-book/(form)/_components/moulding-step/index.tsx` to render the domain-local table-core module on desktop instead of embedded raw `@gnd/ui/table` / `table-fixed` markup.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 281 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/clean-code-sales-form-service-lines/*` for the service-line grid inside the clean-code sales form service step.
- Updated `apps/dashboard/src/app-deps/(clean-code)/(sales)/sales-book/(form)/_components/service-step/index.tsx` to render the domain-local table-core module on desktop instead of embedded raw `@gnd/ui/table` / `table-fixed` markup, while preserving the existing mobile card path and Add Line action.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 277 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/sales-form-shelf-items/*` for the shelf item section/product grid inside the existing legacy Shelf Items sales form section.
- Updated `apps/dashboard/src/components/forms/sales-form/shelf-items.tsx` to render the domain-local table-core module on desktop instead of nested raw `@gnd/ui/table` / `table-sm` markup, while preserving the existing mobile card path and Item Section action.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 273 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics; broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline API/UI errors.
...
- Added `apps/dashboard/src/components/tables-2/sales-form-hpt-lines/*` for the HPT door-size line grid inside the existing HPT sales form section.
- Updated `apps/dashboard/src/components/forms/sales-form/hpt/hpt-section.tsx` to render the domain-local table-core module on desktop instead of embedded raw `@gnd/ui/table` / `table-sm` markup, while preserving the existing mobile card path, Add Door Size control, and door preview column.
- Updated `apps/dashboard/src/components/forms/sales-form/hpt/hpt-note.tsx` so desktop notes render as row-adjacent panels outside the old table row/colspan structure.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 269 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics; broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline API/UI errors.
...
- Added `apps/dashboard/src/components/tables-2/sales-form-moulding-lines/*` for the moulding line grid inside the existing Moulding/Service sales form section.
- Updated `apps/dashboard/src/components/forms/sales-form/moulding-and-service/moulding-content.tsx` to render the domain-local table-core module on desktop instead of embedded raw `@gnd/ui/table` / `table-sm` markup, while preserving the existing mobile card path and Add Moulding popover.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 265 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics; broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline API/UI errors.
...
- Added `apps/dashboard/src/components/tables-2/sales-form-service-lines/*` for the service line grid inside the existing Moulding/Service sales form section.
- Updated `apps/dashboard/src/components/forms/sales-form/moulding-and-service/service-content.tsx` to render the domain-local table-core module on desktop instead of embedded raw `@gnd/ui/table` / `table-sm` markup, while preserving the existing mobile card path.
...
- Validation:
  ...
  - full `bun test apps/dashboard/src/components/tables-2` passed with 261 tests / 2337 assertions.
  - focused Biome passed for the touched form/table/test/settings/config files.
  - touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics; broad `@gnd/dashboard` typecheck remains blocked by unrelated baseline API/UI errors.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/customer-service/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup completed after import scans:
  - removed `apps/dashboard/src/components/tables/customer-service/data-table.tsx`
  - removed `apps/dashboard/src/components/tables/customer-service/columns.tsx`
...
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched customer-services route/table/header/settings/config files.
  - import scans found no remaining runtime references to `components/tables/customer-service` or `tables/customer-service`.
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the customer-service slice.
...
- Validation:
  ...
  - full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2538 assertions.
...
- Added the domain table module under `apps/dashboard/src/components/tables-2/unit-invoices/*`.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Cleanup was initially deferred because `apps/dashboard/src/components/widgets/project-overview/index.tsx` imported `components/tables/unit-invoices/*` for its embeddable project overview tab. The 2026-07-16 Project Overview embed restart moved that tab to `tables-2/unit-invoices`, and the 2026-07-17 legacy cleanup removed the old unit-invoice table files after import scans found no live consumers.
- Browser validation exposed a shared `CustomModal` accessibility issue where the generated Radix `aria-labelledby` / `aria-describedby` ids could not resolve because custom ids overrode the generated title/description ids. `apps/dashboard/src/components/modals/custom-modal.tsx` now lets Radix own normal title/description ids and keeps the hidden accessible title/description plus portal placeholders for the `titleAsChild` / `descriptionAsChild` path.
- Validation:
  ...
  - full `@gnd/dashboard` typecheck still fails on existing workspace baseline errors, but filtered typecheck output had no diagnostics for the touched unit-invoices route/table/header/settings/config files or `CustomModal`.
  ...
  - `git diff -- apps/dashboard/src/components/tables-2/core` was clean and `git diff --check` passed for the unit-invoices slice.
...
- Updated `apps/dashboard/src/app/(sidebar)/community/(main)/unit-invoices/loading.tsx` to render `UnitInvoicesSkeleton` from `components/tables-2/unit-invoices` instead of the old `_v1` `DataTableLoading` wrapper.
- Deleted the now-unused `apps/dashboard/src/components/_v1/data-table/data-table-loading.tsx` wrapper after import scans showed it had no remaining live consumers.
...
- Added `apps/dashboard/src/components/tables-2/unit-invoice-form-tasks/*` for the desktop task grid inside the existing Unit Invoice edit modal.
- Updated `apps/dashboard/src/components/forms/unit-invoice-form.tsx` to render the domain-local table-core module instead of embedded `@gnd/ui/table` / `table-sm` markup on desktop, while leaving the existing mobile card task editor intact.
...
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.
- Validation:
  - `bun test apps/dashboard/src/components/tables-2` passed with 221 tests / 2327 assertions.
  - `bun test apps/dashboard/src/components/tables-2/unit-invoices/migration-parity.test.ts apps/dashboard/src/components/page-sticky-header.test.ts` passed with 9 tests / 43 assertions.
  - Focused Biome passed for the touched form/table/test/settings/config files.
  - Touched-file filtered `@gnd/dashboard` typecheck output showed no diagnostics.
  - HTTP smoke returned `200` for `/community/unit-invoices`.
  - `git diff --check` passed and `components/tables-2/core` has no diff.
...
- Updated `apps/dashboard/src/components/widgets/project-overview/index.tsx` so all operational tabs use restarted `tables-2` modules:
...
- Validation:
  ...
  - broad `@gnd/dashboard` typecheck remains blocked by existing unrelated baseline errors, but the touched-file typecheck filter reported no diagnostics for the overview widget or changed table files.
...
- Validation:
  ...
  - broad `@gnd/dashboard` typecheck remains blocked by existing unrelated baseline errors, but the touched-file typecheck filter reported no diagnostics.
...
- Added `apps/dashboard/src/components/tables-2/contractor-payouts/*` for `/contractors/jobs/payments`.
...
- Removed the unused legacy table folders after the corresponding routes and embeds had already moved to `tables-2`:
  ...
  - `apps/dashboard/src/components/tables/unit-invoices/*`
  - `apps/dashboard/src/components/tables/unit-productions/*`
  - `apps/dashboard/src/components/tables/contractor-jobs/*`
  - `apps/dashboard/src/components/tables/contractor-payouts/*`
- Import scans found no live source consumers outside Brain notes and negative audit assertion strings before deletion.
- The cleanup did not change `apps/dashboard/src/components/tables-2/core/*`, route query contracts, filter contracts, row actions, or table settings.

Final `apps/dashboard/src/components/tables` runtime cleanup evidence captured on 2026-07-17:
- Removed the remaining unused legacy files under `apps/dashboard/src/components/tables`, including old employees, roles/profile tabs, sales accounting transaction helper, sales orders, sales quotes, sales production, shared action-cell/header/row/index, and shared skeleton modules.
- `rg --files apps/dashboard/src/components/tables` now returns no files.
- Exact live-source import scans for `components/tables` across `apps/dashboard/src`, `packages`, and `apps/api/src` return no non-test consumers; only negative audit strings and historical Brain notes remain.
- Validation: `bun test apps/dashboard/src/components/page-sticky-header.test.ts` passed with 3 tests / 3 assertions, and `bun test apps/dashboard/src/components/tables-2` passed with 220 tests / 2314 assertions.
- Follow-up guard hardening on 2026-07-17 added an explicit empty-tree assertion for `apps/dashboard/src/components/tables` to `page-sticky-header.test.ts`, so reintroduced legacy table helper files fail the audit even if they do not import old table primitives. Validation passed with `bun test apps/dashboard/src/components/page-sticky-header.test.ts` at 5 tests / 7 assertions, focused Biome for the audit test, and `find apps/dashboard/src/components/tables -type f` returning no files.
- `apps/dashboard/src/components/tables-2/core/*` remained unchanged.

Live app table-system guard evidence captured on 2026-07-17:
- `apps/dashboard/src/components/page-sticky-header.test.ts` now includes a source-level audit that walks `apps/dashboard/src/app`, `apps/dashboard/src/components`, and `apps/dashboard/src/features` and fails if live surfaces outside `components/tables-2` / retired legacy internals reintroduce old table systems.
...
- Validation: `bun test apps/dashboard/src/components/page-sticky-header.test.ts` passed with 4 tests / 4 assertions; live-source table scans and live `TCell` scans returned no output; focused Biome passed for the audit test and `overview-tab.tsx`; `git diff --check` passed for touched files. Broad `@gnd/dashboard` typecheck remains blocked by pre-existing Sales Overview type debt.
...
- Removed isolated unused helper files from the retired table systems:
  - `apps/dashboard/src/components/(clean-code)/data-table/table-cells.tsx`
  - `apps/dashboard/src/components/(clean-code)/data-table/use-data-table.ts`
  - `apps/dashboard/src/components/(clean-code)/data-table/use-infinity-data-table.tsx`
  - `apps/dashboard/src/components/(clean-code)/data-table/use-table-compose.tsx`
  - `apps/dashboard/src/components/(clean-code)/data-table/data-table-column-header.tsx`
  - `apps/dashboard/src/components/(clean-code)/data-table/query-options.ts`
  - `apps/dashboard/src/components/(clean-code)/data-table/filter-command/filters.ts`
  - `apps/dashboard/src/components/_v1/data-table/data-table-row-actions.tsx`
  - `apps/dashboard/src/components/_v1/data-table/data-table-column-header.tsx`
- Preserved `apps/dashboard/src/components/(clean-code)/data-table/search-params.ts` and `apps/dashboard/src/components/(clean-code)/data-table/Dl.tsx` because they still have live non-table callers.
...
- `apps/dashboard/src/components/page-sticky-header.test.ts` now also allowlists only `Dl.tsx` and `search-params.ts` under the retired clean-code data-table directory, asserts `_v1/data-table` has no source files, and asserts the old `components/tables` runtime tree stays empty, so the deleted helper set cannot be reintroduced without failing the restarted table audit.
- Validation: `bun test apps/dashboard/src/components/page-sticky-header.test.ts` passed with 5 tests / 7 assertions after the `components/tables` guard; filtered `@gnd/dashboard` typecheck scan for the deleted module names returned no diagnostics; focused Biome passed for the audit test; `git diff --check` passed for the deleted files and docs; `apps/dashboard/src/components/tables-2/core/*` stayed unchanged.

Phase 5 site actions evidence captured on 2026-07-16:
- Added `apps/dashboard/src/components/tables-2/site-actions/*` for `/site-actions`.
...
- Added `apps/dashboard/src/components/tables-2/short-links/*` for `/settings/short-links`.
...
- Added `apps/dashboard/src/components/tables-2/task-run-diagnostics/*` for `/task-events/diagnostics`.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics while the broad typecheck remains subject to existing workspace baseline errors.
  - static scan found no live raw table/manual fetch usage in the diagnostics route wrapper.
  - `git diff --check` passed and `components/tables-2/core` has no diff.
  - 2026-07-17 density validation: focused task-run-diagnostics migration-parity tests passed with 3 tests / 37 assertions; full `apps/dashboard/src/components/tables-2` suite passed with 305 tests / 2528 assertions; focused Biome passed; touched-file typecheck scan produced no diagnostics; touched-file `git diff --check` passed; `components/tables-2/core` stayed untouched; the initial browser proof confirmed `56px` loading rows, a `45px` header, table-owned horizontal overflow/scroll (`scrollWidth 1294`, `clientWidth 1146`, `scrollLeft 0 -> 148`), and no document-level horizontal overflow while real-row proof waited on the then-missing local table.
...
- Added `apps/dashboard/src/components/tables-2/task-event-history/*` for the `/task-events/[eventName]` detail route history table.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics while broad typecheck remains subject to existing workspace baseline errors.
...
- Added `apps/dashboard/src/components/tables-2/task-events/*` for the `/task-events` dashboard list.
...
- Added `apps/dashboard/src/components/tables-2/document-approvals/*` for the `/hrm/document-approvals` approval queue.
...
- Added `apps/dashboard/src/components/tables-2/bug-reports/*` for the `/support/bug-reports` support issue board.
...
- Added `apps/dashboard/src/components/tables-2/master-password-logins/*` for `/settings/master-password-logins`.
...
- Added `apps/dashboard/src/components/tables-2/user-logged-in-devices/*` for `/settings/profile` Active Sessions.
...
- Added `apps/dashboard/src/components/tables-2/legacy-square-payment-orders/*` for the fallback `/square-payment/[emailToken]/[orderIds]` multi-order payment details list.
...
- Added `apps/dashboard/src/components/tables-2/customer-statement-report/*` for the Sales Report customer statement report list.
- Added `apps/dashboard/src/components/tables-2/customer-statement-lines/*` for the statement overview line selector.
- Updated `apps/dashboard/src/components/sales-report-menu.tsx` from inline `@gnd/ui/table` markup to the restarted table modules while preserving the existing `customers.getCustomerStatementReport` query, client search, report metrics, customer-open action, send statement flow, PDF download action, include-invoices toggle, and customer email update form.
...
- Added `apps/dashboard/src/components/tables-2/sales-rep-design-activity/*` for the `/sales-rep/design` Recent Activity panel.
- Updated `apps/dashboard/src/app/(sidebar)/(sales)/sales-rep/design/page.tsx` from inline `@gnd/ui/table` markup to the restarted table module while preserving the surrounding design dashboard cards, static sample activity rows, status badge meaning, and the existing filter/export controls.
...
- Added `apps/dashboard/src/components/tables-2/transaction-overview-applications/*` for transaction invoice applications.
- Added `apps/dashboard/src/components/tables-2/transaction-overview-payments/*` for transaction payment history rows.
- Updated `apps/dashboard/src/components/modals/transaction-overview-modal.tsx` from inline `@gnd/ui/table`, manual skeleton rows, `TCell`, and old payment-helper imports to the restarted table modules while preserving `getCustomerTransactionOverviewAction`, the payment summary, and sales overview row-open behavior.
- Updated `apps/dashboard/src/app/(clean-code)/layout.tsx` to hydrate `getInitialTableSettings("transaction-overview-applications" | "transaction-overview-payments")` and pass both settings into the global modal.
...
- Validation:
  ...
  - filtered `@gnd/dashboard` typecheck grep reported no touched-file diagnostics; a broader `employees/v2` grep still finds an unrelated existing `[id]` detail-page `EmployeeOverview` type mismatch.
...
- Validation:
  ...
  - touched-file filtered `@gnd/dashboard` typecheck grep reported no diagnostics while a broader customer-v2 grep still reports unrelated pre-existing `customer-overview-v2-content.tsx` diagnostics.
...
- `apps/dashboard/src/components/tables/sales-orders-v2/*` after confirming no runtime imports.
- `apps/dashboard/src/components/tables/.DS_Store`.
- `apps/dashboard/src/components/tables/sales-dispatch/*` was removed on 2026-06-16 after import scans confirmed no active consumers.
- `apps/dashboard/src/components/tables/inventory-products/*` was removed on 2026-06-16 after import scans confirmed no active consumers.
- `apps/dashboard/src/components/tables/inventory-categories/*` was removed on 2026-06-16 after import scans confirmed no active consumers.
- `apps/dashboard/src/components/tables/inventory-import/*` was removed on 2026-06-16 after import scans confirmed no active consumers.
- `apps/dashboard/src/app-deps/(sidebar)/(sales)/sales-book/(bebug)/debug-*/data-table-copy.tsx` if no route references exist and app-deps are confirmed archival.
...
- old table folders under `apps/dashboard/src/components/tables/<domain>` after their replacement route imports are live
- `apps/dashboard/src/components/tables/index.tsx`
- `apps/dashboard/src/components/tables/table-header.tsx`
- `apps/dashboard/src/components/tables/table-row.tsx`
- `apps/dashboard/src/components/tables/load-more.tsx`
- `packages/ui/src/components/custom/data-table/*`
- `apps/dashboard/src/components/(clean-code)/data-table/*`
- `apps/dashboard/src/components/_v1/data-table/*`
- `apps/dashboard/src/components/common/data-table/*`
- `apps/dashboard/src/app/_components/data-table/*`

Cleanup gate:
1. `rg` shows zero imports from the candidate path in `apps/dashboard/src`, `apps/api/src`, and `packages`.
...
11. Final old table-system cleanup. Status 2026-07-17: complete for the `apps/dashboard/src/components/tables` runtime folder; no files remain there after import scans and restarted table tests.
