...
- Inventory inbound and backorder release routes now include retry guardrails:
  ...
  - `inventories.inboundNeedsApplicationAttentionSummary`: lean count of Received shipments whose active linked Sales Needs still have unapplied shipment capacity
  - `inventories.inboundNeedsApplicationAttention`: modal detail query over the same canonical eligibility rule, capped at 100 rows and enriched with order/author activity context
  - `inventories.applyInboundNeedsApplicationAttention`: applies a unique batch of 1-100 selected Received inbounds to active linked Needs in one transaction, then reconciles affected Sales handoffs after commit
...
- Inventory reconciliation routes now include:
  ...
  - `inventories.resolveSalesInventoryMarkAsAvailabilityForContinue`: guarded safe-repair mutation that accepts the same positive-integer `salesOrderIds` batch contract, changes only confirmed unlinked/unreceived mutable demand, and requires a clean post-mutation preflight before continuation
  - `inventories.overrideSalesInventoryMarkAsAvailabilityForContinue`: `editOrders`-guarded status workflow waiver used by `Mark available and continue`; it confirms every current blocked order, preserves canonical inventory and inbound truth, writes one order-scoped override history row per blocker, and permits continuation only after complete audit evidence
  ...
  - `inventories.resolveSalesInventoryLegacyStatusSetup`: automatic per-order legacy inbound-status adaptation mutation for Sales Overview Inventory records whose shared compatibility state is `legacy_locked`, including partially synchronized orders with needs but no linked inbound; it accepts canonical `continue | clear` plus deprecated `override | reset` aliases, an optional exact `legacyStatus` baseline, synchronizes and materializes `ORDERED` / `PENDING ORDER` / `AVAILABLE` transactionally, creates supplier-less inbounds when supplier resolution is unavailable, returns structured migration evidence, and blocks unsupported or stale statuses without partial writes
  - `inventories.createInboundShipmentFromDemands`: creates an inbound shipment from selected open demand ids, quantity-scoped demand groups, and/or monitored stock component selections. Real shipment status remains `pending` / `in_progress`; the separate `mark_available` operation requires `editOrders`, splits partial demand safely, creates the shipment as pending, then receives it through the canonical stock/movement/log/demand/component path in the same transaction. Only orders with no remaining active inbound demand become `AVAILABLE`; partial selections retain their remaining need. Received components queue the standard backorder allocator. No feature-specific schema column or migration is used.
 ...
 - `inventories.resolveSalesInventoryOrderRepair`: protected guarded repair mutation. It accepts only the reviewed baselines, revalidates sale ownership and current row identity/status/quantities, cancels safe unlinked demand, releases safe allocations, recomputes affected components, and writes one order-scoped SalesHistory audit record.
  ...
  - `inventories.updateInboundShipmentStatus`: positive-integer guards `inboundId`, accepts an optional trimmed note up to 2,000 characters, updates the inbound lifecycle status with a non-deleted and previous-status guard, then writes a notification-backed activity event carrying the note and linked sales order numbers for Sales Overview visibility; selecting `completed` / Received requires `editInboundOrder` and transactionally applies guarded linked demand to material Needs without posting physical stock; cancelling an inbound releases unreceived active demand only through rows still linked to that cancelled parent shipment, recomputes affected components only after confirmed release evidence, and returns release/recompute counts
  - `inventories.updateInboundShipmentNeedsApplication`: `editInboundOrder` mutation for guarded backward-compatible `apply` / `unapply` of a positive-integer inbound id against material Needs; it records versioned actor-attributed Event snapshots and never reverses physical stock
  - `inventories.inboundShipmentDetail`: includes derived Needs application state and action eligibility alongside shipment/item/demand detail
