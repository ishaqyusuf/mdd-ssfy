...
Analysis and first cleanup slice for `apps/dashboard` dead, stale, and migration-era code.

## Commands Run

- `bun run knip --workspace apps/dashboard --include files,exports,dependencies,unlisted,unresolved --no-progress --reporter compact --no-exit-code`
- `bun run knip --workspace apps/dashboard --include files,exports,dependencies,unlisted,unresolved --no-progress --reporter json --no-exit-code > /tmp/gnd-www-knip.json`
...
- `apps/dashboard/src/app` route-convention files: 152.
- `apps/dashboard/src/app-deps` route-convention files at audit start: 21, all stale `loading.tsx` or `not-found.tsx` style files rather than live `page.tsx` routes.
- `apps/dashboard/src/app-deps` is not globally removable. Live code still imports auth helpers, old sales form stores, types, query utilities, and community helpers from `@/app-deps/...`.
...
- Deleted 643 high-confidence stale tracked files:
  - copied middleware/image loader: `apps/dashboard/src/middleware-copy.ts`, `apps/dashboard/image-loader.ts`
  - sandbox/debug/copy artifacts under `apps/dashboard/src/app-deps/sand-box`, `apps/dashboard/src/app-deps/(public)/debug`, `apps/dashboard/src/app-deps/(sidebar)/(sales)/sales-book/(bebug)`, and `apps/dashboard/src/app/(sidebar)/community/(main)/builders/page-copy.tsx`
  - all 21 stale `loading.tsx` route-convention files under `apps/dashboard/src/app-deps`
  - the placeholder `/sales-book/home-page` route that only rendered `HELLO`
  - the isolated `apps/dashboard/src/app-deps/(v1)/_actions/upgrade/*` action files, after removing the no-op `_fixHomeTaskDates()` call from `getHomesAction`
  - stale tracked `apps/dashboard/knip-report.txt` and `apps/dashboard/knip-vscode-report.txt` artifacts; the current report now lives in Brain
  - inert `apps/dashboard/src/app-deps` root convention duplicates: `global-error.tsx`, `not-found.tsx`, and `providers.tsx`
  - unreferenced `apps/dashboard/src/app-deps/_components/data-table` wrappers: `data-table-column-header.tsx`, `index.tsx`, and `toolbar.tsx`; live table cell/context helpers were retained because active common columns still import them
  - ignored backup route `apps/dashboard/src/app/(sidebar)/community/(main)/_page.tsx`
  ...
  - unused MDX/Tiptap prototypes, login mockups, demo outline table, disabled transactions widget, and orphan sales-rep recent-sales widget under `apps/dashboard/src/components`
  ...
  - orphan hooks: unused bin/fn/note/scroll/sidebar/table/status/filter/payment/email hooks under `apps/dashboard/src/hooks`, after removing the stale commented `useSyncStatus` reference from `use-task-trigger.ts`
  - old utility leaves: unreferenced watermark, auth-session, community home-template builder, compose-ref, safe-action, email-transform, option-builder, quick-print, and refresh helpers under `apps/dashboard/src/lib`, after removing the stale commented watermark effect from the home printer
  ...
  - unmounted dev-only sales `_backward-compat` repair menu island under `apps/dashboard/src/app/(clean-code)/(sales)/_backward-compat`, plus its now-orphan `actions/--fix/fix-customer-tax-profiles.ts` server action; the live `fix-undefined-order-id` action was retained because current edit-quote routing imports it
  ...
  - orphan clean-code export pair: `apps/dashboard/src/app/(clean-code)/_common/export/config.ts` and `type.ts`; the pair only referenced each other and had no external imports
  ...
  - orphan root dispatch utility: detached `apps/dashboard/src/utils/db/where.dispatch.ts`; current dispatch code imports its separate clean-code/app-deps dispatch query helpers instead
  - orphan app-local Trigger/email v3 chain: detached `apps/dashboard/src/lib/resend.ts`, app-local email footer/logo/template files, trigger constants/schema, and no-op `apps/dashboard/src/trigger/tasks/email/send-composed-email.tsx`; `@gnd/jobs` now owns the live `send-composed-email` task and `@gnd/email` owns the rendered template
  - orphan app-local Dyke step-component action pair: detached `apps/dashboard/src/actions/save-step-component.ts` and `apps/dashboard/src/actions/update-component-pricing-action.ts`, plus their now-unused schema exports in `apps/dashboard/src/actions/schema.ts`; the active save/pricing flow uses `@gnd/inventory` through inventories tRPC
  - orphan app-deps duplicate legacy step hook: detached `apps/dashboard/src/app-deps/(clean-code)/(sales)/sales-book/(form)/_hooks/legacy/use-dyke-form-step/index.tsx`; live app-deps modal/helper imports point to the app-side legacy step hook
...
- Removed the dead custom image-loader reference from `apps/dashboard/next.config.mjs`.
- Removed 15 high-confidence stale `@gnd/dashboard` package declarations after exact import/config scans and refreshed the Bun lockfile:
...
- Removed 35 more high-confidence stale `@gnd/dashboard` runtime package declarations after exact app import/config scans and refreshed the Bun lockfile:
...
- Removed old tracked `apps/dashboard/tailwind-copy.config` after confirming it had no references and the app's active PostCSS config delegates to `@gnd/ui/postcss.config`.
...
- Repaired the remaining five Knip unresolved import candidates in legacy sales type code:
  ...
  - Replaced deleted private v2 form-action imports inside `apps/dashboard/src/app-deps/(v2)/(loggedIn)/sales-v2/type.ts` with a local structural legacy form type so live old-sales-form type consumers no longer depend on missing files.
...
- Added `server-only` to `@gnd/dashboard` dependencies because five `apps/dashboard` server-only modules import it directly.
...
- Retained the remaining package candidates as tooling/config-owned instead of removing them:
  ...
  - `puppeteer-core` is kept because `apps/dashboard/next.config.mjs` lists it in `serverExternalPackages`.
  - `tailwindcss` is kept because `apps/dashboard` declares `@tailwindcss/postcss`, delegates PostCSS to `@gnd/ui/postcss.config`, and still has shadcn/Tailwind component tooling metadata.
...
- Removed a focused utility/component export slice after exact symbol and import scans:
  ...
  - Removed the now-unused `@date-fns/tz` package declaration from `@gnd/dashboard` and refreshed `bun.lock` because it was only used by the deleted `utils/format.ts` date-range helper.
...
- Removed a utility/action export-surface slice:
  ...
  - Removed the now-unused `bcrypt-ts` dependency declaration from `@gnd/dashboard` and refreshed `bun.lock`.
...
- Removed an old v1 auth/notification/community action export-surface slice:
  ...
  - Removed follow-on unused app-local community cost/pivot and generic numeric helper exports after exact scans showed no remaining `apps/dashboard` imports.
...
| --- | ---: | --- |
| `apps/dashboard/src/components` | 5 | Remaining candidates are mostly test/support files rather than obvious removable UI leaves. |
| `apps/dashboard/src/app-deps` | 3 | Remaining app-deps candidates are tests and were retained by default. |
| `apps/dashboard/src/app` | 1 | Remaining app candidate is test-read production assignment code. |
| `apps/dashboard/src/actions` | 4 | Remaining action candidates include test-read production/inventory reset fixtures and should not be bulk-deleted. |
| `apps/dashboard/src/lib` | 3 | Remaining lib candidates are auth/routing tests and should not be bulk-deleted. |
| `apps/dashboard/src/modules` | 4 | Remaining module candidates are sales-print tests and other false-positive-prone integration/test helpers. |
| `apps/dashboard/src/hooks` | 1 | Remaining hook candidate is test-backed URL-param behavior and should not be bulk-deleted. |
| `apps/dashboard/src/styles` | 1 | Remaining stylesheet is referenced by `components.json` tooling but not imported by the app runtime. |
...
- `apps/dashboard/src/middleware-copy.ts` - removed in first cleanup slice.
  - Active runtime uses `apps/dashboard/src/proxy.ts`.
...
- `apps/dashboard/image-loader.ts` - removed in first cleanup slice.
  - `next.config.mjs` has `images.loader` and `loaderFile` commented out.
- `apps/dashboard/src/app-deps/**/loading.tsx` - the 21 tracked stale files were removed in first cleanup slice.
  - 21 files flagged.
  - `app-deps` is not the active App Router root, and these are not imported by live code.
- Debug/sandbox/copy files:
  - `apps/dashboard/src/app-deps/sand-box/zus-form/*`
  - `apps/dashboard/src/app-deps/(public)/debug/sheet/*`
  - `apps/dashboard/src/app-deps/(public)/debug/sheet-with-select/*`
  - `apps/dashboard/src/app-deps/(sidebar)/(sales)/sales-book/(bebug)/debug-*/data-table-copy.tsx`
  - `apps/dashboard/src/app/(sidebar)/community/(main)/builders/page-copy.tsx`
  - Removed in first cleanup slice.
- One-off fix/upgrade files:
  - `apps/dashboard/src/actions/--fix/fix-customer-tax-profiles.ts` - removed with the unmounted `_backward-compat` repair menu island.
  - `apps/dashboard/src/app-deps/(v1)/_actions/upgrade/*` - removed in second cleanup slice.
  - `apps/dashboard/src/app/(clean-code)/(sales)/_backward-compat/fix-customer-tax-profiles.tsx`
  - `apps/dashboard/src/app/(clean-code)/(sales)/_backward-compat/january-sales-stats.tsx`
- Finder metadata:
  - Earlier audit output noted `.DS_Store` files under `apps/dashboard/src/app-deps`, but the current tracked-file check returned no tracked `.DS_Store` files under `apps/dashboard`.

## Needs Careful Migration Review

These areas look old but are not safe for bulk deletion:

- `apps/dashboard/src/app-deps/(clean-code)/(sales)`
  ...
  - Several imports originate from active legacy sales form components under `apps/dashboard/src/components/forms/sales-form`.
- `apps/dashboard/src/app-deps/(v1)/_actions`
...
- `apps/dashboard/src/components/_v1`
  - 66 file candidates.
...
- `apps/dashboard/src/app/(clean-code)/(sales)`
  - 57 file candidates.
...
- `apps/dashboard/src/modules/error/report.ts`
  - Knip still reports this file as unused, but source inspection shows `apps/dashboard/src/modules/pdf/create-pdf.ts` imports `logError` from it. Treat it as a false positive unless `modules/pdf/create-pdf.ts` is also retired.
...
- MDX/editor stack: `@mdx-js/*`, `@next/mdx`, `@mdxeditor/editor`, `next-mdx-remote`, `@types/mdx`. The old local MDX/Tiptap prototype components were removed; package cleanup was deferred because `apps/dashboard/package.json` already has unrelated user edits in the working tree.
...
- `server-only` is now listed in `apps/dashboard/package.json` because `src/trpc/server.tsx`, `src/lib/auth/session.ts`, `src/lib/auth/web-auth.ts`, `src/utils/get-geolocation.ts`, and `src/lib/sales-visibility.ts` import it directly.
...
2. Delete high-confidence non-code and debug leftovers:
   - `.DS_Store` files under `apps/dashboard/src/app-deps`
...
3. Remove `app-deps` route-convention leftovers:
   - completed for the 21 tracked stale `loading.tsx` files under `apps/dashboard/src/app-deps`
...
- First cleanup slice validation:
  - `bun test apps/dashboard/src/components/sidebar-links.test.ts` passed with 6 tests and 30 expectations.
  - Active sidebar route-literal scan returned 0 missing path literals.
  - `find apps/dashboard/src/app-deps -type f -name 'loading.tsx'` returned no files.
  - scoped `git diff --check -- apps/dashboard brain` passed.
  - no source references remain for `app-deps/(v1)/_actions/upgrade`.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 571 remaining file candidates after the second cleanup slice.
...
- Third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 554 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Fourth cleanup slice validation:
  ...
  - `EmailTypes` now lives in `apps/dashboard/src/lib/modal.ts`, preserving the type used by `openEmailComposer`.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 533 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 522 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 507 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 495 remaining file candidates.
- Eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 480 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 475 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Tenth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 453 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Eleventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 444 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Twelfth cleanup slice validation:
  ...
  - Confirmed the remaining `getUsersListAction` usage resolves to the separate live `apps/dashboard/src/actions/get-users-list.ts` path, not the deleted duplicate data-action file.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 436 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Thirteenth cleanup slice validation:
  ...
  - Confirmed `apps/dashboard/src/_v2/components/common/render-form.tsx`, `apps/dashboard/src/lib/format.ts`, and `apps/dashboard/src/lib/chunker.ts` remain live through current imports and were not removed; `where.dispatch.ts` was later removed after a focused root-utility review.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 430 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Fourteenth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 424 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Fifteenth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 414 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Sixteenth cleanup slice validation:
  ...
  - Confirmed `apps/dashboard/src/actions/--fix/fix-undefined-order-id.ts` remains live through the edit-quote route and was not removed.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 399 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Seventeenth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 392 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Eighteenth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 385 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Nineteenth cleanup slice validation:
  ...
  - Confirmed `apps/dashboard/src/app/(clean-code)/_common/export/config.ts` and `type.ts` remain present while the duplicate app-deps copies were removed.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 379 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Twentieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 372 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Twenty-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 366 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Twenty-second cleanup slice validation:
  ...
  - Confirmed `apps/dashboard/src/components/viewer-shell/controller.ts` remains live through `apps/dashboard/src/modules/sales-print/application/sales-print-service.ts` and was not removed.
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 362 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed after the following twenty-third slice.
- Twenty-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 360 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Twenty-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 355 remaining file candidates.
- Twenty-fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 350 remaining file candidates.
- Twenty-sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 345 remaining file candidates.
  - scoped `git diff --check -- apps/dashboard brain` passed.
- Twenty-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 342 remaining file candidates.
- Twenty-eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 338 remaining file candidates.
- Twenty-ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 331 remaining file candidates.
- Thirtieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 326 remaining file candidates.
- Thirty-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 321 remaining file candidates.
- Thirty-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 311 remaining file candidates.
- Thirty-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 308 remaining file candidates.
- Thirty-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --no-progress --reporter json --no-exit-code` reported 306 remaining file candidates.
- Thirty-fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 302 remaining file candidates.
- Thirty-sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 299 remaining file candidates.
- Thirty-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 297 remaining file candidates.
...
- Fortieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 275 remaining file candidates.
- Forty-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 259 remaining file candidates.
- Forty-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 244 remaining file candidates.
- Forty-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 233 remaining file candidates.
- Forty-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 231 remaining file candidates.
- Forty-fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 227 remaining file candidates.
- Forty-sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 222 remaining file candidates.
- Forty-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 213 remaining file candidates.
- Forty-eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 204 remaining file candidates.
- Forty-ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 199 remaining file candidates.
- Fiftieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 196 remaining file candidates.
- Fifty-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 193 remaining file candidates.
- Fifty-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 190 remaining file candidates.
- Fifty-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 189 remaining file candidates.
- Fifty-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 185 remaining file candidates.
- Fifty-fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 179 remaining file candidates.
- Fifty-sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 177 remaining file candidates.
- Fifty-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 175 remaining file candidates.
- Fifty-eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 174 remaining file candidates.
- Fifty-ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 172 remaining file candidates.
- Sixtieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 169 remaining file candidates.
- Sixty-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 164 remaining file candidates.
- Sixty-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 156 remaining file candidates.
- Sixty-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 148 remaining file candidates.
- Sixty-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 146 remaining file candidates.
- Sixty-fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 142 remaining file candidates.
- Sixty-sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 138 remaining file candidates.
- Sixty-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 137 remaining file candidates.
- Sixty-eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 130 remaining file candidates.
- Sixty-ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 128 remaining file candidates.
- Seventieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 126 remaining file candidates.
- Seventy-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 123 remaining file candidates.
- Seventy-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 110 remaining file candidates.
- Seventy-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 102 remaining file candidates.
- Seventy-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 100 remaining file candidates.
- Seventy-fifth cleanup slice validation:
  ...
  - Confirmed `item-assign-action.ts` remains because `apps/dashboard/src/actions/production-control-reset.test.ts` imports it directly.
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 96 remaining file candidates.
- Seventy-sixth cleanup slice validation:
  ...
  - Confirmed `sales-progress-fallback.ts` remains because `apps/dashboard/src/actions/production-control-reset.test.ts` reads it directly, and confirmed `get-sales-customer-data.ts` remains live through the legacy sales-form customer data section.
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 91 remaining file candidates.
- Seventy-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 86 remaining file candidates.
- Seventy-eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 76 remaining file candidates.
- Seventy-ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 72 remaining file candidates.
- Eightieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 67 remaining file candidates.
- Eighty-first cleanup slice validation:
  - Exact path scans for deleted `apps/dashboard/src/utils/db/where.dispatch.ts` returned no live imports.
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 66 remaining file candidates.
- Eighty-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 59 remaining file candidates.
- Eighty-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 57 remaining file candidates.
- Eighty-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 56 remaining file candidates.
- Eighty-fifth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 53 remaining file candidates.
- Eighty-sixth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 49 remaining file candidates.
- Eighty-seventh cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 45 remaining file candidates.
- Eighty-eighth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 43 remaining file candidates.
- Eighty-ninth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 39 remaining file candidates.
- Ninetieth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 35 remaining file candidates.
- Ninety-first cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 32 remaining file candidates.
- Ninety-second cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 30 remaining file candidates.
- Ninety-third cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 28 remaining file candidates.
- Ninety-fourth cleanup slice validation:
  ...
  - refreshed `bun run knip --workspace apps/dashboard --include files --reporter json` reported 20 remaining file candidates.
- Ninety-fifth cleanup slice validation:
  - The remaining file-only Knip candidates classify as 16 tests, 3 production-control files read directly by `production-control-reset.test.ts`, and 1 shadcn tooling stylesheet referenced by `apps/dashboard/src/components.json`.
...
- Ninety-sixth cleanup slice validation:
  - Exact import/config scans found no live `apps/dashboard` usage for removed package declarations: `@actions/core`, `@actions/github`, Cloudinary React helpers, MDX/MDX editor packages, `next-cloudinary`, `next-mdx-remote`, `crypto` package dependency, `i`, `npm`, `resend`, and `@types/mdx`.
  - `bun install --lockfile-only` refreshed `bun.lock`.
  - refreshed `bun run knip --workspace apps/dashboard --include files,exports,dependencies,unlisted,unresolved --reporter json` reports 39 runtime dependency candidates, 3 dev dependency candidates, 20 file candidates, 5 unlisted entries, 5 unresolved entries, and 551 export candidates.
- No build, typecheck, browser QA, or dev server was run.
- Because `apps/dashboard` still depends on legacy compatibility code, every deletion PR should run a focused import scan first and only then a narrow validation command for the affected domain.
