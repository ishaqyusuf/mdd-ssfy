...
export async function getDealerPortalDashboard(db: Database, dealerId: number) {
	...
	const totals = orderTotals.reduce(
		(acc, order) => {
			const total = Number(
				order.dealerSale?.grandTotal ?? order.grandTotal ?? 0,
			);
		...
		if (!alreadyApproved) {
			...
			const currentMeta =
				...
				!Array.isArray(currentOrder.meta)
					? currentOrder.meta
					: {};
			const previousDeliveryCost = Number(
				row.sale.extraCosts?.[0]?.amount || 0,
			);
			const deliveryDelta =
			...
			await tx.salesOrders.update({
				...
				data: {
					...
					...(deliveryDelta
						? {
								grandTotal:
									Number(currentOrder?.grandTotal || 0) + deliveryDelta,
