import {
	type ContractorAccountingReportFormat,
	type ContractorAccountingReportKind,
	type ContractorLedgerEntryType,
	type ContractorLedgerSourceType,
	type ContractorReconciliationIssueCode,
	type ContractorReconciliationIssueStatus,
	Prisma,
} from "@prisma/client";
import type { Db, TransactionClient } from "../index";

type WriteDb = Pick<
	TransactionClient,
	| "contractorAccountingPeriod"
	| "contractorLedgerEntry"
	| "contractorReconciliationIssue"
	| "contractorReconciliationRun"
	| "contractorAccountingReportRun"
	| "contractorAccountingReportSchedule"
	| "contractorTaxProfile"
	| "jobs"
	| "jobPayments"
	| "users"
>;

export type PostContractorLedgerEntryInput = {
	contractorId: number;
	type: ContractorLedgerEntryType;
	amount: Prisma.Decimal | string | number;
	liabilityDelta: Prisma.Decimal | string | number;
	effectiveAt: Date;
...
export type ListContractorLedgerEntriesInput = {
	q?: string | null;
	from?: Date | null;
	toExclusive?: Date | null;
	contractorIds?: number[] | null;
	entryTypes?: ContractorLedgerEntryType[] | null;
	sourceTypes?: ContractorLedgerSourceType[] | null;
...
export async function assertContractorAccountingDateWritable(
	db: WriteDb,
	effectiveAt: Date,
) {
...
export async function postContractorLedgerEntry(
	db: WriteDb,
	input: PostContractorLedgerEntryInput,
) {
	...
	if (existing) {
		...
		if (!matches) {
			throw new Error(
				`Ledger source ${input.sourceKey} already exists with different accounting values.`,
			);
		}
		return existing;
	}
...
export async function reverseContractorLedgerEntry(
	db: WriteDb,
	input: {
		entryId: string;
		effectiveAt: Date;
	...
	if (!original) throw new Error("Contractor ledger entry was not found.");
...
async function buildLedgerWhere(
	db: Db,
	input: ListContractorLedgerEntriesInput,
) {
	const and: Prisma.ContractorLedgerEntryWhereInput[] = [];
	...
	if (input.entryTypes?.length) and.push({ type: { in: input.entryTypes } });
...
export async function listContractorLedgerEntries(
	db: Db,
	input: ListContractorLedgerEntriesInput,
) {
	...
	const balances = ids.length
		? await db.$queryRaw<Array<{ id: string; balanceAfter: Prisma.Decimal }>>(
				Prisma.sql`
	...
	const balanceMap = new Map(
		balances.map((balance) => [balance.id, balance.balanceAfter]),
	);
	return {
		data: data.map((entry) => ({
			...entry,
...
export async function getContractorLedgerEntry(db: Db, id: string) {
...
export async function getContractorLedgerEntriesThrough(
	db: Db,
	input: {
		toExclusive: Date;
		contractorIds?: number[] | null;
	...
	if (rows.length > limit) {
		throw new Error(
			"Contractor ledger report is too large; narrow the period or contractor filter.",
		);
	}
...
export async function listContractorAccountingPeriods(db: Db) {
...
export async function closeContractorAccountingPeriod(
	db: Db,
	input: {
		from: Date;
		toExclusive: Date;
		timezone: string;
		closingBalance: Prisma.Decimal | string | number;
	...
	return db.$transaction(async (tx) => {
		...
		if (overlapping) {
			throw new Error(
				"This period overlaps an already closed accounting period.",
			);
		}
		const now = new Date();
...
export async function reopenContractorAccountingPeriod(
	db: Db,
	input: { periodId: string; actorId: number; reason: string },
) {
	return db.$transaction(async (tx) => {
...
export async function listContractorReconciliationIssues(
	db: Db,
	input: {
		statuses?: ContractorReconciliationIssueStatus[] | null;
		codes?: ContractorReconciliationIssueCode[] | null;
		contractorIds?: number[] | null;
		cursor?: string | null;
		pageSize?: number | null;
	},
	...
	const contractorIds = [
		...new Set(
			data.flatMap((issue) =>
				issue.contractorId == null ? [] : [issue.contractorId],
			),
		),
	];
	const contractors = contractorIds.length
...
export async function reviewContractorReconciliationIssue(
	db: WriteDb,
	input: {
		id: string;
...
export async function recordContractorReconciliationRun(
	db: Db,
	input: {
		from: Date;
		toExclusive: Date;
		...
		sourceTotals: Prisma.InputJsonValue;
		ledgerTotals: Prisma.InputJsonValue;
		issues: Array<{
			code: ContractorReconciliationIssueCode;
			contractorId?: number | null;
			ledgerEntryId?: string | null;
			message: string;
			expectedAmount?: Prisma.Decimal | string | number | null;
			actualAmount?: Prisma.Decimal | string | number | null;
			differenceAmount?: Prisma.Decimal | string | number | null;
			evidence?: Prisma.InputJsonValue | null;
		}>;
	},
) {
...
export async function getContractorAccountingFilterOptions(db: Db) {
...
export async function createContractorAccountingReportRun(
	db: WriteDb,
	input: {
...
export async function listContractorAccountingReportRuns(
	db: Db,
	input: { requestedById?: number | null; take?: number },
) {
...
export async function createContractorAccountingReportSchedule(
	db: WriteDb,
	input: {
		name: string;
		kind: ContractorAccountingReportKind;
...
export async function listContractorAccountingReportSchedules(db: Db) {
...
export async function updateContractorAccountingReportRun(
	db: WriteDb,
	input: {
		id: string;
		status: "RUNNING" | "COMPLETED" | "FAILED";
		totals?: Prisma.InputJsonValue | null;
...
export async function listContractorTaxProfiles(db: Db) {
...
export async function upsertContractorTaxProfile(
	db: WriteDb,
	input: Prisma.ContractorTaxProfileUncheckedCreateInput,
) {
...
type ContractorPaymentMeta = {
	cancelledAt?: string | null;
};

function paymentMeta(value: Prisma.JsonValue | null): ContractorPaymentMeta {
	return value && typeof value === "object" && !Array.isArray(value)
		? (value as ContractorPaymentMeta)
		: {};
}
...
async function createLedgerEntriesInBatches(
	db: Db,
	entries: Prisma.ContractorLedgerEntryCreateManyInput[],
	batchSize = 500,
) {
...
export async function backfillContractorLedgerFromLegacy(
	db: Db,
	input: { dryRun: boolean; requestedById?: number | null },
) {
	...
	if (jobs.length > 50_000 || payments.length > 25_000) {
		throw new Error(
			"Legacy contractor data exceeds the safe backfill limits; backfill in bounded date ranges.",
		);
	}

	const missingEffectiveDates: string[] = [];
	let legacyJobDateFallbackCount = 0;
	const baseEntries: Prisma.ContractorLedgerEntryCreateManyInput[] = [];
	for (const job of jobs) {
		if (!exactEarnedStatuses.has(job.status)) continue;
		const effectiveAt = job.approvedAt ?? job.statusDate ?? job.createdAt;
		if (!effectiveAt) {
			missingEffectiveDates.push(`JOB:${job.id}`);
			continue;
		}
		if (!job.approvedAt) legacyJobDateFallbackCount += 1;
	...
	for (const payment of payments) {
		...
		for (const adjustment of payment.adjustments) {
			...
			const liabilityDelta =
				adjustment.type === "DEDUCTION" ? amount.negated() : amount;
			baseEntries.push({
	...
	const summary = {
		...
		adjustmentEntryCount: payments.reduce(
			(total, payment) => total + payment.adjustments.length,
			0,
		),
	...
	const payoutEntryByPaymentId = new Map(
		payoutEntries.map((entry) => [Number(entry.sourceId), entry.id]),
	);
	await createLedgerEntriesInBatches(
		db,
		cancelledPayments.flatMap((payment) => {
