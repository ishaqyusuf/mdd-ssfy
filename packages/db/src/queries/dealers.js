...
function getDealerQuoteEditLock(status) {
    ...
    if (status === "rejected") {
        return {
            locked: true,
            reason: "This quote is locked after GND reviewed the request. Contact GND to start a revision.",
        };
    }
    return {
        locked: false,
...
function getDealerCustomerAddressMeta(meta) {
    const objectMeta = getObjectMeta(meta);
    const address = objectMeta.dealerAddress &&
        typeof objectMeta.dealerAddress === "object" &&
        !Array.isArray(objectMeta.dealerAddress)
        ? objectMeta.dealerAddress
        : {};
    return {
        formattedAddress: typeof address.formattedAddress === "string"
...
function buildDealerAddressMeta(input) {
    return {
        formattedAddress: input.formattedAddress?.trim() || input.address?.trim() || null,
...
function getDealerCustomerTaxProfile(customer) {
    ...
    return {
        ...
        taxProfile: taxProfile
            ? {
                ...
                percentage: taxProfile.tax?.percentage == null
                    ? null
...
function sanitizeDealerScopedCustomerProfile(customer, dealerId) {
    if (!customer.profile)
        return customer;
...
function dealerSearchWhere(search) {
...
async function getDealerPortalDashboard(db, dealerId) {
    ...
    const totals = orderTotals.reduce((acc, order) => {
...
function getDealerDashboardTaxTotal(meta) {
    if (!meta || typeof meta !== "object" || Array.isArray(meta))
        return 0;
    const newSalesForm = meta.newSalesForm;
    if (!newSalesForm ||
        typeof newSalesForm !== "object" ||
        Array.isArray(newSalesForm)) {
        return 0;
    }
    const summary = newSalesForm.summary;
    if (!summary || typeof summary !== "object" || Array.isArray(summary)) {
        return 0;
    }
    const value = Number(summary.taxTotal || 0);
...
async function getDealerPortalCustomers(db, dealerId) {
    ...
    const salesCountsByCustomer = await getDealerSalesCountsByCustomer(db, dealerId, customers.map((customer) => customer.id));
...
async function getDealerSalesCountsByCustomer(db, dealerId, customerIds) {
    ...
    for (const countRow of salesCounts) {
        if (!countRow.customerId)
            continue;
        const current = salesCountsByCustomer.get(countRow.customerId) || {
            ordersCount: 0,
            quotesCount: 0,
        };
        if (countRow.type === "order")
            current.ordersCount = countRow._count._all;
        if (countRow.type === "quote")
            current.quotesCount = countRow._count._all;
...
async function getDealerPortalCustomer(db, dealerId, id) {
    ...
    const scopedCustomer = sanitizeDealerScopedCustomerProfile(customer, dealerId);
...
async function getDealerPortalCustomerOverview(db, dealerId, id) {
    ...
    const counts = salesCounts.reduce((acc, row) => {
        if (row.type === "quote")
            acc.quotesCount = row._count._all;
        if (row.type === "order")
            acc.ordersCount = row._count._all;
        return acc;
    }, {
        ordersCount: 0,
        quotesCount: 0,
...
function mapDealerSalesDocument(document) {
    ...
    const { meta: _meta, pickup: _pickup, deliveries: _deliveries, prodStatus: _prodStatus, deliveredAt: _deliveredAt, ...safeDocument } = document;
    const customerTotal = Number(document.dealerSale?.grandTotal ?? document.grandTotal ?? 0);
    const customerAmountDue = Number(document.dealerSale?.dueAmount ?? document.amountDue ?? 0);
    return {
        ...safeDocument,
        ...
        customerPaidAmount: roundCurrency(Math.max(0, customerTotal - customerAmountDue)),
        ...
        requestDecisionAt: (typeof approvalMeta.approvedAt === "string"
            ...
            null,
        requestDecisionNote: (typeof approvalMeta.approverNote === "string"
...
function getDealerFulfillmentStatus(document) {
    ...
    if (document.deliveredAt ||
        ...
        (!/\b(?:not|never)\s+(?:completed|complete|delivered|fulfilled|picked up)\b/.test(status) &&
            /\b(?:completed|complete|delivered|fulfilled|picked up)\b/.test(status))) {
        return "completed";
    }
    if (!/\b(?:not|never)\s+(?:ready|packed)\b/.test(status) &&
...
async function getDealerPortalCustomersList(db, dealerId, input = {}) {
    ...
    const salesCountsByCustomer = await getDealerSalesCountsByCustomer(db, dealerId, customerIds);
...
async function saveDealerPortalCustomer(db, dealerId, input) {
    ...
    const displayAddress = addressMeta.formattedAddress ||
        [
    ...
    return db.$transaction(async (tx) => {
        const saveCustomerTaxProfile = async (customerId) => {
...
function getDealerNewSalesFormMeta(meta) {
    if (!meta || typeof meta !== "object" || Array.isArray(meta))
        return null;
    const value = meta.newSalesForm;
    return value && typeof value === "object" && !Array.isArray(value)
        ? value
        : null;
}
function finitePricingNumber(value) {
    const number = Number(value);
    return Number.isFinite(number) ? number : null;
}
function pricingValueChanged(savedValue, currentValue) {
...
async function getDealerPortalSalesDocument(db, dealerId, id) {
    ...
    const { meta: _meta, items, pickup: _pickup, deliveredAt: _deliveredAt, deliveries: _deliveries, prodStatus: _prodStatus, ...safeDocument } = privateDocument;
    const metaRecord = document.meta &&
        typeof document.meta === "object" &&
        !Array.isArray(document.meta)
        ? document.meta
        : {};
    const savedCoefficient = finitePricingNumber(metaRecord.salesCoefficient) ??
        finitePricingNumber(metaRecord.sales_percentage);
    const currentCoefficient = finitePricingNumber(document.salesProfile?.coefficient);
    const savedDealerSalesPercentage = finitePricingNumber(document.dealerSale?.dealerSalesPercentage);
    const currentDealerSalesPercentage = finitePricingNumber(document.dealerSale?.dealerCustomerProfile?.salesPercentage);
    const dealerSalesMultiplier = 1 + Number(document.dealerSale?.dealerSalesPercentage || 0) / 100;
    const customerGrandTotal = Number(document.dealerSale?.grandTotal ?? document.grandTotal ?? 0);
    const customerAmountDue = Number(document.dealerSale?.dueAmount ?? document.amountDue ?? 0);
    return {
        ...safeDocument,
        officeGrandTotal: Number(document.grandTotal || 0),
        officeAmountDue: nullableFiniteNumber(document.amountDue),
        customerPaymentStatus: customerAmountDue <= 0 ? "paid" : "unpaid",
        customerPaidAmount: roundCurrency(Math.max(0, customerGrandTotal - customerAmountDue)),
        fulfillmentStatus: getDealerFulfillmentStatus(document),
        pricingContext: {
            internal: {
            ...
            dealer: {
                dealerCustomerProfileId: document.dealerSale?.dealerCustomerProfileId ||
                ...
                hasChanged: pricingValueChanged(savedDealerSalesPercentage, currentDealerSalesPercentage),
            },
        },
        ...
        customerProfileId: Number(form?.customerProfileId ||
            ...
            0) || null,
        taxRate: Number(newSalesForm?.summary?.taxRate ??
            document.taxPercentage ??
            0),
        po: typeof form?.po === "string" ? form.po : null,
        paymentTerm: typeof form?.paymentTerm === "string" ? form.paymentTerm : "None",
        goodUntil: typeof form?.goodUntil === "string" ? form.goodUntil : null,
        deliveryOption: typeof form?.deliveryOption === "string"
            ? form.deliveryOption
            : document.deliveryOption || "pickup",
        paymentMethod: typeof form?.paymentMethod === "string" ? form.paymentMethod : null,
        taxCode: typeof form?.taxCode === "string" ? form.taxCode : null,
        sellerOfRecord: form?.sellerOfRecord === "GND" ||
            document.dealerSale?.sellerOfRecord === "GND"
            ? "GND"
            : "DEALER",
        resaleCertificateOnFile: typeof form?.resaleCertificateOnFile === "boolean"
        ...
        lineItems: metaLineItems?.length
            ...
            : items.map((item) => {
                ...
                return {
                    ...
                    title: typeof itemMeta.title === "string"
                        ? itemMeta.title
                        : item.dykeDescription || item.description || "",
                    description: item.description || "",
                    qty: Number(item.qty || 0),
                    unitPrice: roundCurrency(Number(item.rate || 0) * dealerSalesMultiplier),
                    lineTotal: roundCurrency(Number(item.qty || 0) *
...
function pricingCoefficient(profile) {
    const value = Number(profile?.coefficient ?? 1);
    return Number.isFinite(value) && value > 0 ? value : 1;
}
function pricingCoefficientMultiplier(profile) {
    return roundCurrency(1 / pricingCoefficient(profile));
}
function pricingSalesPercentage(profile) {
    const value = Number(profile?.salesPercentage ?? 0);
    return Number.isFinite(value) ? value : 0;
}
function finiteOptionalPricingNumber(value) {
    const number = Number(value);
    return Number.isFinite(number) ? number : null;
}
function positiveOptionalPricingNumber(value) {
    const number = finiteOptionalPricingNumber(value);
    return number != null && number > 0 ? number : null;
}
function pricingSalesPercentageMultiplier(profile) {
    return 1 + pricingSalesPercentage(profile) / 100;
}
function baseUnitPriceFromDealerLine(line) {
...
function dealerLineMeta(line) {
    return line.meta && typeof line.meta === "object" && !Array.isArray(line.meta)
        ? line.meta
        : {};
}
function dealerLineServiceRows(line) {
    const rows = dealerLineMeta(line).serviceRows;
    return Array.isArray(rows)
        ? rows.filter((row) => Boolean(row) && typeof row === "object" && !Array.isArray(row))
        : [];
}
function dealerLineIsTaxable(line) {
    if (typeof line.taxxable === "boolean")
        return line.taxxable;
    const meta = dealerLineMeta(line);
    if (typeof meta.taxxable === "boolean")
        return meta.taxxable;
    if (typeof meta.tax === "boolean")
        return meta.tax;
...
function dealerLineIsProduceable(line) {
    const meta = dealerLineMeta(line);
    if (typeof meta.produceable === "boolean")
        return meta.produceable;
...
function numberSetFromUnknown(value) {
    const ids = Array.isArray(value) ? value : [];
    return new Set(ids
        .map((entry) => numberFromUnknown(entry))
        .filter((entry) => entry != null));
}
function getDealerShelfVisibilityFromSettings(meta) {
    const settingsMeta = getObjectMeta(meta);
    const nestedSettingsMeta = getObjectMeta(settingsMeta.data);
    const rawVisibility = getObjectMeta(Object.keys(getObjectMeta(settingsMeta.dealerShelfCategoryVisibility))
        .length
        ? settingsMeta.dealerShelfCategoryVisibility
        : nestedSettingsMeta.dealerShelfCategoryVisibility);
    const mode = rawVisibility.mode === "allowlist" ? "allowlist" : "all";
    return {
        mode,
        categoryIds: mode === "allowlist"
            ? numberSetFromUnknown(rawVisibility.categoryIds)
            : new Set(),
    };
}
async function getDealerQuoteVisibilitySettings(tx) {
    ...
    const nestedSettingsMeta = getObjectMeta(settingsMeta.data);
    const rawRoute = getObjectMeta(Object.keys(getObjectMeta(settingsMeta.route)).length
        ? settingsMeta.route
        : nestedSettingsMeta.route);
    const hiddenRootUids = new Set();
...
function lineUsesHiddenDealerRoot(line, hiddenRootUids) {
    if (!hiddenRootUids.size)
        return false;
    ...
    const candidateUids = [
        ...
        ...(Array.isArray(line.formSteps) ? line.formSteps : []).map((step) => {
            ...
            return (stringFromUnknown(stepMeta.prodUid) ||
...
async function getShelfProductsById(tx, productIds) {
...
function shelfItemAllowedByCategory(shelfItem, product, allowedCategoryIds) {
...
async function validateDealerPortalQuoteVisibility(tx, lineItems) {
    ...
    const shelfItems = lineItems.flatMap((line) => Array.isArray(line.shelfItems) ? line.shelfItems : []);
...
function calculateDealerQuotePricing({ lineItems, taxRate, internalProfile, dealerProfile, createdAt, sellerOfRecord = "DEALER", resaleCertificateOnFile = false, }) {
    ...
    const snapshotCreatedAt = createdAt instanceof Date
    ...
    const internalSubTotal = roundCurrency(lines.reduce((sum, line) => sum + line.internalLineTotal, 0));
    const dealerSubTotal = roundCurrency(lines.reduce((sum, line) => sum + line.dealerLineTotal, 0));
    const internalTaxableSubTotal = roundCurrency(lines.reduce((sum, line) => sum + (line.taxable ? line.internalLineTotal : 0), 0));
    const dealerTaxableSubTotal = roundCurrency(lines.reduce((sum, line) => sum + (line.taxable ? line.dealerLineTotal : 0), 0));
    const internalTaxExempt = sellerOfRecord === "DEALER" && resaleCertificateOnFile;
...
function calculateDealerApprovalPricing({ lineItems, taxRate, internalGrandTotal, internalCoefficient, dealerSalesPercentage, fallbackDealerGrandTotal, }) {
    ...
    const nonLineAdjustment = roundCurrency(internalGrandTotal - pricing.internalPricing.grandTotal);
    return {
...
function mergeDealerApprovalDeliveryMeta({ meta, deliveryCost, extraCostId, }) {
    ...
    const persistedExtraCosts = Array.isArray(persistedForm.extraCosts)
        ? persistedForm.extraCosts.filter((cost) => String(getObjectMeta(cost).type || "").toLowerCase() !== "delivery")
        : [];
    return {
        ...root,
        deliveryCost,
...
async function createDealerProgramPartnerIdentity(db, type) {
...
async function saveDealerPortalQuote(db, dealerId, input) {
    return db.$transaction(async (tx) => {
        ...
        if (editLock.locked) {
            throw new DealerQuoteEditLockedError(latestRequestStatus, editLock.reason);
        }
        ...
        const dealerDefaults = getDealerDefaultsFromMeta(dealerAccount?.meta);
        const requestedDealerProfileId = input.customerProfileId ||
            customer.customerTypeId ||
            dealerDefaults.defaultCustomerProfileId ||
            null;
        const [requestedDealerProfile, defaultDealerProfile, firstDealerProfile, internalProfile,] = await Promise.all([
        ...
        const dealerProfile = requestedDealerProfile ||
            (input.customerProfileId
                ? null
                : defaultDealerProfile || firstDealerProfile);
        const customerTaxProfile = getDealerCustomerTaxProfile(customer);
        const effectiveTaxCode = input.taxCode === undefined
        ...
        const selectedTaxModel = effectiveTaxCode
            ...
            : null;
        const defaultTaxRate = selectedTaxModel?.percentage == null
            ? null
            : Number(selectedTaxModel.percentage);
        const effectiveTaxRate = effectiveTaxCode && defaultTaxRate != null
            ? defaultTaxRate
        ...
        const effectiveDeliveryOption = input.deliveryOption || dealerDefaults.defaultFulfillmentMode || "pickup";
        if (!dealerProfile) {
            throw new Error("Dealer customer profile is required before saving a quote.");
        }
        if (!dealerProfile.id) {
        ...
        const normalizedLines = input.lineItems.map((line, index) => {
            ...
            const lineTotal = roundCurrency(Number.isFinite(Number(line.lineTotal))
        ...
        const effectiveInternalProfile = {
            ...internalProfile,
            coefficient: positiveOptionalPricingNumber(input.pricingContext?.salesCoefficient) ??
                internalProfile?.coefficient,
        };
        const effectiveDealerProfile = {
            ...dealerProfile,
            salesPercentage: finiteOptionalPricingNumber(input.pricingContext?.dealerSalesPercentage) ?? dealerProfile.salesPercentage,
        };
        const pricing = calculateDealerQuotePricing({
        ...
        const dealerMarkupAmount = roundCurrency(pricing.dealerPricing.subTotal - pricing.internalPricing.subTotal);
        const selectedTaxModelSnapshot = {
            ...
            sellerOfRecord,
            resaleCertificateOnFile,
            internalTaxApplies: !(sellerOfRecord === "DEALER" && resaleCertificateOnFile),
        ...
        const identity = existing || (await createDealerProgramPartnerIdentity(tx, "quote"));
        const orderData = {
            ...
            isDyke: true,
            dealerAuthId: dealerId,
            salesRepId: dealerAccount?.salesRepId ||
                null,
            customerId: customer.id,
...
async function convertDealerPortalQuoteToOrderTx(tx, dealerId, quoteId, metaPatch = {}) {
    ...
    const currentMeta = quote.meta && typeof quote.meta === "object" && !Array.isArray(quote.meta)
        ? quote.meta
        : {};
...
async function requestDealerPortalQuoteOrder(db, dealerId, quoteId) {
    return db.$transaction(async (tx) => {
        ...
        const deliveryOption = String(quote.deliveryOption || quoteForm.deliveryOption || "pickup").toLowerCase();
        const customerAddress = getDealerCustomerAddressMeta(quote.customer?.meta);
        const recipientName = quote.customer?.businessName?.trim() ||
        ...
        const recipientAddress = customerAddress.address1 ||
        ...
        if (requiresRecipient &&
            (!recipientName ||
                ...
                !recipientPhone ||
                !recipientAddress)) {
            throw new Error("Customer name, email, phone, and shipping address are required before requesting delivery or shipment.");
        }
        let billingAddressId = quote.billingAddressId || null;
        let shippingAddressId = quote.shippingAddressId || null;
        let fulfillmentRecipient = getObjectMeta(quoteMeta.dealerFulfillmentRecipient);
...
async function getSalesRequestUserScope(db, userId) {
    ...
    const roleNames = user?.roles
        ...
        .filter(Boolean) || [];
    const isAdmin = roleNames.some((role) => ["admin", "super admin", "sales admin", "sales manager"].includes(role));
...
function mapDealerOrderRequest(row) {
    const sale = row.sale || {};
    ...
    const decisionAt = (typeof approvalMeta.approvedAt === "string"
        ? approvalMeta.approvedAt
        : null) ||
        (typeof rejectionMeta.rejectedAt === "string"
            ? rejectionMeta.rejectedAt
    ...
    return {
        ...
        currentDeliveryCost: Number.isFinite(currentDeliveryCost)
            ...
            : null,
        decisionAt,
        decisionNote: (typeof approvalMeta.approverNote === "string"
            ? approvalMeta.approverNote
            : null) ||
            (typeof rejectionMeta.reason === "string" ? rejectionMeta.reason : null),
        ...
        sla: getDealerRequestSla({
            createdAt: row.createdAt,
            decisionAt: decisionAt || (row.status !== "pending" ? row.updatedAt : null),
            status: row.status,
        }),
    };
}
...
function getDealerRequestSla({ createdAt, decisionAt, status, now = new Date(), }) {
    ...
    const ageHours = created
        ...
        : 0;
    const dueAt = created
        ? new Date(created.getTime() + DEALER_REQUEST_SLA_HOURS * 3_600_000).toISOString()
        : null;
    const slaStatus = status !== "pending"
        ? "resolved"
...
function summarizeDealerRequestSla(rows, now = new Date()) {
    ...
    return {
        ...
        approved,
        rejected,
        averageDecisionHours: decided > 0 ? Math.round((totalDecisionHours / decided) * 10) / 10 : 0,
...
async function getDealerOrderRequests(db, userId, input = {}) {
    ...
    const cursor = Number(input.cursor || 0);
    const scope = await getSalesRequestUserScope(db, userId);
    const where = dealerOrderRequestWhere(userId, scope.canReviewUnassigned, input);
    const select = {
        id: true,
        request: true,
        status: true,
    ...
    if (input.status && input.status !== "all") {
        ...
        const historicalRows = historicalTake
            ...
            : [];
        rows = [...pendingRows, ...historicalRows];
    }
...
async function approveDealerOrderRequest(db, userId, requestId, input = {}) {
    return db.$transaction(async (tx) => {
        ...
        const deliveryCost = input.deliveryCost == null
            ? null
            : Math.max(0, Number(input.deliveryCost));
        const requiresDeliveryReview = ["delivery", "ship"].includes(deliveryOption.toLowerCase());
        ...
        const lineItems = (row.sale.items || []).map((item, index) => ({
        ...
        const approvalPricing = calculateDealerApprovalPricing({
            ...
            dealerSalesPercentage: finitePricingNumber(row.sale.dealerSale?.dealerSalesPercentage),
            fallbackDealerGrandTotal: Number(row.sale.dealerSale?.grandTotal ?? row.sale.grandTotal ?? 0),
        });
        let order = row.sale.type === "order"
            ? {
                id: row.sale.id,
        ...
        let finalAmountDue = Number(alreadyApproved
        ...
        if (!order) {
            ...
            order = await convertDealerPortalQuoteToOrderTx(tx, row.sale.dealerAuthId, row.sale.id, {
        ...
        if (!alreadyApproved) {
            ...
            const currentMeta = currentOrder?.meta &&
            ...
            const deliveryDelta = deliveryCost == null ? 0 : deliveryCost - previousDeliveryCost;
            ...
            const approvalMeta = deliveryCost != null && deliveryExtraCostId != null
            ...
            await tx.dealerSales.updateMany({
                where: {
        ...
        return {
            ...
            paymentContext: {
                ...
                customerPhone: row.sale.billingAddress?.phoneNo ||
...
async function rejectDealerOrderRequest(db, userId, requestId, reason) {
    return db.$transaction(async (tx) => {
        ...
        const currentMeta = row.sale.meta &&
...
async function saveDealerPortalSettings(db, dealerId, input) {
    return db.$transaction(async (tx) => {
        ...
        const currentMeta = dealer.meta &&
            typeof dealer.meta === "object" &&
            !Array.isArray(dealer.meta)
            ? dealer.meta
            : {};
        const currentBrandingVersion = typeof currentMeta.brandingVersion === "number"
        ...
        const defaultFulfillmentMode = normalizeDealerDefaultFulfillmentMode(input.defaultFulfillmentMode);
