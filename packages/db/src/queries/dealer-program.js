...
export function getDealerInvitationRetryAt(invitation, now = new Date()) {
    ...
    const anchor = invitation.deliveryStatus === "PENDING"
        ? invitation.createdAt
        : invitation.deliveredAt || invitation.createdAt;
    if (!anchor)
        return null;
    const delay = invitation.deliveryStatus === "PENDING"
...
export function resolveDealerPartnershipSummary(input) {
    ...
    if (input.dealer) {
        ...
        else if (input.dealer.restricted ||
        ...
        state = `APPLICATION_${input.application.status}`;
        blockingReason =
        ...
        const invitationCampaignActive = input.invitation.campaign.status === "ACTIVE" &&
            campaignIsInWindow(input.invitation.campaign, now);
        if (input.invitation.deliveryStatus === "FAILED" ||
...
export async function resolveDealerRecruitmentBanner(db, input) {
    ...
    const selectedProfiles = new Set(campaign.profiles.map((profile) => profile.customerProfileId));
    const selectedCustomers = new Set(campaign.customers.map((entry) => entry.customerId));
    const audienceMatches = campaign.audienceMode === "ALL_ELIGIBLE" ||
        selectedCustomers.has(customer.id) ||
        (customer.customerTypeId != null &&
            selectedProfiles.has(customer.customerTypeId));
    if (!isDealerRecruitmentCandidate({
...
function sanitizeDeliveryFailure(value) {
    if (!value)
        return null;
    return (value
        .replace(/[\r\n\t]+/g, " ")
...
export async function getDealerPartnershipSummaries(db, customers, input) {
    ...
    const [activeCampaign, invitations, applications, dealers] = await Promise.all([
        db.dealerRecruitmentCampaign.findFirst({
    ...
    const dealerByCustomer = new Map();
    for (const customer of customers) {
        const email = normalizedEmail(customer.email);
        const dealer = dealers.find((entry) => entry.dealerId === customer.id ||
            (Boolean(email) && normalizedEmail(entry.email) === email));
        if (dealer)
            dealerByCustomer.set(customer.id, dealer);
    }
...
export async function sendDirectDealerProgramInvitation(db, actorId, input, deliver) {
    ...
    if (acquired.count !== 1) {
        throw new DealerProgramInvitationError("An invitation is already being sent for this customer.", "CONFLICT");
    }
    try {
        const prepared = await db.$transaction(async (tx) => {
            ...
            if (!campaign || !campaignIsInWindow(campaign, now)) {
                throw new DealerProgramInvitationError("An active recruitment campaign is required.", "INELIGIBLE");
            }
            if (!customer) {
                throw new DealerProgramInvitationError("This customer cannot receive a partnership invitation.", "NOT_FOUND");
            }
            const email = validEmail(customer.email);
            if (!email) {
                throw new DealerProgramInvitationError("Add a valid customer email before sending an invitation.", "INELIGIBLE");
            }
            if (customer.auth || application) {
                throw new DealerProgramInvitationError("This customer already has a dealer account or application.", "INELIGIBLE");
            }
            ...
            if (emailConflict) {
                throw new DealerProgramInvitationError("A dealer account already uses this email address.", "INELIGIBLE");
            }
            ...
            if (previous) {
                const previousCampaignActive = previous.campaign.status === "ACTIVE" &&
                    campaignIsInWindow(previous.campaign, now);
                const retryAt = previous.expiresAt <= now || !previousCampaignActive
                    ? null
                    : getDealerInvitationRetryAt(previous, now);
                if (retryAt) {
                    throw new DealerProgramInvitationError(`This invitation can be resent after ${retryAt.toISOString()}.`, "TOO_EARLY");
                }
            }
            const rawToken = createDealerProgramInvitationToken();
        ...
        try {
            result = await deliver({
                to: prepared.email,
                customerName: prepared.customer.businessName ||
        ...
        await db.$transaction(async (tx) => {
            const ownsCurrentLease = await tx.dealerRecruitmentCustomerState.findFirst({
                where: {
            ...
            const deliveryUpdate = await markDealerRecruitmentInvitationDelivery(tx, prepared.invitation.id, { ...result, attemptedAt });
...
export async function setDealerRecruitmentCampaignStatus(db, actorId, input) {
    return db.$transaction(async (tx) => {
        ...
        if (!campaign)
            throw new Error("Recruitment campaign was not found.");
...
export async function submitDealerProgramApplication(db, rawToken) {
    ...
    return db.$transaction(async (tx) => {
        ...
        if (previous)
            return { application: previous, created: false };
...
export async function decideDealerProgramApplication(db, actorId, input) {
    return db.$transaction(async (tx) => {
        ...
        if (existingDealer?.dealerId && existingDealer.dealerId !== customer.id) {
            throw new Error("Another dealer account already uses this applicant email.");
        }
...
export async function setDealerAccountSuspension(db, actorId, input) {
    return db.$transaction(async (tx) => {
        ...
        if (!dealer)
            throw new Error("Dealer account was not found.");
