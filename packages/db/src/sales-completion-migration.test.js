...
const migrationPath = new URL("./migrations/20260901200350_add_sales_completion_record/migration.sql", import.meta.url);
...
describe("status-only sales completion database contract", () => {
    test("creates an append-only completion record with idempotency and one-active-record constraints", () => {
        ...
        expect(migration).toContain("UNIQUE INDEX `SalesCompletionRecord_requestId_key`(`requestId`)");
        expect(migration).toContain("UNIQUE INDEX `SalesCompletionRecord_cancellationRequestId_key`(`cancellationRequestId`)");
        expect(migration).toContain("UNIQUE INDEX `SalesCompletionRecord_activeKey_key`(`activeKey`)");
        expect(salesSchema).toContain("model SalesCompletionRecord {");
        expect(salesSchema).toContain("order                 SalesOrders                @relation(fields: [salesOrderId], references: [id], onDelete: Restrict)");
    });
    ...
    test("does not seed completion records or mutate operational workflow tables", () => {
        ...
        for (const operationalTable of [
            ...
            expect(migration).not.toMatch(new RegExp(`(?:INSERT|UPDATE|DELETE|ALTER)\\s+(?:INTO\\s+|FROM\\s+|TABLE\\s+)?\\\`${operationalTable}\\\``, "i"));
        }
    });
});
