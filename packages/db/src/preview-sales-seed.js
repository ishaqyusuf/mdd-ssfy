...
export function quotePreviewIdent(identifier) {
...
export function parsePreviewSalesSeedArgs(argv, env = process.env) {
    ...
    const sourceUrl = values.get("--source-url") ??
        ...
        env.DATABASE_URL ??
        "";
    const targetUrl = values.get("--target-url") ?? env.PREVIEW_DATABASE_URL ?? "";
    const limit = parseInteger(values.get("--limit"), 150);
    if (limit < MIN_ORDER_LIMIT || limit > MAX_ORDER_LIMIT) {
        throw new Error(`--limit must be between ${MIN_ORDER_LIMIT} and ${MAX_ORDER_LIMIT}`);
    }
    if (!sourceUrl)
        throw new Error("Missing --source-url (or SOURCE_DATABASE_URL/DATABASE_URL)");
    if (!targetUrl)
...
export function databaseCredentialFingerprint(databaseUrl) {
...
export function redactPreviewDatabaseUrl(databaseUrl) {
    const parsed = new URL(databaseUrl);
    return `${parsed.protocol}//<redacted>@${parsed.hostname}${parsed.pathname}`;
}
export function assertSafePreviewSeedConnections(options) {
    const source = new URL(options.sourceUrl);
    const target = new URL(options.targetUrl);
    if (!LOCAL_HOSTS.has(source.hostname)) {
        throw new Error(`Preview seed source must be a local database, received ${source.hostname}`);
    }
    if (!target.hostname.endsWith("psdb.cloud")) {
        throw new Error(`Preview seed target must be PlanetScale, received ${target.hostname}`);
    }
    if (source.href === target.href)
        throw new Error("Preview seed source and target must be different databases");
    const fingerprint = databaseCredentialFingerprint(options.targetUrl);
    if (!options.dryRun && !options.expectedTargetFingerprint) {
        throw new Error("Writes require --expect-target-fingerprint from a successful dry run");
    }
    if (options.expectedTargetFingerprint &&
        options.expectedTargetFingerprint !== fingerprint) {
        throw new Error("Target credential fingerprint does not match the confirmed dry-run target");
    }
    return fingerprint;
}
...
export function getModelKeyFields(model) {
    if (model.primaryKey?.fields.length)
        return model.primaryKey.fields;
    if (model.uniqueFields[0]?.length)
        return model.uniqueFields[0];
    const field = model.fields.find((candidate) => candidate.isId || candidate.isUnique);
    return field ? [field.name] : [];
}
function rowKey(model, row) {
...
function addRows(selection, model, rows, maxRows = Number.POSITIVE_INFINITY) {
    ...
    for (const row of rows) {
        ...
        if (bucket.size >= maxRows) {
            throw new Error(`Preview seed exceeded the ${maxRows}-row safety cap for ${model.name}`);
        }
        bucket.set(key, row);
        added += 1;
    }
...
function buildPreviewChildEdges(models) {
    const edges = [];
    ...
    while (queue.length) {
        ...
        if (!parentModel)
            continue;
        for (const backRelation of parentModel.fields.filter((field) => field.kind === "object" &&
            !field.relationFromFields?.length &&
            PREVIEW_CHILD_MODELS.has(field.type))) {
            const childModel = models.get(backRelation.type);
            if (!childModel)
                continue;
            const childRelation = childModel.fields.find((field) => field.kind === "object" &&
...
export function selectScenarioBalancedOrders(rows, limit) {
...
function sanitizeJson(value, key = "") {
    ...
    if (/phone|mobile/i.test(key))
        return "5550000000";
    if (/address|recipient|customer.?name/i.test(key))
        return "Preview data";
    return value;
}
export function sanitizePreviewRow(modelName, row) {
    const sanitized = Object.fromEntries(Object.entries(row).map(([key, value]) => [
        key,
...
function selectColumns(model) {
    return scalarFields(model)
        .map((field) => `${quotePreviewIdent(fieldColumn(field))} AS ${quotePreviewIdent(field.name)}`)
        .join(", ");
}
async function fetchRowsByTuples(client, model, fields, tuples) {
    const valid = tuples.filter((tuple) => tuple.length === fields.length && tuple.every((value) => value != null));
    if (valid.length === 0)
        return [];
    const results = [];
    const columns = fields.map((fieldName) => {
        ...
        if (!field)
            throw new Error(`Model ${model.name} does not contain field ${fieldName}`);
    ...
    for (let start = 0; start < valid.length; start += RELATION_BATCH_SIZE) {
        ...
        results.push(...(await client.$queryRawUnsafe(query, ...batch.flat())));
    }
    return results;
}
function relationValues(rows, fields) {
    const seen = new Set();
    const tuples = [];
    for (const row of rows) {
...
async function collectSelection(source, limit) {
    ...
    const candidates = await source.$queryRawUnsafe(`SELECT ${selectColumns(salesModel)} FROM ${quotePreviewIdent(modelTable(salesModel))} WHERE ${quotePreviewIdent("deletedAt")} IS NULL ORDER BY ${quotePreviewIdent("createdAt")} DESC, ${quotePreviewIdent("id")} DESC LIMIT ?`, Math.min(limit * 20, 4000));
    const orders = selectScenarioBalancedOrders(candidates, limit);
    if (orders.length < MIN_ORDER_LIMIT) {
        throw new Error(`Local source has only ${orders.length} eligible sales orders; at least ${MIN_ORDER_LIMIT} are required`);
    }
    const selection = new Map();
    addRows(selection, salesModel, orders);
    const internalUsers = await source.$queryRawUnsafe(`SELECT ${selectColumns(usersModel)} FROM ${quotePreviewIdent(modelTable(usersModel))} WHERE ${quotePreviewIdent("deletedAt")} IS NULL AND (${quotePreviewIdent("type")} IS NULL OR ${quotePreviewIdent("type")} <> 'CUSTOMER') ORDER BY ${quotePreviewIdent("id")} LIMIT ?`, USER_LIMIT);
    ...
    for (let round = 0; round < 10; round += 1) {
        ...
        for (const { parentName, childModel, childRelation } of childEdges) {
            ...
            if (parentRows.length === 0)
                continue;
            const tuples = relationValues(parentRows, childRelation.relationToFields ?? []);
            const rows = await fetchRowsByTuples(source, childModel, [...(childRelation.relationFromFields ?? [])], tuples);
        ...
        for (const [modelName, bucket] of [...selection.entries()]) {
            ...
            for (const relation of model.fields.filter((field) => field.kind === "object" && field.relationFromFields?.length)) {
                if (relation.type === "SalesOrders")
                    continue;
                if (BLOCKED_PARENT_MODELS.has(relation.type))
                    continue;
                const parentModel = models.get(relation.type);
                if (!parentModel)
                    continue;
                const tuples = relationValues(rows, relation.relationFromFields ?? []);
                const parents = await fetchRowsByTuples(source, parentModel, [...(relation.relationToFields ?? [])], tuples);
        ...
        if (added === 0)
            break;
        if (round === 9)
            throw new Error("Preview seed relation closure did not converge within 10 passes");
    }
    return selection;
}
export function normalizeWriteValue(value) {
    if (typeof value === "bigint")
...
async function upsertModelRows(target, model, rows) {
    if (rows.length === 0)
        return 0;
    const fields = scalarFields(model).filter((field) => rows.some((row) => Object.hasOwn(row, field.name)));
    ...
    for (let start = 0; start < rows.length; start += WRITE_BATCH_SIZE) {
        ...
        const updates = columns
            .map((column) => `${quotePreviewIdent(column)} = VALUES(${quotePreviewIdent(column)})`)
            .join(", ");
        ...
        const values = batch.flatMap((row) => fields.map((field) => normalizeWriteValue(row[field.name])));
        await target.$executeRawUnsafe(sql, ...values);
        written += batch.length;
    }
    return written;
}
function insertionOrder(selection, models) {
...
async function countRows(client, model) {
    const rows = await client.$queryRawUnsafe(`SELECT COUNT(*) AS count FROM ${quotePreviewIdent(modelTable(model))}`);
    return Number(rows[0]?.count ?? 0);
}
export async function seedPreviewSales(options) {
    ...
    try {
        ...
        if (targetOrdersBefore > 0 && !options.allowExisting) {
            throw new Error(`Preview target already contains ${targetOrdersBefore} sales orders; pass --allow-existing only for an intentional repeatable upsert`);
        }
        const selection = await collectSelection(source, options.limit);
        const rowsByModel = Object.fromEntries([...selection.entries()].map(([name, rows]) => [name, rows.size]).sort());
        ...
        if (!options.dryRun) {
            for (const name of insertionOrder(selection, models)) {
                ...
                if (!model)
                    continue;
                const rows = [...(selection.get(name)?.values() ?? [])].map((row) => sanitizePreviewRow(name, row));
