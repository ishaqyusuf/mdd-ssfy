...
import { assertSafePreviewSeedConnections, databaseCredentialFingerprint, normalizeWriteValue, parsePreviewSalesSeedArgs, sanitizePreviewRow, selectScenarioBalancedOrders, } from "./preview-sales-seed";
const localUrl = "mysql://root:local@127.0.0.1:3306/gnd";
const previewUrl = "mysql://preview-user:preview-password@aws.connect.psdb.cloud/gndprodesk?sslaccept=strict";
describe("preview sales seed safety", () => {
    test("requires a local source and PlanetScale target", () => {
        ...
        expect(assertSafePreviewSeedConnections(options)).toBe(databaseCredentialFingerprint(previewUrl));
        expect(() => assertSafePreviewSeedConnections({ ...options, sourceUrl: previewUrl })).toThrow("source must be a local database");
        expect(() => assertSafePreviewSeedConnections({ ...options, targetUrl: localUrl })).toThrow("target must be PlanetScale");
    });
    test("requires the dry-run credential fingerprint before writes", () => {
        ...
        expect(() => assertSafePreviewSeedConnections(options)).toThrow("Writes require --expect-target-fingerprint");
        expect(() => assertSafePreviewSeedConnections({
            ...options,
            expectedTargetFingerprint: "wrong",
        })).toThrow("fingerprint does not match");
    });
    test("enforces a 100-200 order dataset", () => {
        expect(() => parsePreviewSalesSeedArgs([
            "--source-url",
            localUrl,
            "--target-url",
            previewUrl,
