          ...
          {section.rows.map((row, ri) => (
            ...
            >
              {row.cells.map((cell, ci) => (
                <View
                  key={ci}
                  style={{...cn(`border uppercase`), flex: 1 }}
                >
                  <Text
                    style={cn(`p-1 ${cell.bold ? "font-bold" : ""} ${cell.align === "right" ? "text-right" : cell.align === "center" ? "text-center" : ""} ${row.isGroupHeader ? "text-center uppercase" : ""}`)}
                  >
                    {cell.value ?? ""}
                  </Text>
                </View>
