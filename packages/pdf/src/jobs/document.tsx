import { Document, Font, Image, Text, View } from "@react-pdf/renderer";
...
export function JobsPdfDocument({
	...
	const contextLabel =
		data.context === "payment-portal"
			...
			: data.context === "payroll-report"
				? "Payroll Report"
				: "Jobs List";
	const summaryJobLabel =
		data.context === "payroll-report" ? "Unpaid Jobs" : "Selected Jobs";
	const summaryAmountLabel =
		data.context === "payroll-report" ? "Total Payable" : "Total Amount";
	const footerAmountLabel =
		data.context === "payroll-report" ? "Total Payable" : "Print Total";
	const footerCountLabel =
		data.context === "payroll-report" ? "unpaid jobs" : "jobs selected";
		...
		<Document title={title || data.title}>
			...
			>
				...
				>
					...
					>
						...
						>
							...
							>
								...
								<MetaLine
									label="Printed"
									value={formatPrintDate(data.printedAt)}
									light
								/>
								<MetaLine
									label="Jobs"
					...
					>
						<MetricCard
							label={summaryJobLabel}
							value={String(data.summary.jobCount)}
						/>
						<MetricCard
							label={summaryAmountLabel}
							value={formatCurrency(data.summary.totalAmount)}
						/>
						<MetricCard
				...
				>
					...
					<View>
						<Text style={{ fontSize: 9, color: "#ffffff", textAlign: "right" }}>
							{data.summary.jobCount} {footerCountLabel}
						</Text>
						<Text
							style={{
								fontSize: 9,
