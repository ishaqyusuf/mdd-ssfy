...
import { hasUpstashRestConfig, sendUpstashRestCommand } from "./upstash-rest";
...
export class RedisCache {
  ...
  private get useRest(): boolean {
    return !process.env.REDIS_URL && hasUpstashRestConfig();
  }
  ...
  async delete(key: string): Promise<void> {
    ...
    try {
      await withTimeout(
        this.useRest
          ? sendUpstashRestCommand(["DEL", this.getKey(key)])
  ...
  async healthCheck(): Promise<void> {
    ...
    try {
      await withTimeout(
        this.useRest
          ? sendUpstashRestCommand(["PING"])
          : this.redis.send("PING", []),
        "PING",
      );
      const elapsed = performance.now() - start;
