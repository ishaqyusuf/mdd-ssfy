...
import { buildSalesPaymentSummaryLines } from "../../payment-system/domain/payment-summary";
...
function appendPaidSummary(
	...
	for (const summaryLine of buildSalesPaymentSummaryLines(
		paymentState.paymentSummary,
	)) {
		lines.push({
			label: summaryLine.label,
			value:
				summaryLine.kind === "count"
...
export function composeFooter(
	...
	const extraCostTypes = new Set(applicableExtraCosts.map((cost) => cost.type));
