...
describe("legacy dispatch manifest item", () => {
  ...
  it("does not double-count legacy handled rows that repeat their total in qty", () => {
    const item = normalizeLegacyDispatchManifestItem({
      title: "Handled door",
      orderedQty: { qty: 1, lh: 1 },
      packedQty: { qty: 1, lh: 1 },
    });

    expect(item.orderedQty).toEqual({ qty: 0, lh: 1, rh: 0, total: 1 });
    expect(item.packedQty).toEqual({ qty: 0, lh: 1, rh: 0, total: 1 });
    expect(item.remainingQty.total).toBe(0);
  });
