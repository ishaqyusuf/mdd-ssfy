...
export const SALES_REQUEST_PROMPT_VERSION = "new-sales-form-seed-v11";
...
export function buildSalesRequestInstructions(
	...
	return [
		...
		"Select only source-stated facts; leave omitted optional choices absent for the application's default resolver. Do not add a No/None choice merely because the customer did not mention that option.",
		...
		"Before returning JSON, verify every selected step belongs to its line's route, every selected product matches the source, and every door line with HPT rows either selects one exact Door product or has a line-scoped unresolved Door entry. Return unresolved facts explicitly; do not silently drop them.",
	].join("\n");
}
