...
export const SALES_REQUEST_PROMPT_VERSION = "new-sales-form-seed-v18";
...
export function buildSalesRequestInstructions(
	...
	return [
		...sourceInstructions,
		"Produce the best useful partial draft the current catalog can support. This is an editable starting point that a sales representative will compare with the original request before saving; do not turn ordinary omitted configuration into blockers.",
		"Use explicit customer values and confirmed representative clarifications first. Apply provided business rules only where they do not conflict with those explicit values. Then use exact compatible catalog conversions. Leave routine omitted configuration absent so the application's deterministic default resolver can select the first eligible configured option.",
		"For each genuinely unresolved entry, give a concise actionable question in reason and quote the exact ambiguous customer phrase when available. Ask only when a conflict, unreadable value, or missing product identity prevents a defensible route or catalog match. Confirmed representative clarifications already supplied should resolve that field unless insufficient.",
		...
		"For pre-hung, exterior, or garage door routes, use customer-stated jamb size, left/right handing, and in/out swing when supplied. When an ordinary configured choice such as jamb size, handing, swing, finish, or bore is omitted, leave that form step absent without adding unresolved so the application's deterministic default resolver can select the first eligible option after route visibility is known. Do not emit one review per defaulted field; the application presents defaults as one compact review notice. Do not ask for an unsupported route field.",
		...
		"For a named-room door schedule, preserve each room's stated size and count in native HPT rows or an unresolved entry naming that room and size; bare line qty is not enough. Keep the exact room name in an interpretation source quote or unresolved reason so repeated sizes remain distinguishable. When a plausible door width is written with a single apostrophe in an otherwise complete named door-size row, such as Cabana 30' x 96\", treat it as the likely inches typo only when the exact converted size (30 x 96 inches, or 2-6 x 8-0) is available for that line's selected route and configuration. Create the compatible line and add an unsupported widthAssumption review quoting the source and conversion; do not ask a blocking unit question. If that exact size is unavailable, leave the row for Sales review instead of substituting another size. A blank room entry still needs a named dimension question.",
		"For a line-by-line architectural door schedule, keep one seed line per source door row, in source order, including repeated sizes. Give each a distinct uid and preserve its own size, handing and quantity; when a row cannot be configured, quote that exact row in a row-specific unsupported Sales review entry. Never collapse several separately listed doors into a grouped quantity or count an accessory as a door row.",
		...
		"Architectural slash notation states feet/inches, not a fraction: 2/8 8/0 RH states a 2-8 x 8-0 door with right handing; 2/10 6/8 states 2-10 x 6-8. A space between those two dimensions is valid without an x separator. Preserve these explicit dimensions even when the Door product is unresolved. Never replace an unavailable height, such as 8/8, with the closest available Height component; omit the unavailable selection and record the exact requested dimensions as an unsupported Sales review entry.",
		...
		"Only a fully pending Mouldings line may have qty:0. For any other line, qty must be positive. Preserve every explicit source quantity. When a non-Mouldings product is clearly identified but its quantity is omitted, use qty:1 as the editable draft minimum and do not ask a quantity question. If an accessory itself cannot be identified, keep that accessory unresolved rather than creating a speculative line.",
		"Never infer leaf count or handed quantities from an opening count, or infer swing from other facts.",
		"If width or height is absent, omit housePackageTool for that line and preserve the requested count in qty. If handed quantities are omitted on a handled route, also omit housePackageTool and preserve qty without asking a routine handing question; an explicit conflict may remain unresolved. Never emit placeholder dimensions such as 3-0 x 6-8 or invented left/right splits. Thickness is not width or height: 36 x 1 3/4 x 80 means width 36 inches, thickness 1 3/4 inches, height 80 inches.",
		...
		"Before returning JSON, verify every selected step belongs to its line's route, every inferred match has an interpretation, and every door line with HPT rows either selects one compatible Door product or has a line-scoped unresolved Door entry. Keep truly unmapped source lines as separate Sales review entries. Omit routine defaultable fields instead of repeating them as unresolved questions.",
	].join("\n");
}
