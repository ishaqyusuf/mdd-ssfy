...
export function resolveInteractiveWorkflowStepIndex(
	...
	if (!isWorkflowRedirectDisabledStep(steps[clampedIndex])) {
		return clampedIndex;
	}
	for (let index = clampedIndex + 1; index < steps.length; index += 1) {
		if (!isWorkflowRedirectDisabledStep(steps[index])) {
			return index;
		}
	}
...
export function resolveInitialWorkflowStepIndex(steps: WorkflowStepLike[]) {
	if (!steps.length) return 0;
	return resolveInteractiveWorkflowStepIndex(
		steps,
...
export function resolveComponentPriceByDeps(
	...
	const depValueGroups = deps
		.map((stepUid: string) => {
			const selectedAll = Array.isArray(
				options?.selectedProdUidsByStepUid?.[stepUid],
			)
				? (options?.selectedProdUidsByStepUid?.[stepUid] || []).map((uid) =>
						String(uid),
					)
				: [];
			if (selectedAll.length)
				return Array.from(new Set(selectedAll.filter(Boolean)));
	...
	const depCombos = cartesian(depValueGroups).map((values) =>
