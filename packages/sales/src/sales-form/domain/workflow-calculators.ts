...
function isShelfPlaceholderRow(row: any) {
	...
	return (
		!row?.productId && !description && basePrice == null && customPrice == null
	);
}
...
export function resolveSizeFromPricingKey(
	key: string,
	...
	if (supplierVariant) {
		...
		const normalizedMultiplier =
			Number.isFinite(multiplier) && multiplier > 0
				...
				: 1;
		const derivedSales =
			basePrice != null ? multiplyMoney(basePrice, normalizedMultiplier) : null;
...
export function summarizeDoors(
	...
	const totalPrice = sumMoney(
		normalized.map((row) => Number(row?.lineTotal || 0)),
	);
	return {
		rows: normalized,
		totalDoors,
...
export function summarizeServiceRows(lineUid: string, nextRowsRaw: any[]) {
	...
	const lineTotal = sumMoney(
		rows.map((row: any) => Number(row.lineTotal || 0)),
	);
	const groupPricing = authoritativeGroupPricing(lineTotal, qtyTotal);
...
export function summarizeShelfRows(
	...
	const lineTotal = sumMoney(
		activeRows.map((row: any) => Number(row.totalPrice || 0)),
	);
	const groupPricing = authoritativeGroupPricing(lineTotal, qtyTotal);
	return {
		rows,
...
export function buildShelfSections(
	...
	rows.forEach((row: any, index: number) => {
		...
		const parentCategoryId =
			Number(rowMeta?.shelfParentCategoryId || 0) || categoryIds[0] || null;
		const categoryId =
		...
		if (existing) {
			...
			existing.subTotal = sumMoney(
				existing.rows.map((sectionRow: any) =>
...
export function deriveMouldingRows({
	...
	return initialRows.map((row: any) => {
		...
		return {
			...
			description:
				row.description || component?.title || row.title || "Moulding",
...
export function summarizeMouldingPersistRows(
	nextRowsRaw: any[],
	sharedComponentPrice: number,
) {
	...
	const total = sumMoney(
		calculated.map((row: any) => Number(row.lineTotal || 0)),
	);
	const groupPricing = authoritativeGroupPricing(total, qtyTotal);
	return {
