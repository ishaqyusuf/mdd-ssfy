...
> = {
	...
	onEditPricing?: (component: TComponent) => void;
...
>(props: WorkflowStepComponentPanelProps<TComponent>) {
	const [managementSelection, setManagementSelection] = useState<Set<string>>(
		new Set(),
	);
	...
	const toggleManagementSelection = (component: TComponent) => {
		const uid = String(component.uid || "");
		if (!uid) return;
		setManagementSelection((current) => {
	...
	return (
		<>
			<StepComponentPicker
				...
				renderComponent={(component) => {
					...
					return (
						<WorkflowComponentCard
							...
							selectionSlot={
								managementSelection.size ? (
									<Checkbox
										checked={isManagementSelected}
										aria-label={`Select ${props.componentLabel(component.title)} for catalog management`}
										onCheckedChange={() => toggleManagementSelection(component)}
									/>
								) : null
							}
							actionsSlot={
								canManageCatalog && !managementSelection.size ? (
									<WorkflowComponentActionMenu
										...
										onEditDetails={
											props.onEditDetails
												? () => props.onEditDetails?.(component)
												: undefined
										}
										onEditVisibility={
											props.onEditVisibility
												? () => props.onEditVisibility?.([component])
												: undefined
										}
										onEditPricing={
											props.onEditPricing
												? () => props.onEditPricing?.(component)
												: undefined
										}
										showPricing={canManageCatalog}
										pricingDisabled={isDoorStep}
						...
						>
							{props.isMouldingSelectionStep && !managementSelection.size ? (
							...
							managementSelection.size ? (
								<div className="flex flex-wrap items-center justify-end gap-2">
									<span className="text-sm font-medium">
										{managementSelection.size} selected
									</span>
									{props.onEditVisibility ? (
										<Button
											type="button"
											variant="outline"
											onClick={() =>
												props.onEditVisibility?.(selectedManagementComponents)
											}
										>
											Edit Visibility
										</Button>
									...
									{props.onArchive ? (
										<Button
											...
											variant="destructive"
											onClick={() =>
												setArchiveTargets(selectedManagementComponents)
											}
										>
											Delete
										</Button>
									...
									<Button
										type="button"
										variant="ghost"
										onClick={() => setManagementSelection(new Set())}
									>
										Unmark All
									</Button>
								</div>
			...
			<Dialog
				...
				onOpenChange={(open) => {
					if (!open && !archiving) setArchiveTargets([]);
				}}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Delete workflow components?</DialogTitle>
						<DialogDescription>
							This archives {archiveTargets.length} component
							{archiveTargets.length === 1 ? "" : "s"} from future pickers.
							Existing saved sales keep their snapshots.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						<Button
						...
						<Button
							...
							onClick={async () => {
								...
								setArchiving(true);
								try {
									const archived = await props.onArchive(archiveTargets);
									if (archived !== false) {
										setManagementSelection(new Set());
										setArchiveTargets([]);
									}
