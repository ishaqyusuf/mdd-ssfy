...
export async function submitProductionAssignmentInTransaction(
  ...
  if (!assignment.assignedToId) {
    throw new Error(
      "Production assignment must have an active assigned worker.",
    );
  }
  if (
  ...
  if (review.state === "pending_material_review") {
    await refreshProductionSubmissionAssignmentScope(tx, review.reviewId);
  }
  return {
