...
it("uses one completed tone for audited status-only and operational completion", () => {
	expect(getDispatchCalendarPresentation("administratively_completed", "queue")).toEqual({ tone: "completed", label: "Marked as completed", completed: true });
	expect(getDispatchCalendarPresentation("fulfilled", "fulfilled")).toEqual({ tone: "completed", label: "Fulfilled", completed: true });
	expect(matchesDispatchCalendarStages(["fulfilled"], "ready_to_assign", "administratively_completed")).toBe(true);
	expect(matchesDispatchCalendarStages(["ready_to_assign"], "ready_to_assign", "administratively_completed")).toBe(false);
});
