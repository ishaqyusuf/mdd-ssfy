...
function itemMetaRecord(item: DataItem): Record<string, any> {
	return item?.meta &&
...
function printableShelfItems(item: DataItem) {
	return item.shelfItems?.length
		? item.shelfItems
		: metadataArray(itemMetaRecord(item).shelfItems);
}

function printableHousePackageTool(item: DataItem) {
	if (item.housePackageTool) return item.housePackageTool;
	const metaHpt = itemMetaRecord(item).housePackageTool;
	return metaHpt && typeof metaHpt === "object" && !Array.isArray(metaHpt)
		? metaHpt
		: null;
}
...
function transformSalesPrint(data: Data) {
	...
	data.items
		...
		.map((item) => {
			// shelf items...
			const shelfItems = printableShelfItems(item);
			if (shelfItems?.length) {
				...
				const section: PrintLineSection = {
					...
					tableHeader: [
						...
						.map((b) => b!) as any,
					tableRows: shelfItems.map((sItem) => {
						...
						if (is.moulding || is.service) {
							...
							return mItem.housePackageTool?.doors?.map((door) => {
								...
								row["Total"] = cell(
									formatCurrency(
										door?.lineTotal ||
											sum([door?.unitPrice! * door?.totalQty!]),
									),
								);
...
function composeDykeItems(item: DataItem, data: Data) {
	...
	const multies = _multies.map((multiItem) => ({
		...multiItem,
		housePackageTool: printableHousePackageTool(multiItem),
	}));
	const itemMeta = item!.meta as any as SalesItemMeta;
