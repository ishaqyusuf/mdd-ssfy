...
export type GuardedPackingLine = {
  productionSubmissionId: number | null;
...
export function removeGuardedQuantitiesFromPackingRequests<
  T extends {
    salesItemId: number;
    itemUid?: string;
    qty: Quantity;
  },
>(requests: T[], guardedLines: GuardedPackingLine[]) {
  return requests
    .map((request) => {
      const guardedQty = recomposeQty(
        qtyMatrixSum(
          ...guardedLines
            .filter(
              (line) =>
                line.salesOrderItemId === request.salesItemId &&
                line.itemUid === request.itemUid,
            )
            .map((line) => ({
              qty: line.qty,
              lh: line.lhQty,
      ...
      const qty = normalizedQty(
        qtyMatrixDifference(normalizedQty(request.qty), guardedQty),
      );
      return { ...request, qty };
    })
    .filter((request) => hasQty(request.qty));
}
...
export function buildPackAllTarget(
  ...
  const orderedQty = normalizedQty(item.totalQty || {});
  ...
  if (singleQuantity) {
    const sourceTotal = Math.max(0, quantityTotal(sourceQty));
    return {
      qty: hasQty(orderedQty)
        ? Math.min(sourceTotal, quantityTotal(orderedQty))
        : sourceTotal,
      lh: 0,
      rh: 0,
    };
  ...
  return {
    qty: 0,
    lh: Math.min(
      Math.max(0, Number(sourceQty.lh || 0)),
      hasQty(orderedQty)
        ? Math.max(0, Number(orderedQty.lh || 0))
        : Number.POSITIVE_INFINITY,
    ),
    rh: Math.min(
      Math.max(0, Number(sourceQty.rh || 0)),
      hasQty(orderedQty)
        ? Math.max(0, Number(orderedQty.rh || 0))
        : Number.POSITIVE_INFINITY,
    ),
  };
}
...
export function buildGuardedPackingPlan(
  ...
  for (const item of items) {
    ...
    const guardedSubmissionIds = new Set(
      guardedCandidates
        .map((line) => line.productionSubmissionId)
    ...
    for (const line of guardedCandidates) {
      ...
      if (hasQty(picked.picked)) {
        ...
        guardedLines.push({
          ...
          itemUid: line.itemUid || item.itemUid,
