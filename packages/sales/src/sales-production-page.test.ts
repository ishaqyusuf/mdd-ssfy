...
type Selection = {
	[key: string]: boolean | { select?: Selection; where?: unknown };
};
type Query = {
...
function project(value: unknown, selection: Selection): unknown {
	...
	if (Array.isArray(value))
		return value.map((item) => project(item, selection));
	...
	return Object.fromEntries(
		Object.entries(selection)
			...
			.map(([key, pick]) => [
				key,
				typeof pick === "object" && pick.select
					? project(source[key], pick.select)
					: source[key],
			]),
	);
}
		...
		itemControls: [
			{
				produceable: true,
				shippable: true,
				qtyControls: [{ type: "prodCompleted", itemTotal: 5, total: 0 }],
				assignments: [{ id }],
			},
		],
		assignments: [
...
function projection(row: ReturnType<typeof order>) {
	return {
		salesOrderId: row.id,
...
describe("Production page evidence contract", () => {
	it(
		"loads scoped details concurrently and preserves page order and populated evidence",
		() =>
			inLegacyMode(async () => {
				...
				const db = {
					salesOrderListProjection: {
						findMany: async () => rows.map(projection),
					},
					salesOrders: {
						findMany: async (query: Query) => {
							if (query.take === 250) return [];
							...
							const pageDetail = anchorWhere !== undefined;
							if (query.select.customer)
								expect(query.where?.AND).toEqual([anchorWhere]);
							if (pageDetail && query.select.assignments)
								expect(JSON.stringify(query.select.assignments)).toContain(
									'"assignedToId":17',
								);
							if (!pageDetail)
								return [...rows]
	...
	it.each(["customer", "itemControls", "assignments", "deliveries"])(
		"rejects a missing %s branch instead of returning a partial page",
		(missing) =>
			inLegacyMode(async () => {
				...
				const db = {
					salesOrderListProjection: {
						findMany: async () => [projection(source)],
					},
					salesOrders: {
						findMany: async (query: Query) => {
							if (query.take === 250) return [];
							...
							const pageDetail = anchored;
							return pageDetail && query.select[missing]
								? []
								: [project(source, query.select)];
						},
					},
				};
				await expect(
					getSalesProductions(db as unknown as Db, {
						size: 20,
						includeMaterials: false,
					}),
				).rejects.toThrow(
					"Production order evidence changed. Refresh and retry.",
				);
			}),
	);

	it("rejects a replacement order even when all branch lengths match", () =>
		inLegacyMode(async () => {
			const source = order(1);
			let anchored = false;
			const db = {
				...
				salesOrders: {
					findMany: async (query: Query) => {
						...
						return [
							project(
								order(pageDetail && query.select.assignments ? 2 : 1),
								query.select,
							),
						];
					},
