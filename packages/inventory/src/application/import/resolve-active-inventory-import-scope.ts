...
type StepRecord = Awaited<ReturnType<typeof loadStepsForImportScope>>[number];
...
export async function resolveActiveInventoryImportScope(
		...
		for (const pricing of step.priceSystem || []) {
			...
			for (const token of rawDep.split("-").filter(Boolean)) {
				...
				if (token.startsWith("w")) {
					const widthStep = steps.find(
						(candidate) => candidate.title === "Width",
					);
	...
	const dependencyStepUids = activeStepUids.filter(
		(uid) => !baseStepUids.has(uid),
	);
