...
export const inventoryImportSourceReviewSchema = z.object({
	limit: z.number().int().positive().max(200).optional().default(100),
	inventoryIds: z.array(z.number().int().positive()).max(200).optional(),
});
export type InventoryImportSourceReview = z.infer<
	typeof inventoryImportSourceReviewSchema
>;

export const inventoryImportSourceArchiveSchema = z.object({
	inventoryIds: z.array(z.number().int().positive()).min(1).max(200),
	apply: z.boolean().optional().default(false),
});
export type InventoryImportSourceArchive = z.infer<
	typeof inventoryImportSourceArchiveSchema
>;

export const inventoryImportSourceDispositionSchema = z.object({
	inventoryId: z.number().int().positive(),
	targetCategoryId: z.number().int().positive(),
	disposition: z
		.enum(["retain_as_inventory", "retain_as_custom"])
		.optional()
		.default("retain_as_inventory"),
	baseline: z.object({
		categoryId: z.number().int().positive(),
		sourceStepUid: z.string().nullable(),
		sourceComponentUid: z.string().nullable(),
		sourceCustom: z.boolean(),
	}),
});
export type InventoryImportSourceDisposition = z.infer<
	typeof inventoryImportSourceDispositionSchema
>;

export const inventoryImportSourceDispositionBatchSchema = z.object({
	items: z
		.array(inventoryImportSourceDispositionSchema)
		.min(1)
		.max(25)
...
export type InventoryImportSourceDispositionBatch = z.infer<
	typeof inventoryImportSourceDispositionBatchSchema
>;

export const inventoryImportCategoryCleanupReviewSchema = z.object({
	limit: z.number().int().positive().max(100).optional().default(50),
	categoryIds: z.array(z.number().int().positive()).min(1).max(100).optional(),
});
export type InventoryImportCategoryCleanupReview = z.infer<
	typeof inventoryImportCategoryCleanupReviewSchema
>;

export const inventoryImportCategoryCleanupSchema = z.object({
	categoryIds: z.array(z.number().int().positive()).min(1).max(100),
	apply: z.boolean().optional().default(false),
});
export type InventoryImportCategoryCleanup = z.infer<
	typeof inventoryImportCategoryCleanupSchema
>;
...
export const inventoryImportRunHistorySchema = z.object({
	limit: z.number().int().positive().max(20).optional().default(8),
});
export type InventoryImportRunHistory = z.infer<
	typeof inventoryImportRunHistorySchema
>;

export const inventoryImportProjectionHistorySchema = z.object({
	limit: z.number().int().positive().max(20).optional().default(8),
});
export type InventoryImportProjectionHistory = z.infer<
	typeof inventoryImportProjectionHistorySchema
>;

export const inventoryImportProjectionRetrySchema = z.object({
	diagnosticId: z.string().trim().min(1).max(191),
});
export type InventoryImportProjectionRetry = z.infer<
	typeof inventoryImportProjectionRetrySchema
>;
...
export type DeleteInventorySupplier = z.infer<
	typeof deleteInventorySupplierSchema
>;
