...
const SEED_FIXTURE_ID = "INV-FIX-INBOUND";
...
const FIXTURE = {
	supplierUid: "inv-fix-inbound-supplier",
	supplierName: "Inventory Validation Inbound Supplier",
	categoryUid: "inv-fix-inbound-category",
	categoryTitle: "Inventory Validation Inbound",
	itemUid: "inv-fix-inbound-item",
	itemName: "Inventory Validation Inbound Item",
	variantUid: "inv-fix-inbound-variant",
	variantSku: "INV-FIX-INBOUND",
	saleOrderId: "INV-FIX-INBOUND",
	saleSlug: "inv-fix-inbound",
	lineUid: "inv-fix-inbound-line",
	lineTitle: "Inventory Validation Inbound Line",
	shipmentReference: "INV-FIX-INBOUND-SHIPMENT",
};
...
function usage() {
	console.log(
		[
			"Usage:",
			"  bun scripts/inventory-validation-seed-inbound-fixture.ts",
			"  bun scripts/inventory-validation-seed-inbound-fixture.ts --json",
			"  bun scripts/inventory-validation-seed-inbound-fixture.ts --apply",
			"  bun scripts/inventory-validation-seed-inbound-fixture.ts --rollback",
			"  bun scripts/inventory-validation-seed-inbound-fixture.ts --rollback --apply",
			"",
			"Options:",
			"  --apply     Write or repair the inbound validation fixture",
			"  --json      Print the dry-run/apply result as JSON",
			"  --rollback  Soft-delete/cancel the inbound validation fixture instead of preparing it",
			"",
			"Default mode is dry-run. This script only prepares INV-FIX-INBOUND.",
		].join("\n"),
	);
}
...
async function prepareFixture(
	...
	const desiredSaleData = {
		title: "Inventory Validation Inbound Order",
		orderId: FIXTURE.saleOrderId,
	...
	const desiredLineComponentData = {
		...
		qtyInbound: 2,
		qtyReceived: 0,
		status: "inbound_required" as const,
	};
	...
	const desiredDemandData = {
		...
		notes: `${SEED_FIXTURE_ID} open inbound demand validation fixture`,
	};
...
async function rollbackFixture(
	...
	const lineComponentStep: SeedStep = {
		...
		id: lineComponent?.id,
		changes: lineComponent ? { status: "cancelled", qtyInbound: 0 } : {},
	};
...
function printHumanResult(payload: {
	...
	console.log("Inventory validation inbound fixture seed");
	...
	if (payload.mode == "dry-run") {
		console.log("No data was written. Re-run with --apply to create or repair INV-FIX-INBOUND.");
	} else if (payload.mode == "rollback-dry-run") {
		console.log(
			"No data was written. Re-run with --rollback --apply to soft-delete/cancel INV-FIX-INBOUND rows.",
		);
	}
...
main().catch((error) => {
	console.error(
		`[inventory-validation-seed-inbound-fixture] ${
